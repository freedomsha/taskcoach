...
class BaseTaskViewer(mixin.SearchableViewerMixin,  # pylint: disable=W0223
    ...
    def __DisplayBalloon(self):
        if self.toolbar.getToolIdByCommand('ViewerHideTasks_completed') != wx.ID_ANY and \
                self.toolbar.IsShownOnScreen() and hasattr(wx.GetTopLevelParent(self), 'AddBalloonTip'):
            wx.GetTopLevelParent(self).AddBalloonTip(self.settings, 'filtershiftclick', self.toolbar, message=_('''Shift-click on a filter tool to see only 
                                                     tasks belonging to the corresponding status'''),
...
class BaseTaskTreeViewer(BaseTaskViewer):  # pylint: disable=W0223
    ...
    def createTaskPopupMenu(self):
        ...
        return TaskPopupMenu(self.parent, self.settings,
...
class HierarchicalCalendarViewer(mixin.AttachmentDropTargetMixin,
    ...
    def onLayoutAttributeChanged(self, new_value, sender):
        self.refresh()
...
class CalendarViewer(mixin.AttachmentDropTargetMixin,
    ...
    def createModeToolBarUICommands(self):
        return super().createModeToolBarUICommands() + (None, uicommand.CalendarViewerConfigure(viewer=self),
    ...
    def reconfig(self):
        ...
        try:
            ...
            if hcolor:
                highlight_color = wx.Colour(*tuple([int(c) for c in hcolor.split(',')]))
...
class TaskViewer(mixin.AttachmentDropTargetMixin,
    ...
    def activate(self):
        if hasattr(wx.GetTopLevelParent(self), 'AddBalloonTip'):
            wx.GetTopLevelParent(self).AddBalloonTip(self.settings, 'manualordering', self.widget, message=_(
                '''Show the "Manual ordering" column, then drag and drop items 
                from this column to sort them arbitrarily.'''),
                                                     title=_('Manual ordering'), getRect=lambda: wx.Rect(0, 0, 28, 16))
...
class TaskStatsViewer(BaseTaskViewer):  # pylint: disable=W0223
    ...
    def refreshParts(self):
        ...
        for part, status in zip(series, task.Task.possibleStatuses()):
            ...
            percentage = round(old_div(100. * nrTasks, total)) if total else 0
...
try:
    ...
    class TaskInterdepsViewer(BaseTaskViewer):
        ...
        def determine_vertex_weight(budget, priority):
            budg_h = old_div(budget.total_seconds(), 3600)
            return (budg_h + priority * (budg_h + 1) + 10) % 200

        @staticmethod
        def convert_rgba_to_rgb(rgba):
            rgb = (rgba[0], rgba[1], rgba[2])
            return "#" + struct.pack('BBB', *rgb).decode('hex')  # unresolved attribute reference encode for class bytes
        ...
        def form_depend_graph(self):
            ...
            # visual_style["vertex_size"] = [(i_deg//max_i_degree) * 20 + vert_w
            visual_style["vertex_size"] = [(old_div(i_deg, max_i_degree)) * 20 + vert_w
