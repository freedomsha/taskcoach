...
def contains_any(strings, setchars):
    """Check whether 'str' contains ANY of the chars in 'set'"""
    return 1 in [c in strings for c in setchars]
...
def get_files_for_name(name):
    ...
    if not os.path.exists(name):
        ...
        if contains_any(name, "*?[]"):
            ...
            listfiles = []
            for file in files:
                listfiles.extend(get_files_for_name(file))
            return listfiles
...
class TokenEater:
    ...
    def __waiting(self, ttype, tstring, lineno):
        ...
        if ttype == tokenize.STRING:
            # maybe_fstring = ast.parse(tstring, mode='eval').body  # TO FIX
            ast_expression = ast.parse(tstring, mode='eval')
            maybe_fstring = ast_expression.body
