...
class PieCtrlLegend(wx.Window):
    ...
    def OnPaint(self, event):
        ...
        for ii in range(len(self._parent._series)):
            ...
            mdc.DrawCircle(self._horborder+5, dy+old_div(th,2), 5)
...
class PieCtrl(wx.Window):
    ...
    def GetPartAngles(self):
        ...
        for ii in range(len(self._series)):
            ...
            angles.append(old_div(360.0*current,total))
        ...
        for ii in range(len(angles)):
            ...
            if ii > 0:
                ...
                if angles[ii-1] != angles[ii]:
                    dc.DrawEllipticArc(0, int((1-sin(self._angle))*(old_div(h,2))+cy), w,
                                       int(h*sin(self._angle)),
                                       angles[ii-1]+old_div(self._rotationangle,pi)*180,
                                       angles[ii]+old_div(self._rotationangle,pi)*180)
        ...
        if len(self._series) > 0:
            ...
            while x <= 2*pi:
                ...
                if angleindex < len(angles):
                    ...
                    if old_div(x,pi)*180.0 >= angles[angleindex+1]:

                        changeangle = True
                        x = angles[angleindex+1]*pi/180.0

                points[0] = points[1]
                px = int(old_div(w,2)*(1+cos(x+self._rotationangle)))
                py = int(old_div(h,2)-old_div(sin(self._angle)*h,2)*sin(x+self._rotationangle)-1)
                points[1] = [px, py]
                triangle[0] = [old_div(w, 2), old_div(h, 2)]
                triangle[1] = points[0]
                triangle[2] = points[1]
            ...
            while x <= 2*pi:
                ...
                if angleindex < len(angles):
                    ...
                    if old_div(x,pi)*180 >= angles[angleindex+1]:

                        changeangle = True
                        x = old_div(angles[angleindex+1]*pi,180)
...
class ProgressPie(PieCtrl):
    ...
    def SetValue(self, value):
        ...
        self._series[0].set_value(max(self._value, 0.0))
        self._series[1].set_value(max(self._maxvalue - self._value, 0.0))
        self.Refresh()
