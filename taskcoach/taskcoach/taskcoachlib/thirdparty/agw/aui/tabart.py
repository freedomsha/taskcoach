...
class AuiDefaultTabArt(object):
    ...
    def SetSizingInfo(self, tab_ctrl_size, tab_count, minMaxTabWidth):
        ...
        if tab_count > 0:
            self._fixed_tab_width = old_div(tot_width, tab_count)

        if self._fixed_tab_width < 100:
            self._fixed_tab_width = 100

        if self._fixed_tab_width > old_div(tot_width, 2):
            self._fixed_tab_width = old_div(tot_width, 2)
    ...
    def GetTabSize(self, dc, wnd, caption, bitmap, active, close_button_state, control=None):
        ...
        dc.set_font(self._measuring_font)
    ...
    def DrawButton(self, dc, wnd, in_rect, button, orientation):
        ...
        if orientation == wx.LEFT:
            ...
            rect.SetX(in_rect.x)
            rect.SetY((old_div((in_rect.y + in_rect.height), 2)) - (old_div(bmp.GetHeight(), 2)))
            ...
            rect = wx.Rect(in_rect.x + in_rect.width - bmp.GetWidth(),
                           (old_div((in_rect.y + in_rect.height), 2)) - (old_div(bmp.GetHeight(), 2)),
    ...
    def DrawFocusRectangle(self, dc, page, wnd, draw_text, text_offset, bitmap_offset, drawn_tab_yoff, drawn_tab_height,
        ...
        if page.active and wx.Window.FindFocus() == wnd:
            ...
            focusRectText = wx.Rect(text_offset, (drawn_tab_yoff + old_div((drawn_tab_height), 2) - (old_div(texty, 2))),
                                    textx, texty)

            if page.bitmap.IsOk():
                focusRectBitmap = wx.Rect(bitmap_offset,
                                          drawn_tab_yoff + (old_div(drawn_tab_height, 2)) - (old_div(page.bitmap.GetHeight(), 2)),
    ...
    def ShowDropDown(self, wnd, pages, active_idx):
        ...
        for i, page in enumerate(pages):
            ...
            if useImages:
                ...
                if page.bitmap:
                    menuItem.set_bitmap(page.bitmap)
...
class AuiSimpleTabArt(object):
    ...
    def __init__(self):
        ...
        self._normal_font = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        self._selected_font = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        self._selected_font.SetWeight(wx.BOLD)
        self._measuring_font = self._selected_font
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                if control:
                    text_offset = tab_x + (old_div(tab_height, 2)) + close_button_width - (old_div(textx, 2)) - 2
                else:
                    text_offset = tab_x + (old_div(tab_height, 2)) + (old_div((tab_width + close_button_width), 2)) - (old_div(textx, 2)) - 2
            else:
                if control:
                    text_offset = tab_x + (old_div(tab_height, 2)) + close_button_width - (old_div(textx, 2))
                else:
                    text_offset = tab_x + (old_div(tab_height, 2)) + (old_div((tab_width - close_button_width), 2)) - (old_div(textx, 2))

        else:

            text_offset = tab_x + (old_div(tab_height, 3)) + (old_div(tab_width, 2)) - (old_div(textx, 2))
            if control:
                if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                    text_offset = tab_x + (old_div(tab_height, 3)) - (old_div(textx, 2)) + close_button_width + 2
                else:
                    text_offset = tab_x + (old_div(tab_height, 3)) - (old_div(textx, 2))
        ...
        ypos = old_div((tab_y + tab_height), 2) - (old_div(texty, 2)) + 1
        ...
        if page.active and wx.Window.FindFocus() == wnd and (agwFlags & AUI_NB_NO_TAB_FOCUS) == 0:
            focusRect = wx.Rect(text_offset, (old_div((tab_y + tab_height), 2) - (old_div(texty, 2)) + 1),
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + tab_height - 2,
                               tab_y + (old_div(tab_height, 2)) - (old_div(bmp.GetHeight(), 2)) + 1,
    ...
    def GetTabSize(self, dc, wnd, caption, bitmap, active, close_button_state, control=None):
        ...
        x_extent = tab_width - (old_div(tab_height, 2)) - 1
    ...
    def DrawTab(self, dc, wnd, page, in_rect, close_button_state, paint_control=False):
        ...
        if pagebitmap.IsOk():
            ...
            dc.DrawBitmap(pagebitmap, bitmap_offset,
                          drawn_tab_yoff + (old_div(drawn_tab_height, 2)) - (old_div(pagebitmap.GetHeight(), 2)) + shift,
                          True)
        ...
        ypos = drawn_tab_yoff + old_div((drawn_tab_height), 2) - (old_div(texty, 2)) - 1 + shift
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + 4,
                               drawn_tab_yoff + (old_div(drawn_tab_height, 2)) - (old_div(bmp.GetHeight(), 2)) + shift,
                               close_button_width, tab_height)
            else:
...
class FF2TabArt(AuiDefaultTabArt):
    ...
    def DrawTabBackground(self, dc, rect, focus, upperTabs):
        ...
        if focus:
            if upperTabs:
                leftPt = wx.Point(rect.x, rect.y + (old_div(rect.height, 10)) * 8)
                rightPt = wx.Point(rect.x + rect.width - 2, rect.y + (old_div(rect.height, 10)) * 8)
            else:
                leftPt = wx.Point(rect.x, rect.y + (old_div(rect.height, 10)) * 5)
                rightPt = wx.Point(rect.x + rect.width - 2, rect.y + (old_div(rect.height, 10)) * 5)
        else:
            leftPt = wx.Point(rect.x, rect.y + (old_div(rect.height, 2)))
            rightPt = wx.Point(rect.x + rect.width - 2, rect.y + (old_div(rect.height, 2)))
...
class ChromeTabArt(AuiDefaultTabArt):
    ...
    def __init__(self):
        ...
        closeBmp = tab_close.get_bitmap()
        closeHBmp = tab_close_h.get_bitmap()
        closePBmp = tab_close_p.get_bitmap()
    ...
    def SetBitmaps(self, mirror):
        ...
        bmps = [tab_active_left.get_bitmap(), tab_active_center.get_bitmap(),
                tab_active_right.get_bitmap(), tab_inactive_left.get_bitmap(),
                tab_inactive_center.get_bitmap(), tab_inactive_right.get_bitmap()]
        ...
        if close_button_state != AUI_BUTTON_STATE_HIDDEN:
            ...
            if agwFlags & AUI_NB_CLOSE_ON_TAB_LEFT:
                rect = wx.Rect(tab_x + leftw - 2,
                               drawn_tab_yoff + (old_div(drawn_tab_height, 2)) - (old_div(bmp.GetHeight(), 2)) + 1,
                               close_button_width, tab_height)
            else:
