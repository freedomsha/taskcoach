...
def getShadow():
    ...
    sh_tr = wx.ImageFromStream(io.StringIO(getDataTR())).ConvertToBitmap()
    sh_bl = wx.ImageFromStream(io.StringIO(getDataBL())).ConvertToBitmap()
    sh_sh = wx.ImageFromStream(io.StringIO(getDataSH())).Rescale(500, 500, wx.IMAGE_QUALITY_HIGH)
...
class ScrolledThumbnail(wx.ScrolledWindow):
    ...
    def SetThumbSize(self, width, height, border=6):
        ...
        self.SetScrollRate(old_div((self._tWidth + self._tBorder),4),
                           old_div((self._tHeight + self._tBorder),4))
    ...
    def ShowThumbs(self, thumbs, caption):
        ...
        _thread.start_new_thread(self.ThreadImage, (newfiles,))
        wx.MilliSleep(20)
    ...
    def SetCaption(self, caption=""):
        ...
        if self._labelcontrol:
            ...
            maxWidth = old_div(self._labelcontrol.GetSize().GetWidth(),8)
    ...
    def GetItemIndex(self, x, y):
        ...
        col = old_div((x - self._tBorder),(self._tWidth + self._tBorder))
    ...
    def UpdateProp(self, checkSize=True):
        ...
        self._cols = old_div((width - self._tBorder),(self._tWidth + self._tBorder))
        
        if self._cols == 0:
            self._cols = 1

        tmpvar = (len(self._items)%self._cols and [1] or [0])[0]
        self._rows = old_div(len(self._items),self._cols) + tmpvar
    ...
    def ScrollToSelected(self):
        ...
        # get row
        row = old_div(self.GetSelection(),self._cols)
        ...
        # scroll view
        xu, yu = self.GetScrollPixelsPerUnit()
        sy = old_div(sy,yu) + (sy%yu and [1] or [0])[0] # convert sy to scroll units
    ...
    def DrawThumbnail(self, bmp, thumb, index):
        ...
        x = old_div(self._tBorder,2)
        y = old_div(self._tBorder,2)
        ...
        # image
        img = thumb.get_bitmap(self._tWidth, self._tHeight)
        ww = img.GetWidth()
        hh = img.GetHeight()
        ...
        imgRect = wx.Rect(x + old_div((self._tWidth - img.GetWidth()),2),
                          y + old_div((self._tHeight - img.GetHeight()),2),
        ...
        if self._showfilenames:
            textWidth = 0
            dc.set_font(self.GetCaptionFont())
            ...
            textWidth = sw + 8
            tx = x + old_div((self._tWidth - textWidth),2)
            ty = y + self._tHeight

            txtcolour = "#7D7D7D"
            dc.SetTextForeground(txtcolour) 
            
            tx = x + old_div((self._tWidth - sw),2)
            if hh >= self._tHeight:
                ty = y + self._tHeight + old_div((self._tTextHeight - sh),2) + 3
            else:
                ty = y + hh + old_div((self._tHeight-hh),2) + old_div((self._tTextHeight - sh),2) + 3
    ...
    def OnPaint(self, event):
        ...
        for ii in range(len(self._items)):
            ...
            xwhite = old_div(((w - self._cols*(self._tWidth + self._tBorder))),(self._cols+1))
            tx = xwhite + col*(self._tWidth + self._tBorder)

            ty = old_div(self._tBorder,2) + row*(self._tHeight + self._tBorder) + \
        ...
        rect = wx.Rect(xwhite, old_div(self._tBorder,2),
    ...
    def Rotate(self, angle=90):
        ...
        for thumb in selected:
            count = count + 1
            if TN_USE_PIL:
                newangle = old_div(thumb.GetRotation()*180,pi) + angle
                fil = opj(thumb.GetFullFileName())
                pil = Image.open(fil).rotate(newangle)
                img = wx.EmptyImage(pil.size[0], pil.size[1])
                img.SetData(pil.convert('RGB').tostring())
                thumb.SetRotation(old_div(newangle*pi,180))
            else:
                img = thumb._threadedimage
                newangle = thumb.GetRotation() + old_div(angle*pi,180)
                thumb.SetRotation(newangle)
                img = img.Rotate(newangle, (old_div(img.GetWidth(),2), old_div(img.GetHeight(),2)), True)
