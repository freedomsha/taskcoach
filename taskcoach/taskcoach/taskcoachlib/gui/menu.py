...
class Menu(wx.Menu, uicommandcontainer.UICommandContainerMixin):
    ...
    def appendUICommand(self, uiCommand):
        ...
        # Why this line try ?
        try:
            print(f"tclib.gui.menu.py.Menu.appendUICommand Essaie d'ajout de UICommand au menu {self}"
                  f" dans {self._window}")
...
class DynamicMenu(Menu):
    ...
    def myId(self):
        ...
        # Lorsque cela sera corrigé, remplacez les 7 lignes ci-dessous par celle-ci:
...
class ViewViewerMenu(Menu):
    def __init__(self, mainwindow, settings, viewerContainer, taskFile):
        ...
        try:
            ...
            viewViewerCommands.append(ViewViewer(
...
class TaskPopupMenu(Menu):
    def __init__(self, mainwindow, settings, tasks, efforts, categories, taskViewer):
        super().__init__(mainwindow)
        self.appendUICommands(  # Les commandes de menu sont ici
...
class ColumnPopupMenu(ColumnPopupMenuMixin, Menu):
    ...
    def appendUICommands(self, *args, **kwargs):
        ...
        try:
            ...
            # print(f"Append UI Command: {uiCommand}, id = {uiCommand.id}") # Ajout de journalisation
        # except wx.PyDeadObjectError:
        except RuntimeError:
            pass
