...
class WindowGeometryTracker:
    ...
    def _load_dialog_geometry(self, x, y, width, height, min_w, min_h):
        ...
        if size_missing:
            log.debug("Rule 2: Missing saved size, letting system decide")
            self._clear_dialog_cache()
            return
    ...
    def _center_on_parent_with_size(self, width, height):
        ...
        self.size = (width, height)
        self._window.SetSize(x, y, width, height)
        log.debug(f"Centered on parent: pos={self.position} size={self.size}")
    ...
    def _clear_dialog_cache(self):
        ...
        self.position = None
        self.size = None
        log.debug("Cleared dialog geometry cache")

    def _clear_position_cache(self):
        """Clear only position cache in settings."""
        self._set_setting("position", (-1, -1))
        log.debug("Cleared position cache")
    ...
    def check_and_correct(self):
        ...
        if self._window.IsIconized():
            log.debug(f"ERROR: Window is iconized before ready!")
            log.debug(
        ...
        if self._window.IsMaximized():
            log.debug(f"ERROR: Window is maximized before ready!")
            log.debug(
                f"  Desired state was: pos={self.position} size={self.size} maximized={self.maximized}"
            )
            self._clear_state()
    ...
    def _mark_ready(self):
        ...
        if self.maximized:
            log.debug(f"  Maximizing (fire and forget)")
            self._window.Maximize()
    ...
    def _on_maximize(self, event):
        ...
        log.debug(f"EVT_MAXIMIZE: IsMaximized={self._window.IsMaximized()}")
        if not self.ready:
            self.check_and_correct()
        else:
            self.cache_from_window()
    ...
    def _validate_geometry(self, x, y, width, height):
        ...
        for i in range(num_displays):
            ...
            if (
                ...
                log.debug(f"  Geometry valid for monitor {i}")
                return (x, y, width, height)

        log.debug(f"Position ({x}, {y}) not valid for any monitor")
        return None
    ...
    def _log_position_tick(self):
        ...
        log.debug(f"_log_position_tick: ready={self.ready}")
        ...
        log.debug(f"_log_position_tick: scheduled next tick in {interval}ms")
        # return
...
class WindowDimensionsTracker(WindowGeometryTracker):
    ...
    def __init__(self, window, settings):
        ...
        if self.__start_iconized():
            ...
            if not operating_system.isMac() and self._get_setting(
                ...
                wx.LogDebug(
                    "WindowDimensionsTracker : CallAfter passé avec succès."
                )
    ...
    def __start_iconized(self):
        ...
        if start_iconized == "Never":
            return False
        return self._get_setting("iconized")

    def save_position(self) -> None:
        """Save the position of the window in the settings."""
        iconized = self._window.IsIconized()
        self._set_setting("iconized", iconized)
        if not iconized:
            self._set_setting("position", self._window.GetPosition())
        self.save()
