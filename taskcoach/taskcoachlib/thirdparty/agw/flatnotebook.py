...
def PaintStraightGradientBox(dc, rect, startColour, endColour, vertical=True):
    ...
    for i in range(high+1):
        ...
        r = startColour.Red() + old_div((old_div((i*rd*100),high)),100)
        g = startColour.Green() + old_div((old_div((i*gd*100),high)),100)
        b = startColour.Blue() + old_div((old_div((i*bd*100),high)),100)
...
class TabNavigatorWindow(wx.Dialog):
    ...
    def OnPanelPaint(self, event):
        ...
        bmpPt.y = old_div((rect.height - self._bmp.GetHeight()),2)
        bmpPt.x = 3
...
class FNBRenderer(object):
    ...
    def CalcTabWidth(self, pageContainer, tabIdx, tabHeight):
        ...
        if hasImage:
            ...
            if not pc.IsDefaultTabs():
                ...
                # Default style
                tabWidth += 16 + pc._pParent.GetPadding() + old_div(shapePoints,2)
    ...
    def DrawTabs(self, pageContainer, dc):
        ...
        for i in range(pc._nFrom, len(pc._pagesInfoVec)):
            ...
            dc.set_font((i == pc.GetSelection() and [boldFont] or [normalFont])[0])
...
class FNBRendererDefault(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        if hasImage:
            textOffset = 2*pc._pParent._nPadding + 16 + old_div(shapePoints,2)
        else:
            textOffset = pc._pParent._nPadding + old_div(shapePoints,2)
...
class FNBRendererFirefox2(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        if hasImage:
            textOffset = 2*padding + 16 + old_div(shapePoints,2)
        else:
            textOffset = padding + old_div(shapePoints,2)
...
class FNBRendererRibbonTabs(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        spacer = math.ceil(old_div(float(FNB_HEIGHT_SPACER)/2,2))
        ...
        padding = pc.GetParent().GetPadding()
        hasImage = pc._pagesInfoVec[tabIdx].GetImageIndex() != -1
        imageYCoord = old_div(FNB_HEIGHT_SPACER,2)
...
class PageContainer(wx.PyPanel):
    ...
    def OnMouseWheel(self, event):
        ...
        steps = int(old_div(rotation,delta))
    ...
    def OnRightDown(self, event):
        ...
        if where in [FNB_TAB, FNB_TAB_X]:
            ...
            if self._pagesInfoVec[tabIdx].GetEnabled():
                ...
                event = FlatNotebookEvent(wxEVT_FLATNOTEBOOK_PAGE_CONTEXT_MENU, self.GetParent().get_id())
                event.SetSelection(tabIdx)
...
##        self.DoSetSelection(page)
    ...
    def DoDeletePage(self, page):
        ...
        if self._iActivePage >= 0:
            ...
            event = FlatNotebookEvent(wxEVT_FLATNOTEBOOK_PAGE_CHANGING, self.GetParent().get_id())
    ...
    def ShowTabTooltip(self, tabIdx):
        ...
        if pWindow:
            ...
            if pToolTip and pToolTip.get_window() == pWindow:
                self.SetToolTipString(pToolTip.get_tip())
    ...
    def OnDropTarget(self, x, y, nTabPage, wnd_oldContainer):
        ...
        if oldNotebook == newNotebook:
            ...
            if nTabPage >= 0:
                ...
                event = FlatNotebookEvent(wxEVT_FLATNOTEBOOK_PAGE_DROPPED, self.GetParent().get_id())
                ...
                if nTabPage >= 0:
                    ...
                    if window:
                        ...
                        if imageindex >= 0:
                            ...
                            bmp = oldNotebook.GetImageList().get_bitmap(imageindex)
                    ...
                    event = FlatNotebookDragEvent(wxEVT_FLATNOTEBOOK_PAGE_DROPPED_FOREIGN, self.GetParent().get_id())
                    event.SetSelection(nIndex)
                    event.SetOldSelection(nTabPage)
    ...
    def PopupTabsMenu(self):
        ...
        for i in range(len(self._pagesInfoVec)):
            ...
            if wx.VERSION > (2, 6, 3, 0) and self.TabHasImage(i):
                item.set_bitmap(self.GetImageList().get_bitmap(pi.GetImageIndex()))
