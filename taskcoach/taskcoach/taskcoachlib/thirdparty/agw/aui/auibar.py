...
class CommandToolBarEvent(wx.PyCommandEvent):
    ...
    def __init__(self, command_type, win_id):
        ...
        if isinstance(command_type, int):
            ...
            wx.PyCommandEvent.__init__(self, command_type.GetEventType(), command_type.get_id())
...
class AuiToolBarEvent(CommandToolBarEvent):
    ...
    def __init__(self, command_type=None, win_id=0):
        ...
        if isinstance(command_type, int):
            ...
            self.notify = wx.NotifyEvent(command_type.GetEventType(), command_type.get_id())
...
class AuiDefaultToolBarArt(object):
    ...
    def DrawLabel(self, dc, wnd, item, rect):
        ...
        if orient == AUI_TBTOOL_HORIZONTAL:
            text_x = rect.x
            text_y = rect.y + old_div((rect.height - text_height), 2)
            dc.DrawText(item.GetLabel(), text_x, text_y)

        elif orient == AUI_TBTOOL_VERT_CLOCKWISE:
            text_x = rect.x + old_div((rect.width + text_width), 2)
            text_y = rect.y
    ...
    def DrawDropDownButton(self, dc, wnd, item, rect):
        ...
        dropdown_rect = wx.Rect(rect.x + rect.width - BUTTON_DROPDOWN_WIDTH - 1, rect.y, BUTTON_DROPDOWN_WIDTH + 1,
                                rect.height)
        ...
        if horizontal:
            ...
            dropdown_rect = wx.Rect(rect.x, rect.y + rect.height - BUTTON_DROPDOWN_WIDTH - 1, rect.width,
        ...
        dropbmp_x = dropdown_rect.x + (old_div(dropdown_rect.width, 2)) - old_div(dropbmp_width, 2)
        dropbmp_y = dropdown_rect.y + (old_div(dropdown_rect.height, 2)) - old_div(dropbmp_height, 2)
        ...
        if horizontal:
            ...
            dc.DrawBitmap(wx.BitmapFromImage(
                dropbmp.ConvertToImage().Rotate90(item.GetOrientation() == AUI_TBTOOL_VERT_CLOCKWISE)),
    ...
    def DrawControlLabel(self, dc, wnd, item, rect):
        ...
        text_x = rect.x + (old_div(rect.width, 2)) - (old_div(text_width, 2)) + 1
    ...
    def GetToolSize(self, dc, wnd, item):
        ...
        if not item.get_bitmap().IsOk() and not self._agwFlags & AUI_TB_TEXT:
            return wx.Size(16, 16)

        width = item.get_bitmap().GetWidth()
        height = item.get_bitmap().GetHeight()
    ...
    def DrawSeparator(self, dc, wnd, _rect):
        ...
        if horizontal:
            ...
            rect.x += (old_div(rect.width, 2))
            rect.width = 1
            new_height = old_div((rect.height * 3), 4)
            rect.y += (old_div(rect.height, 2)) - (old_div(new_height, 2))
            rect.height = new_height

        else:

            rect.y += (old_div(rect.height, 2))
            rect.height = 1
            new_width = old_div((rect.width * 3), 4)
            rect.x += (old_div(rect.width, 2)) - (old_div(new_width, 2))
    ...
    def DrawOverflowButton(self, dc, wnd, rect, state):
        ...
        x = rect.x + 1 + old_div((rect.width - self._overflow_bmp.GetWidth()), 2)
        y = rect.y + 1 + old_div((rect.height - self._overflow_bmp.GetHeight()), 2)
    ...
    def ShowDropDown(self, wnd, items):
        ...
        for item in items:
            ...
            if item.GetKind() not in [ITEM_SEPARATOR, ITEM_SPACER, ITEM_CONTROL]:
                ...
                kind = item.GetKind()
                m = wx.MenuItem(menuPopup, item.get_id(), text, item.GetShortHelp(), kind)
                orientation = item.GetOrientation()
                item.SetOrientation(AUI_TBTOOL_HORIZONTAL)

                if kind not in [ITEM_CHECK, ITEM_RADIO]:
                    m.set_bitmap(item.get_bitmap())
    ...
    def GetToolsPosition(self, dc, item, rect):
        ...
        bmp_width = item.get_bitmap().GetWidth()
        bmp_height = item.get_bitmap().GetHeight()
        ...
        if horizontal and text_bottom:
            bmp_x = rect.x + (old_div(rect.width, 2)) - (old_div(bmp_width, 2))
            bmp_y = rect.y + 3
            text_x = rect.x + (old_div(rect.width, 2)) - (old_div(text_width, 2))
            text_y = rect.y + ((bmp_y - rect.y) * 2) + bmp_height

        elif horizontal and text_right:
            bmp_x = rect.x + 3
            bmp_y = rect.y + (old_div(rect.height, 2)) - (old_div(bmp_height, 2))
            text_x = rect.x + ((bmp_x - rect.x) * 2) + bmp_width
            text_y = rect.y + (old_div(rect.height, 2)) - (old_div(text_height, 2))
...
class AuiToolBar(wx.PyControl):
    ...
    def AddToggleTool(self, tool_id, bitmap, disabled_bitmap, toggle=False, client_data=None, short_help_string="",
                      long_help_string=""):
        """
        ...
        return self.AddTool(tool_id, "", bitmap, disabled_bitmap, kind, short_help_string, long_help_string,
                            client_data)

    def AddTool(self, tool_id, label, bitmap, disabled_bitmap, kind, short_help_string='', long_help_string='',
    ...
    def AddCheckTool(self, tool_id, label, bitmap, disabled_bitmap, short_help_string="", long_help_string="",
                     client_data=None):
        """
        ...
        return self.AddTool(tool_id, label, bitmap, disabled_bitmap, ITEM_CHECK, short_help_string, long_help_string,
                            client_data)

    def AddRadioTool(self, tool_id, label, bitmap, disabled_bitmap, short_help_string="", long_help_string="",
                     client_data=None):
        """
        ...
        return self.AddTool(tool_id, label, bitmap, disabled_bitmap, ITEM_RADIO, short_help_string, long_help_string,
                            client_data)
    ...
    def SetFont(self, font):
        ...
        res = wx.PyControl.set_font(self, font)
    ...
    def GetToolFitsByIndex(self, tool_id):
        ...
        rect = self._items[tool_id].sizer_item.get_rect()
    ...
    def GetToolRect(self, tool_id):
        ...
        if tool and tool.sizer_item:
            return tool.sizer_item.get_rect()
    ...
    def GetOverflowRect(self):
        ...
        overflow_rect = wx.Rect(*self._overflow_sizer_item.get_rect())
        overflow_size = self._art.GetElementSize(AUI_TBART_OVERFLOW_SIZE)
    ...
    def OnPaint(self, event):
        ...
        if self._agwStyle & AUI_TB_GRIPPER and gripper_size > 0 and self._gripper_sizer_item:
            gripper_rect = wx.Rect(*self._gripper_sizer_item.get_rect())
            if horizontal:
        ...
        for item in self._items:
            ...
            item_rect = wx.Rect(*item.sizer_item.get_rect())
    ...
    def OnLeftDown(self, event):
        ...
        if self._action_item:
            ...
            rect = wx.Rect(*self._action_item.sizer_item.get_rect())
    ...
    def OnRightDown(self, event):
        ...
        if self._gripper_sizer_item:
            gripper_rect = self._gripper_sizer_item.get_rect()
            if gripper_rect.Contains(event.GetPosition()):
                return
