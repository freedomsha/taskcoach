...
class TabTextCtrl(ExpandoTextCtrl):
    ...
    def __init__(self, owner, tab, page_index):
        ...
        y = old_div((self._tabEdited.rect.height - h), 2) + 1
...
class TabNavigatorWindow(wx.Dialog):
    ...
    def __init__(self, parent, props):
        ...
        if not self._props.Icon.IsOk():
            self._props.Icon = Mondrian.get_bitmap()
    ...
    def OnPanelPaint(self, event):
        ...
        bmpPt.y = old_div((rect.height - self._props.Icon.GetHeight()), 2)
        bmpPt.x = 3
        ...
        txtPt.y = old_div((rect.height - fontHeight), 2)
...
class AuiTabContainer(object):
    ...
    def MinimizeTabOffset(self, dc, wnd, max_width):
        ...
        for i, page in reversed(list(enumerate(self._pages))):
            ...
            (tab_width, tab_height), x_extent = self._art.GetTabSize(dc, wnd, page.caption, page.bitmap, page.active,
                                                                     tab_button.cur_state, page.control)
    ...
    def Render(self, raw_dc, wnd):
        ...
        if self._tab_offset > 0:
            ...
            rect = wx.Rect(offset - x_extent, 0, self._rect.width - right_buttons_width - offset - x_extent - 2,
                           self._rect.height)
        ...
        if self._tab_offset <= active < len(self._pages):
    ...
    def IsTabVisible(self, tabPage, tabOffset, dc, wnd):
        ...
        for i in range(tabOffset, page_count):
            ...
            if i == tabPage:
                ...
                if (self._rect.width - right_buttons_width - offset - 2) <= 0 and (
                        self._rect.width - right_buttons_width - left_buttons_width) > x_extent:
                    return False
                else:
                    return True
...
class AuiTabCtrl(wx.PyControl, AuiTabContainer):
    ...
    def OnKeyDown2(self, event):
        ...
        :warning: This method implementation is now deprecated. Refer to L{on_key_down}
         for the correct one.
        """
...
class AuiNotebook(wx.PyPanel):
    ...
    def InitNotebook(self, agwStyle):
        ...
        self._mgr.AddPane(self._dummy_wnd,
                          framemanager.AuiPaneInfo().Name("dummy").Bottom().CaptionVisible(False).Show(False))
        self._mgr.Update()
    ...
    def SetPageImage(self, page, image):
        ...
        bitmap = self._imageList.get_bitmap(image)
    ...
    def GetPageImage(self, page):
        ...
        for indx in range(self._imageList.GetImageCount()):
            imgListBmp = self._imageList.get_bitmap(indx)
    ...
    def Split(self, page, direction):
        ...
        if direction == wx.LEFT:
            ...
            pane_info.Left()
            mouse_pt = wx.Point(0, old_div(cli_size.y, 2))

        elif direction == wx.RIGHT:

            pane_info.Right()
            mouse_pt = wx.Point(cli_size.x, old_div(cli_size.y, 2))

        elif direction == wx.TOP:

            pane_info.Top()
            mouse_pt = wx.Point(old_div(cli_size.x, 2), 0)

        elif direction == wx.BOTTOM:

            pane_info.Bottom()
            mouse_pt = wx.Point(old_div(cli_size.x, 2), cli_size.y)
        ...
        if self._agwFlags & AUI_NB_TAB_EXTERNAL_MOVE:
            ...
            if tab_ctrl:
                ...
                if nb != self:
                    ...
                    e3 = AuiNotebookEvent(wxEVT_COMMAND_AUINOTEBOOK_DRAG_DONE, nb.get_id())
    ...
    ##        # which was focused
    ##        idx = self._tabs.GetIdxFromWindow(event.get_window())
    ...
    def OnTabButton(self, event):
        ...
        if button_id == AUI_BUTTON_CLOSE:
            ...
            if selection != -1:
                ...
                if close_wnd.get_name() == "__fake__page__":
    ...
    def NotebookPreview(self, thumbnail_size=200):
        ...
        self.AddPage(listCtrl, "AuiNotebook Preview", True, bitmap=auinotebook_preview.get_bitmap(),
