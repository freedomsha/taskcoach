...
class AquaButton(wx.PyControl):
    ...
    def OnPaint(self, event):
        ...
        pos_x = old_div((width-bw-tw),2)+btnOffset-shadowOffset      # adjust for bitmap and text to centre
        if self._bitmap:
            pos_y =  old_div((height-bh-shadowOffset),2)+btnOffset
            gc.DrawBitmap(self._bitmap, pos_x, pos_y, bw, bh) # draw bitmap if available
            pos_x = pos_x + 2   # extra spacing from bitmap

        # Create a Path to draw the text
        gc.DrawText(label, pos_x + bw + btnOffset, old_div((height-th-shadowOffset),2)+btnOffset)      # draw the text
        ...
        if self._saveBitmap:
            ...
            gcMemory.set_font(font)
            gcMemory.DrawText(label, pos_x + bw + btnOffset, old_div((height-th-shadowOffset),2)+btnOffset)
    ...
    def SetDefault(self):
        ...
        :note: Under Windows, only dialog box buttons respond to this function. As normal
         ...
         when the return key is pressed. See also `wx.Window.set_focus` which sets the
