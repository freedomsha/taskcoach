...
class FMRenderer(object):
    ...
    def __init__(self):
        ...
        self.itemTextColourDisabled = ArtManager.Get().LightColour(wx.SystemSettings_GetColour(wx.SYS_COLOUR_GRAYTEXT),
                                                                   30)
    ...
    def DrawSeparator(self, dc, xCoord, yCoord, textX, sepWidth):
        ...
        dcsaver = DCSaver(dc)
        sepRect1 = wx.Rect(xCoord + textX, yCoord + 1, old_div(sepWidth, 2), 1)
        sepRect2 = wx.Rect(xCoord + textX + old_div(sepWidth, 2), yCoord + 1, old_div(sepWidth, 2) - 1, 1)
    ...
    def DrawMenuItem(self, item, dc, xCoord, yCoord, imageMarginX, markerMarginX, textX, rightMarginX, selected=False,
        ...
        if bmp.Ok():
            ...
            if imageMarginX == 0:
                xx = rect.x + old_div((leftMarginWidth - imgWidth), 2)
            else:
                xx = rect.x + old_div(((leftMarginWidth - rect.height) - imgWidth), 2) + rect.height

            yy = rect.y + old_div((rect.height - imgHeight), 2)
        ...
        if item.GetKind() == wx.ITEM_CHECK:
            ...
            if item.IsChecked():
                ...
                dc.DrawBitmap(item._checkMarkBmp, rr.x + old_div((rr.width - 16), 2),
                              rr.y + old_div((rr.height - 16), 2), True)
        ...
        if item.GetKind() == wx.ITEM_RADIO:
            ...
            if item.IsChecked():
                ...
                dc.DrawBitmap(item._radioMarkBmp, rr.x + old_div((rr.width - 16), 2),
                              rr.y + old_div((rr.height - 16), 2), True)
        ...
        if text:
            ...
            if item.IsEnabled() and item.get_text_colour():
                textColour = item.get_text_colour()
            ...
            if item._mnemonicIdx != wx.NOT_FOUND:
                ...
                posy = old_div((itemHeight - h), 2) + yCoord
                ...
                w, h = dc.GetTextExtent(text)
                dc.DrawText(text, xCoord + textX + borderXSize, old_div((itemHeight - h), 2) + yCoord)
        ...
        if item.GetAccelString():
            ...
            dc.DrawText(item.GetAccelString(), xCoord + rightMarginX - accelWidth,
                        old_div((itemHeight - accelHeight), 2) + yCoord)
        ...
        if item.GetSubMenu():
            ...
            dc.DrawBitmap(rightArrowBmp, rr.x + 4, rr.y + old_div((rr.height - 16), 2), True)
    ...
    def DrawMenuBar(self, menubar, dc):
        ...
        for item in menubar._items:
            ...
            rect = wx.Rect(posx + old_div(padding, 2), posy, textWidth, textHeight)
            ...
            textOffset = old_div((rect.width - ww), 2)
...
class FMRendererMSOffice2007(FMRenderer):
    ...
    def DrawLeftMargin(self, item, dc, menuRect):
        ...
        dc.DrawLine(marginRect.x + marginRect.width, marginRect.y, marginRect.x + marginRect.width,
                    marginRect.y + marginRect.height)

        borderColour = artMgr.DarkColour(faceColour, 10)
        dc.SetPen(wx.Pen(borderColour))
        dc.DrawLine(marginRect.x + marginRect.width - 1, marginRect.y, marginRect.x + marginRect.width - 1,
                    marginRect.y + marginRect.height)
...
class FlatMenuBar(wx.Panel):
    ...
    def OnIdle(self, event):
        ...
        if self._watch.Time() > self._interval:
            ...
            for but in self._tbButtons:
                event = wx.UpdateUIEvent(but._tbItem.get_id())
    ...
    def GetOptions(self):
        ...
        :see: L{set_options} for a list of valid options.
        """
    ...
    def UpdateItem(self, item):
        ...
        for but in self._tbButtons:
            if but._tbItem.get_id() == item.get_id():
    ...
    def DrawToolbar(self, dc, rect):
        ...
        for i in range(len(self._tbButtons)):
            ...
            if tbItem.IsSeparator():
                ...
                ctrlPos = wx.Point(xx, rect.y + old_div((rect.height - ctrlSize.y), 2))
            ...
            if bmp.Ok():
                ...
                x = xx - old_div(self._toolbarSpacer, 2)
                # y = rect.y + (rect.height - bmp.GetHeight())/2 - 1
                y = rect.y + old_div(self._toolbarMargin, 2)
                ...
                if i < len(self._tbButtons) and i >= 0:
                    ...
                    if self._tbButtons[i]._state == ControlFocus or self._tbButtons[i]._tbItem.IsSelected():
                        self.GetRenderer().DrawMenuBarButton(dc, buttonRect,
                                                             tmpState)  # TODO DrawToolbarButton? With separate toolbar colors
                    else:
                        self._tbButtons[i]._state = ControlNormal

                imgx = buttonRect.x + old_div((buttonRect.width - bmp.GetWidth()), 2)
                imgy = buttonRect.y + old_div((buttonRect.height - bmp.GetHeight()), 2)
    ...
    def DrawMoreButton(self, dc, state):
        ...
        if (
        not self._showCustomize) and self.GetInvisibleMenuItemCount() < 1 and self.GetInvisibleToolbarItemCount() < 1:
            return
        ...
        xx = rect.x + old_div((DROP_DOWN_ARROW_WIDTH - dropArrowBmp.GetWidth()), 2)
        yy = rect.y + old_div((rect.height - dropArrowBmp.GetHeight()), 2)
    ...
    def DrawToolbarItem(self, dc, idx, state):
        ...
        x = rect.x + old_div((buttonRect.width - self._tbButtons[idx]._tbItem.get_bitmap().GetWidth()), 2)
        y = rect.y + old_div((buttonRect.height - self._tbButtons[idx]._tbItem.get_bitmap().GetHeight()), 2)

        if state == ControlFocus:
            # place a dark image under the original image to provide it
            # with some shadow
            # shadow = ConvertToMonochrome(self._tbButtons[idx]._tbItem.get_bitmap())
            ...
            x -= 1
            y -= 1
        dc.DrawBitmap(self._tbButtons[idx]._tbItem.get_bitmap(), x, y, True)
    ...
    def DoToolbarAction(self, idx):
        ...
        if tbItem.IsRegularItem() or tbItem.IsCheckItem() or tbItem.IsRadioItem():
            # Create the event
            event = wx.CommandEvent(wxEVT_FLAT_MENU_SELECTED, tbItem.get_id())
            event.SetEventObject(self)
    ...
    def FindMenu(self, title):
        ...
        for ii, item in enumerate(self._items):
            accelIdx, labelOnly = GetAccelIndex(item.get_title())

            if labelOnly == title or item.get_title() == title:
                return ii
        ...
        if self._showToolbar and invT > 0:
            ...
            for i in range(len(self._tbButtons) - invT, len(self._tbButtons)):
                if self._tbButtons[i]._tbItem.IsSeparator():
                    ...
                    tbitem = self._tbButtons[i]._tbItem
                    item = FlatMenuItem(self._tbMenu, tbitem.get_id(), tbitem.GetLabel(), "", wx.ITEM_NORMAL, None,
                                        tbitem.get_bitmap(), tbitem.GetDisabledBitmap())
    ...
    def AddTool(self, toolId, label="", bitmap1=wx.NullBitmap, bitmap2=wx.NullBitmap,
        ...
        self._tbButtons.append(
            ToolBarItem(FlatToolbarItem(bitmap1, toolId, label, bitmap2, kind, shortHelp, longHelp), wx.Rect(),
                        ControlNormal))
    ...
    def SetUpdateInterval(self, interval):
        ...
        :param integer `interval`: the update_ui interval in milliseconds.
        """
    ...
    def PositionAUI(self, mgr, fixToolbar=True):
        ...
        pn.Caption("Menu Bar")
        pn.Top()
        pn.MinSize(wx.Size(old_div(xx, 2), self._barHeight))
...
class FlatMenuButton(object):
    ...
    def Draw(self, dc):
        ...
        rect = wx.RectPS(self._pos, self._size)
        xx = rect.x + old_div((rect.width - self._normalBmp.GetWidth()), 2)
        yy = rect.y + old_div((rect.height - self._normalBmp.GetHeight()), 2)
...
class FlatMenu(FlatMenuBase):
    ...
    def AddItem(self, menuItem):
        ...
        if self._originalBackgroundImage:
            ...
            img = img.Scale(self._menuWidth * self._numCols - 2 - self._leftMarginWidth, menuHeight,
        ...
        if accel:
            accel.Set(accel.GetFlags(), accel.GetKeyCode(), menuItem.get_id())
    ...
    def GetMenuItemWidth(self, menuItem):
        ...
        font = menuItem.get_font()
        if font is None:
            font = ArtManager.Get().get_font()
        ...
        if menuItem.get_bitmap().Ok():
            bmpHeight = menuItem.get_bitmap().GetHeight()
            bmpWidth = menuItem.get_bitmap().GetWidth()
        ...
        # left margin
        factor = (((menuItem.get_bitmap() != wx.NullBitmap) and \
    ...
    def HitTest(self, pos):
        ...
        for ii, item in enumerate(self._itemsArr):
            ...
            if item.get_rect().Contains(pos) and item.IsEnabled() and item.IsShown():
    ...
    def SendOverItem(self, itemIdx, over):
        ...
        # Create the event
        event = FlatMenuEvent((over and [wxEVT_FLAT_MENU_ITEM_MOUSE_OVER] or [wxEVT_FLAT_MENU_ITEM_MOUSE_OUT])[0],
                              item.GetId())
    ...
    def Clear(self):
        ...
        for ii in range(lenItems):
            self.Destroy(self._itemsArr[0].get_id())
    ...
    def OnTimer(self, event):
        ...
        if self._upButton and self._upButton.GetTimerId() == event.get_id():

            self.ScrollUp()

        elif self._downButton and self._downButton.GetTimerId() == event.get_id():
