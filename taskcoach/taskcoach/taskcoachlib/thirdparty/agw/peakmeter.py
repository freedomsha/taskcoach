...
class PeakMeterCtrl(wx.PyControl):
    ...
    def DoTimerProcessing(self):
        ...
        decValue  = old_div(self._maxValue,self._ledBands)
        noChange = True
        ...
        for pm in self._meterData:
            ...
            if pm._value > 0:
                ...
                pm._value -= (self._ledBands > 1 and [decValue] or [old_div(self._maxValue*BAND_PERCENT,100)])[0]
                if pm._value < 0:
                    pm._value = 0
    ...
    def DrawHorzBand(self, dc, rect):
        ...
        horzBands = (self._ledBands > 1 and [self._ledBands] or [old_div(self._maxValue*BAND_PERCENT,100)])[0]
        minHorzLimit = old_div(self._minValue*horzBands,self._maxValue)
        medHorzLimit = old_div(self._medValue*horzBands,self._maxValue)
        maxHorzLimit = horzBands

        size = wx.Size(old_div(rect.width,horzBands), old_div(rect.height,self._numBands))
        ...
        for vert in range(self._numBands):
            ...
            horzLimit = old_div(self._value*horzBands,self._maxValue)
    ...
    def DrawVertBand(self, dc, rect):
        ...
        vertBands = (self._ledBands > 1 and [self._ledBands] or [old_div(self._maxValue*BAND_PERCENT,100)])[0]
        minVertLimit = old_div(self._minValue*vertBands,self._maxValue)
        medVertLimit = old_div(self._medValue*vertBands,self._maxValue)
        maxVertLimit = vertBands

        size = wx.Size(old_div(rect.width,self._numBands), old_div(rect.height,vertBands))
        rectBand = wx.RectPS(rect.GetTopLeft(), size)
        ...
        for horz in range(self._numBands):
            ...
            vertLimit = old_div(self._value*vertBands,self._maxValue)
            ...
            if self._showFalloff:
                ...
                points[0].y = rectPrev.GetBottomRight().y - old_div(self._meterData[horz]._falloff*maxHeight,self._maxValue)
