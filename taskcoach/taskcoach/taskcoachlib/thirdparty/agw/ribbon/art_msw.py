...
class RibbonMSWArtProvider(object):
        ...
        if self._flags & RIBBON_BAR_SHOW_PAGE_ICONS:
            ...
            if icon.IsOk():
                ...
                if self._flags & RIBBON_BAR_SHOW_PAGE_LABELS == 0:
                    x = tab.rect.x + old_div((tab.rect.width - icon.GetWidth()), 2)
                    
                dc.DrawBitmap(icon, x, tab.rect.y + 1 + old_div((tab.rect.height - 1 - icon.GetHeight()), 2), True)
        
        if self._flags & RIBBON_BAR_SHOW_PAGE_LABELS:
            label = tab.page.GetLabel()
            if label.strip():            
                dc.set_font(self._tab_label_font)
                ...
                y = tab.rect.y + old_div((tab.rect.height - text_height), 2)

                if width <= text_width:                
                    dc.SetClippingRegion(x, tab.rect.y, width, tab.rect.height)
                    dc.DrawText(label, x, y)
                else:                
                    dc.DrawText(label, x + old_div((width - text_width), 2) + 1, y)
    ...
    def DrawScrollButton(self, dc, wnd, rect_, style):
        ...
        if result == RIBBON_SCROLL_BTN_LEFT:
            arrow_points[0] = wx.Point(old_div(rect.width, 2) - 2, old_div(rect.height, 2))
            ...
            arrow_points[0] = wx.Point(old_div(rect.width, 2), old_div(rect.height, 2) - 2)
            if style & RIBBON_SCROLL_BTN_ACTIVE:
                arrow_points[0].y += 1
            arrow_points[1] = arrow_points[0] + wx.Point( 3, 3)
            arrow_points[2] = arrow_points[0] + wx.Point(-3, 3)
    ...
    def DrawPanelBackground(self, dc, wnd, rect):
        ...
        dc.set_font(self._panel_label_font)
        dc.SetPen(wx.TRANSPARENT_PEN)
        ...
        if clip_label:        
            ...
            dc.DrawText(label, label_rect.x, label_rect.y + old_div((label_rect.GetHeight() - label_size.GetHeight()), 2))        
        else:        
            dc.DrawText(label, label_rect.x + old_div((label_rect.GetWidth() - label_size.GetWidth()), 2),
                        label_rect.y + old_div((label_rect.GetHeight() - label_size.GetHeight()), 2))
    ...
    def DrawGalleryBackgroundCommon(self, dc, wnd, rect):
        ...
        if self._flags & RIBBON_BAR_FLOW_VERTICAL:        
            ...
            up_btn = wx.Rect(rect.x, rect.y + rect.height - 15, old_div(rect.width, 3), 15)
            ...
            up_btn = wx.Rect(rect.x + rect.width - 15, rect.y, 15, old_div(rect.height, 3))
    ...
    def DrawGalleryButton(self, dc, rect, state, bitmaps):
        ...
        dc.DrawRectangle(rect.x, rect.y, rect.width, old_div(rect.height, 2))

        lower = wx.Rect(*rect)
        lower.height = old_div((lower.height + 1), 2)
        lower.y += rect.height - lower.height
        dc.GradientFillLinear(lower, btn_colour, btn_grad_colour, wx.SOUTH)

        dc.DrawBitmap(btn_bitmap, rect.x + old_div(rect.width, 2) - 2, lower.y - 2, True)
    ...
    def DrawMinimisedPanel(self, dc, wnd, rect, bitmap):
        ...
        if wnd.GetExpandedPanel() != None:        
            ...
            client_rect.width -= 2
            client_rect.y += 1
            client_rect.height = (rect.y + old_div(rect.height, 5)) - client_rect.x
        ...
        mid_pos = rect.y + old_div(rect.height, 5) - preview.y
        ...
        if bitmap.IsOk():        
            dc.DrawBitmap(bitmap, preview.x + old_div((preview.width - bitmap.GetWidth()), 2),
                          preview.y + old_div((preview.height - 7 - bitmap.GetHeight()), 2), True)
    ...
    def DrawMinimisedPanelCommon(self, dc, wnd, true_rect):
        ...
        if self._flags & RIBBON_BAR_FLOW_VERTICAL:        
            ...
            preview.y = true_rect.y + old_div((true_rect.height - preview.height), 2)
        else:        
            preview.x = true_rect.x + old_div((true_rect.width - preview.width), 2)
            preview.y = true_rect.y + 4
        
        dc.set_font(self._panel_label_font)
        label_width, label_height = dc.GetTextExtent(wnd.GetLabel())

        xpos = true_rect.x + old_div((true_rect.width - label_width + 1), 2)
        ypos = preview.y + preview.height + 5

        if self._flags & RIBBON_BAR_FLOW_VERTICAL:        
            xpos = preview.x + preview.width + 5
            ypos = true_rect.y + old_div((true_rect.height - label_height), 2)
        ...
        if self._flags & RIBBON_BAR_FLOW_VERTICAL:
            xpos += label_width
            arrow_points[0] = wx.Point(xpos + 5, ypos + old_div(label_height, 2))
            arrow_points[1] = arrow_points[0] + wx.Point(-3,  3)
            arrow_points[2] = arrow_points[0] + wx.Point(-3, -3)
        else:        
            ypos += label_height
            arrow_points[0] = wx.Point(old_div(true_rect.width, 2), ypos + 5)
            arrow_points[1] = arrow_points[0] + wx.Point(-3, -3)
            arrow_points[2] = arrow_points[0] + wx.Point( 3, -3)
    ...
    def DrawButtonBarButton(self, dc, wnd, rect, kind, state, label, bitmap_large, bitmap_small):
        ...
        dc.set_font(self._button_bar_label_font)
    ...
    def DrawButtonBarButtonForeground(self, dc, rect, kind, state, label, bitmap_large, bitmap_small):
        ...
        if result == RIBBON_BUTTONBAR_BUTTON_LARGE:
            ...
            padding = 2
            dc.DrawBitmap(bitmap_large, rect.x + old_div((rect.width - bitmap_large.GetWidth()), 2),
                          rect.y + padding, True)
            ...
            if label_w + 2 * padding <= rect.width:
                ...
                dc.DrawText(label, rect.x + old_div((rect.width - label_w), 2), ypos)
                if arrow_width != 0:                
                    self.DrawDropdownArrow(dc, rect.x + old_div(rect.width, 2),
                                           ypos + old_div((label_h * 3), 2),
            ...
            x_cursor = rect.x + 2
            dc.DrawBitmap(bitmap_small, x_cursor, rect.y + old_div((rect.height - bitmap_small.GetHeight()),2), True)
            x_cursor += bitmap_small.GetWidth() + 2
            label_w, label_h = dc.GetTextExtent(label)
            dc.DrawText(label, x_cursor, rect.y + old_div((rect.height - label_h), 2))
            x_cursor += label_w + 3
            
            if kind != RIBBON_BUTTON_NORMAL:
                self.DrawDropdownArrow(dc, x_cursor, rect.y + old_div(rect.height, 2),
                                       self._button_bar_label_colour)
    ...
    def DrawTool(self, dc, wnd, rect, bitmap, kind, state):
        ...
        :param `wnd`: The window which is being drawn onto. In most cases this will
         be a L{RibbonToolBar}, but it doesn't have to be;
        :param `rect`: The rectangle within which to draw. The size of this rectangle
         will at least the size returned by L{get_tool_size}, and the height of it will
        ...
        # Background
        bg_rect_top = wx.Rect(*bg_rect)
        bg_rect_top.height = old_div((bg_rect_top.height * 2), 5)
        ...
        if kind != RIBBON_BUTTON_NORMAL:        
            ...
            dc.DrawBitmap(self._toolbar_drop_bitmap, bg_rect.x + avail_width + 2,
                          bg_rect.y + (old_div(bg_rect.height, 2)) - 2, True)
        
        dc.DrawBitmap(bitmap, bg_rect.x + old_div((avail_width - bitmap.GetWidth()), 2),
                      bg_rect.y + old_div((bg_rect.height - bitmap.GetHeight()), 2), True)
    ...
    def GetGalleryClientSize(self, dc, wnd, size, client_offset=None, scroll_up_button=None,
        ...
        if self._flags & RIBBON_BAR_FLOW_VERTICAL:
            ...
            scroll_up.height = 15
            scroll_up.x = 0
            scroll_up.width = old_div((size.GetWidth() + 2), 3)
            ...
            scroll_up.width = 15
            scroll_up.y = 0
            scroll_up.height = old_div((size.GetHeight() + 2), 3)
