...
class BPArt(object):
    ...
    def __init__(self, parentStyle):
        ...
        sysfont = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
    ...
    def SetFont(self, id, font):
        ...
        :see: L{get_font} for a list of meaningful font ids.
        """
        ...
        if isVertical:
            ...
            if text != "":
                ...
                if textAlignment == BP_BUTTONTEXT_ALIGN_RIGHT:
                    fullExtent = bmpxsize + old_div(padding.x, 2) + textW
                    bmpypos = rect.y + old_div((rect.height - bmpysize), 2)
                    bmpxpos = rect.x + old_div((rect.width - fullExtent), 2)
                    textxpos = bmpxpos + old_div(padding.x, 2) + bmpxsize
                    textypos = bmpypos + old_div((bmpysize - textH), 2)
                else:
                    bmpxpos = rect.x + old_div((rect.width - bmpxsize), 2)
                    bmpypos = rect.y + padding.y
                    textxpos = rect.x + old_div((rect.width - textW), 2)
                    textypos = bmpypos + bmpysize + old_div(padding.y, 2)
            else:
                bmpxpos = rect.x + old_div((rect.width - bmpxsize), 2)
                bmpypos = rect.y + old_div((rect.height - bmpysize), 2)
            ...
            if text != "":
                ...
                if textAlignment == BP_BUTTONTEXT_ALIGN_RIGHT:
                    ...
                    fullExtent = bmpysize + old_div(padding.y, 2) + textH
                    bmpxpos = rect.x + old_div((rect.width - bmpxsize), 2)
                    bmpypos = rect.y + old_div((rect.height - fullExtent), 2)
                    textxpos = rect.x + old_div((rect.width - textW), 2)
                    textypos = bmpypos + bmpysize + old_div(padding.y, 2)
            else:
                bmpxpos = rect.x + old_div((rect.width - bmpxsize), 2)
...
class ButtonPanelText(Control):
    ...
    def CreateDC(self):
        ...
        textFont = self._parent._art.get_font(BP_TEXT_FONT)
        dc.set_font(textFont)
...
class ButtonInfo(Control):
    ...
    def GetBestSize(self):
        ...
        if self.HasText():
            ...
            dc = wx.ClientDC(self._parent)
            normalFont = self._parent._art.get_font(BP_BUTTONTEXT_FONT)
            dc.set_font(normalFont)
        ...
        for child in children:
            ...
            if userData:
                ...
                if child.IsSpacer():
                    ...
                    self.AddControl(child.get_window(), child.GetProportion(),
