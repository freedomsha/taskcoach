...
def viewer2html(viewer, settings, css_filename=None, selectionOnly=False, columns=None):
    converter = Viewer2HTMLConverter(viewer, settings)
    columns = columns or viewer.visibleColumns()
    return converter(css_filename, columns, selectionOnly)
...
class Viewer2HTMLConverter(object):
    ...
    def style(self, level, includeAllCSS):
        ...
        for column, alignment in zip(visibleColumns, columnAlignments):
            column_style = self.indent('.%s {text-align: %s}' % (column.name(), alignment), level + 1)
            styleContent.append(column_style)
        if self.viewer.isShowingTasks():
            for status in task.Task.possibleStatuses():
                status_color = task.Task.fgColorForStatus(status)
                status_color = self.cssColorSyntax(status_color)
                status_style = '.%s {color: %s}' % (status, status_color)
                styleContent.append(self.indent(status_style, level + 1))
        if includeAllCSS:
    ...
    def htmlBody(self, columns, selection_only, printing, level):
        ...
        if printing:
            htmlBodyContent.append(self.wrap(self.viewer.title(), 'h1', level,
                                             oneLine=True))
        htmlBodyContent.extend(self.table(columns, selection_only, printing, level + 1))
        return self.wrap(htmlBodyContent, 'body', level)

    def table(self, columns, selection_only, printing, level):
        ...
        tableContent.extend(self.tableHeader(columns, printing, level + 1) +
                            self.tableBody(columns, selection_only,
                                           printing, level + 1))
    ...
    def wrap(cls, lines, tagName, level, oneLine=False, **attributes):
        ...
        if attributes:
            attributes = ' ' + ' '.join(sorted('%s="%s"' % (key, value) for key, value in list(attributes.items())))
        else:
            attributes = ''
        ...
        if oneLine:
            ...
            return [cls.indent(openTag, level)] + \
                lines + \
    ...
    def render(item, column, indent=False):
        ...
        if column.name() == 'notes':
            ...
            return u'<br />'.join(map(html.escape, sorted([attachment.subject() for attachment in item.attachments()])))
