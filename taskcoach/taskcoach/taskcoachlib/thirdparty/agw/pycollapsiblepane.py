...
Usage example::
    ...
    class MyFrame(wx.Frame):
        ...
        def __init__(self, parent):
            ...
            title.set_font(wx.Font(18, wx.SWISS, wx.NORMAL, wx.BOLD))
...
class GTKExpander(wx.PyControl):
    ...
    def OnDrawGTKExpander(self, dc):
        ...
        if self._parent.IsCollapsed():
            startX, startY = old_div(triangleWidth,2), size.y - triangleHeight - 1 - descent
            pt1 = wx.Point(startX, startY)
            pt2 = wx.Point(startX, size.y - 1 - descent)
            pt3 = wx.Point(startX+triangleWidth, size.y-old_div(triangleHeight,2) - 1 - descent)
        else:

            startX, startY = 0, size.y - triangleWidth - descent - 1
            pt1 = wx.Point(startX, startY)
            pt2 = wx.Point(startX+triangleHeight, startY)
            pt3 = wx.Point(startX+old_div(triangleHeight,2), size.y - descent - 1)
        ...
        if self.HasAGWFlag(CP_USE_STATICBOX):
            ...
            if self.IsExpanded():
                ...
                if 'wxMac' not in wx.PlatformInfo:
                    adjustment = -3
                self._sz.Children[0].AssignSpacer((1, old_div(sz.height,2) + adjustment))
                self._contentSizer.Children[0].AssignSpacer((1, old_div(sz.height,2)))
