...
class Set(set):
    """ The builtin set type does not like keyword arguments, so to keep
        ...
        Le type set intégré n'aime pas les arguments de mots-clés,
        donc pour le garder heureux, nous ne les transmettons pas.
        """

    def __new__(cls, iterable=None, *args, **kwargs):
        return set.__new__(cls, iterable)  # Unexpected argument 'iterable'
...
class Event:
    ...
    def __repr__(self):  # pragma: no cover
        ...
        return 'Event({})'.format(self.__sourcesAndValuesByType)
...
class Observer:
    def __init__(self, *args, **kwargs):
        ...
        # super().__init__(*args, **kwargs)  # TypeError: set() takes no keyword arguments
        # j'essais sans **kwargs :
        super().__init__(*args)
...
class ObservableCollection:
    ...
    def addItemEventType(cls):
        """ The event type used to notify observers that one or more items
            have been added to the collectio
            n. """
        return '%s.add' % cls
    ...
    def modificationEventTypes(cls):
        try:
            # Unresolved attribute reference 'modificationEventTypes' for class 'object'
            eventTypes = super().modificationEventTypes()  # ?
        except AttributeError:
            eventTypes = []
        return eventTypes + [cls.addItemEventType(),
