...
class ZoomBarImage(object):
    ...
    def ZoomImage(self, nxcoord):
        ...
        if nxcoord < self._vCenter:
            inc = int(old_div(((nxcoord - self._oldLeft)*10.0),(self._oldWidth/2.0))) + 1
        else:
            inc = int(old_div(((self._oldLeft + self._oldWidth - nxcoord)*10.0),(old_div(self._oldWidth,2)))) + 1
        ...
        if not self._isAReflection:
            ...
            self._left = self._vCenter - old_div(self._width,2)
...
class ImageBar(object):
    ...
    def SetBarColour(self, colour, xSize=None, ySize=None):
        ...
        bitmap.Rescale(self._size.width, old_div(self._size.height,2))
...
class ZoomBar(wx.PyControl):
    ...
    def ResetSize(self):
        ...
        if self._showReflections:
            ySize += old_div(self._buttonSize,2)
        if self._centerZoom:
    ...
    def InitialReposition(self):
        ...
        center = old_div(size.GetWidth(),2)
        nbCenter = old_div(totalWidth,2)
        nLeft = center - nbCenter

        if self._showReflections:
            nTop = self._imgBar._top - (old_div(oButton._height,2))
        else:
            if self._centerZoom:
                nTop = old_div(size.height,2) - old_div(oButton._height,2)
            else:
    ...
    def SnapToBottom(self, size):
        ...
        xPos = old_div((size.width - backgroundSize.width),2)
    ...
    def DrawLabels(self, dc):
        ...
        for button in self._buttons:
            ...
            xpos = buttonPos.x + old_div((buttonSize.width - textWidth),2)
