...
# ]
tips = [
    _('''{name} is actively developed. Although the {name} developers try hard to prevent them, 
        bugs do happen. So, backing up your work on a regular basis is strongly advised.'''.format(**meta.metaDict)),
    _('''{name} has a mailing list where you can discuss usage of {name} with fellow users, discuss and
        request features and complain about bugs. Go to {urlpo} and join today!'''.format(**meta.metaDict)),
    _('''{name} has unlimited undo and redo. Any change that you make, be it editing a task description, 
        or deleting an effort record, is undoable. Select 'Edit' -> 'Undo' and 'Edit' -> 'Redo' to go backwards and 
        forwards through your edit history.'''.format(**meta.metaDict)),
    _('''{name} is available in a number of different languages. Select 'Edit' -> 'Preferences' to see whether 
        your language is one of them. If your language is not available or the translation needs improvement, 
        please consider helping with the translation of {name}. Visit {urlpo} for more information about how you 
        can help.'''.format(**meta.metaDict)),
    _('''If you enter a URL (e.g. {urlpo}) in a task or effort description, it becomes a link. Clicking on the 
        link will open the URL in your default web browser.'''.format(**meta.metaDict)),
    _('''You can drag and drop tasks in the tree view to rearrange parent-child relationships between tasks. The 
        same goes for categories.'''),
    _('''You can drag files from a file browser onto a task to create attachments. Dragging the files over a tab 
        will raise the appropriate page, dragging the files over a collapsed task (the boxed + sign) in the tree view 
...
class TipDialog(sized_controls.SizedDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        wx.StaticBitmap(pane,
                        bitmap=wx.ArtProvider.GetBitmap('lamp_icon',
                                                        wx.ART_MENU, (32, 32)))
    ...
    def __create_checkbox(self, pane):
        """ Create a checkbox for users to indicate whether they want to
            see tips on startup. """
