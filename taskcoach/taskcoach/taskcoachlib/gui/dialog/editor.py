...
class AttachmentSubjectPage(SubjectPage):
    ...
    def addLocationEntry(self):
        ...
        # pylint: disable=W0201
        current_location = self.items[0].location() if len(self.items) == 1 \
...
class TaskAppearancePage(Page):
    ...
    def addColorEntry(self, labelText, colorType, defaultColor):
        currentColor = getattr(self.items[0], '%sColor' % colorType)(recursive=False) if len(
            self.items) == 1 else None
...
class DatesPage(Page):
    ...
    def addDateEntry(self, label, TaskMethodName):
        ...
        datetimeSync = attributesync.AttributeSync(TaskMethodName,
                                                    ...
                                                    callback=self.__onPlannedStartDateTimeChanged
                                                    if TaskMethodName == 'plannedStartDateTime' else None)
    ...
    def addReminderEntry(self):
        ...
        self._reminderDateTimeSync = attributesync.AttributeSync('reminder',
                                                                 self._reminderDateTimeEntry, reminderDateTime,
                                                                 self.items,
...
class ProgressPage(Page):
    ...
    def addProgressEntry(self):
        # pylint: disable=W0201
        currentPercentageComplete = self.items[0].percentageComplete() if len(
            self.items) == 1 else self.averagePercentageComplete(self.items)
...
class LocalCategoryViewer(category.BaseCategoryViewer):  # pylint: disable=W0223
...
class LocalAttachmentViewer(viewer.attachment.AttachmentViewer):  # pylint: disable=W0223
...
class LocalNoteViewer(viewer.note.BaseNoteViewer):  # pylint: disable=W0223
    def __init__(self, *args, **kwargs):
...
class LocalPrerequisiteViewer(viewer.task.CheckableTaskViewer):  # pylint: disable=W0223
    def __init__(self, items, *args, **kwargs):
        self.__items = items
        super().__init__(*args, **kwargs)
...
class EditBook(widgets.Notebook):
    ...
    def NavigateBook(self, forward):
        ...
        if 0 <= cur_sel < self.GetPageCount():
    ...
    def __load_perspective(self, items_are_new=False):
        ...
        if perspective:
            ...
            finally:  # pylint: disable=W0702  finally not except
                pass
        if items_are_new:
...
class EffortEditBook(Page):
    ...
    def __add_start_and_stop_entries(self):
        ...
        self._startDateTimeEntry = entry.DateTimeEntry(self, self._settings,
                                                       current_start_date_time, noneAllowed=False, showRelative=True,
        ...
        self._startDateTimeSync = attributesync.AttributeSync('getStart',
                                                              self._startDateTimeEntry, current_start_date_time,
                                                              self.items,
        ...
        self._stopDateTimeEntry = entry.DateTimeEntry(self, self._settings,
                                                      ...
                                                      units=[(_('Minute(s)'), 60), (_('Hour(s)'), 3600),
                                                             (_('Day(s)'), 24 * 3600), (_('Week(s)'), 7 * 24 * 3600)],
                                                      **date_time_entry_kw_args)
        self._stopDateTimeSync = attributesync.AttributeSync('getStop',
                                                             self._stopDateTimeEntry, current_stop_date_time,
                                                             self.items,
...
class Editor(BalloonTipManager, widgets.Dialog):
    ...
    EditBookClass = lambda *args: 'Subclass responsibility'  # PEP 8: E731 do not assign a lambda expression, use a def
    ...
    def __init__(self, parent, items, settings, container, task_file,
        ...
        if operating_system.isMac():
            ...
            # be pending changes...
            id_ = IdProvider.get()  # Parameter 'self' unfilled -> Obtenir un identifiant unique
        ...
        self.__dimensions_tracker = windowdimensionstracker.WindowSizeAndPositionTracker(
            self, settings, self._interior.settingsSection())
    ...
    def __close_if_item_is_deleted(self, items):
        for item in items:
            if self._interior.isDisplayingItemOrChildOfItem(item) and \
                    not item in self._taskFile:  # PEP 8: E713 test for membership should be 'not in'
                self.Close()
                break
