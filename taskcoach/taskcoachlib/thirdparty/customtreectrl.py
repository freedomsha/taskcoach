...
""" L'élément de l'arborescence n'est ni sélectionné ni développé. """
TreeItemIcon_Selected = 1  # selected, not expanded
""" L'élément de l'arborescence est sélectionné et non développé. """
...
# wx.TR_EXTENDED n'existe pas dans wx a retirer de hypertreelist
...
class DragImage(wx.DragImage):
    ...
    def __init__(self, treeCtrl, item):
        ...
        if checkimage is not None:
            xcheckpos = 2
            ycheckpos = ((total_h > image_h) and [(total_h - image_h) / 2] or [0])[
                0
            ] + 2
    ...
    def CreateBitmap(self):
        ...
        memory.SetBackgroundMode(wx.PENSTYLE_TRANSPARENT)
        ...
        if wx.Platform != "__WXMAC__":
            ...
            for y in xrange(timg.GetHeight()):
                for x in xrange(timg.GetWidth()):
                    pix = wx.Colour(
                        timg.GetRed(x, y), timg.GetGreen(x, y), timg.GetBlue(x, y)
                    )
...
class CommandTreeEvent(wx.PyCommandEvent):
    ...
    def __init__(
        self, evtType, evtId, item=None, evtKey=None, point=None, label=None, **kwargs
    ):
        """
        Default class constructor.
        For internal use: do not call it in your code!
...
class TreeEvent(CommandTreeEvent):
    ...
    def __init__(
        ...
        CommandTreeEvent.__init__(
            self, evtType, evtId, item, evtKey, point, label, **kwargs
        )
...
class GenericTreeItem(object):
    ...
    def SetWindow(self, wnd):
        ...
        :param wx.Window wnd: a non-toplevel window to be displayed next to the item, any
    ...
    def Get3StateValue(self):
        ...
        if not self.Is3State():
            raise Exception(
                "Get3StateValue can only be used with 3-state checkbox items."
            )
        ...
        if not (level == 0 and theCtrl.HasAGWFlag(TR_HIDE_ROOT)):
            ...
            if self._y < point.y < self._y + h:
                ...
                if wx.Platform == "__WXMAC__":
                    ...
                    if (
                        xCross - 4 < point.x < xCross + 10
                            and y_mid - 4 < point.y < y_mid + 10
                        and self.HasPlus()
                        and theCtrl.HasButtons()
                    ):
                    ...
                    if (
                        xCross - 6 < point.x < xCross + 6
                            and y_mid - 6 < point.y < y_mid + 6
                        and self.HasPlus()
                        and theCtrl.HasButtons()
                    ):
                ...
                if self._x <= point.x <= self._x + self._width:
                    ...
                    if self.GetImage() != _NO_IMAGE and theCtrl._imageListNormal:
                        image_w, image_h = theCtrl._imageListNormal.GetSize(
                            self.GetImage()
                        )

                    if self.GetCheckedImage() is not None:
                        wcheck, hcheck = theCtrl._imageListCheck.GetSize(
                            self.GetCheckedImage()
                        )
...
class CustomTreeCtrl(wx.PyScrolledWindow):
    ...
    def __init__(
        ...
        self._grayedImageListLeft = None
        ...
        self._imageListNormal = self._imageListButtons = self._imageListState = (
            self._imageListCheck
        ) = self._imageListLeft = None
        self._ownsImageListNormal = self._ownsImageListButtons = (
            self._ownsImageListState
        ...
        self._boldFont = wx.Font(
            self._normalFont.GetPointSize(),
            family,
        ...
        self._firstcolour = colour = wx.SystemSettings.GetColour(
        ...
        wx.PyScrolledWindow.__init__(
            self, parent, id, pos, size, style | wx.HSCROLL | wx.VSCROLL, name
        )
    ...
    def SetAGWWindowStyleFlag(self, agwStyle):
        ...
        if (
            self._anchor
            and not self.HasAGWFlag(TR_HIDE_ROOT)
            and agwStyle & TR_HIDE_ROOT
        ):
    ...
    def GetPrevSibling(self, item):
        ...
        return (index == 0 and [None] or [siblings[index - 1]])[0]  # TODO ?
    ...
    def GetNextExpanded(self, item):
        ...
        if nextSibling:
            ...
            return self.GetNextExpanded(nextSibling)
    ...
    def GetPrevVisible(self, item):
        ...
        if (
            not self.HasChildren(lastGoodItem)
            or not self.IsExpanded(lastGoodItem)
            or (self.GetItemParent(item) == lastGoodItem)
        ):
            return lastGoodItem
    ...
    def FindItem(self, idParent, prefixOrig):
        ...
        if not id:
            ...
            while id != idParent and not self.GetItemText(id).lower().startswith(
                prefix
            ):
                id = self.GetNext(id)
    ...
    def DoInsertItem(
        ...
        item = GenericTreeItem(
            parent, text, ct_type, wnd, image, selImage, data, separator
        )
    ...
    def PrependItem(
        ...
        return self.DoInsertItem(
            parent, 0, text, ct_type, wnd, image, selImage, data, separator
        )

    def InsertItemByItem(
        self,
        parentId,
        idPrevious,
        text,
        ...
        if idPrevious:
            ...
            try:
                ...
                raise Exception(
                    "ERROR: Previous Item In CustomTreeCtrl.InsertItem() Is Not A Sibling"
                )

        return self.DoInsertItem(
            parentId, index + 1, text, ct_type, wnd, image, selImage, data, separator
        )

    def InsertItemByIndex(
        self,
        parentId,
        idPrevious,
        text,
        ...
        return self.DoInsertItem(
            parentId, idPrevious, text, ct_type, wnd, image, selImage, data, separator
        )
    ...
    def InsertItem(
        ...
        # if type(input) == type(1):  # E721 Use `is` and `is not` for type comparisons, or `isinstance()` for isinstance checks
        if type(input) is type(
            1
        ):  # E721 Use `is` and `is not` for type comparisons, or `isinstance()` for isinstance checks
            return self.InsertItemByIndex(
                parentId, input, text, ct_type, wnd, image, selImage, data, separator
            )
        else:
            return self.InsertItemByItem(
                parentId, input, text, ct_type, wnd, image, selImage, data, separator
            )

    def AppendItem(
        self, parentId, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None
    ):
        """
        ...
        return self.DoInsertItem(
            parent, len(parent.GetChildren()), text, ct_type, wnd, image, selImage, data
        )
    ...
    def ChildrenClosing(self, item):
        ...
        if (
            self._editCtrl is not None
            and item != self._editCtrl.item()
            and self.IsDescendantOf(item, self._editCtrl.item())
        ):
    ...
    def Delete(self, item):
        ...
        if self._editCtrl is not None and self.IsDescendantOf(
            item, self._editCtrl.item()
        ):
    ...
    def SelectAllChildren(self, item):
        ...
        if not self.HasAGWFlag(TR_MULTIPLE):
            raise Exception(
                "SelectAllChildren can be used only with multiple selection enabled."
            )
    ...
    def SelectAll(self):
        ...
        if not self.HasAGWFlag(TR_MULTIPLE):
            raise Exception(
                "SelectAll can be used only with multiple selection enabled."
            )
    ...
    def SelectItemRange(self, item1, item2):
        ...
        if not self.HasAGWFlag(TR_MULTIPLE):
            raise Exception(
                "SelectItemRange can be used only with multiple selection enabled."
            )
    ...
    def SetImageListCheck(self, sizex, sizey, imglist=None):
        ...
        if imglist is None:
            ...
            # Get the Checkboxes
            self._imageListCheck.Add(
                self.GetControlBmp(
                    checkbox=True, checked=True, enabled=True, x=sizex, y=sizey
                )
            )
            self._grayedCheckList.Add(
                self.GetControlBmp(
                    checkbox=True, checked=True, enabled=False, x=sizex, y=sizey
                )
            )

            self._imageListCheck.Add(
                self.GetControlBmp(
                    checkbox=True, checked=False, enabled=True, x=sizex, y=sizey
                )
            )
            self._grayedCheckList.Add(
                self.GetControlBmp(
                    checkbox=True, checked=False, enabled=False, x=sizex, y=sizey
                )
            )

            self._imageListCheck.Add(
                self.GetControlBmp(
                    checkbox=True, checked=2, enabled=True, x=sizex, y=sizey
                )
            )
            self._grayedCheckList.Add(
                self.GetControlBmp(
                    checkbox=True, checked=2, enabled=False, x=sizex, y=sizey
                )
            )
            ...
            self._imageListCheck.Add(
                self.GetControlBmp(
                    checkbox=False, checked=True, enabled=True, x=sizex, y=sizey
                )
            )
            self._grayedCheckList.Add(
                self.GetControlBmp(
                    checkbox=False, checked=True, enabled=False, x=sizex, y=sizey
                )
            )

            self._imageListCheck.Add(
                self.GetControlBmp(
                    checkbox=False, checked=False, enabled=True, x=sizex, y=sizey
                )
            )
            self._grayedCheckList.Add(
                self.GetControlBmp(
                    checkbox=False, checked=False, enabled=False, x=sizex, y=sizey
                )
            )
    ...
    def DrawVistaRectangle(self, dc, rect, hasfocus):
        ...
        filRect = wx.Rect(*rect.Get())  # TODO : Comment obtenir x, y,  width & height ?
        filRect.Deflate(1, 1)
        ...
        flrect = float(filRect.GetHeight())
        if flrect < 1:
        ...
        for y in xrange(filRect.GetY(), filRect.GetY() + filRect.GetHeight()):
            currCol = (r1 + rf, g1 + gf, b1 + bf)
            dc.SetBrush(wx.Brush(currCol, wx.SOLID))
            # dc.DrawRectangle(filRect.x, y, filRect.width, 1)
            dc.DrawRectangle(filRect.GetX, y, filRect.GetWidth(), 1)
            rf = rf + rstep
            gf = gf + gstep
            bf = bf + bstep
    ...
    def PaintItem(self, item, dc, level, align):
        ...
        if item.IsSelected():
            ...
            if wx.Platform == "__WXMAC__":
                ...
                dc.SetBrush(
                    (
                        self._hasFocus
                        and [self._hilightBrush]
                        or [self._hilightUnfocusedBrush]
                    )[0]
                )
        ...
        if image != _NO_IMAGE:
            ...
            dc.SetClippingRegion(
                item.GetX(), item.GetY(), wcheck + image_w - 2, total_h
            )
            if item.IsEnabled():
            ...
            imglist.Draw(
                ...
                item.GetX() + wcheck,
                item.GetY()
                + ((total_h > image_h) and [(total_h - image_h) / 2] or [0])[0],
        ...
        if wcheck:
            ...
            imglist.Draw(
                ...
                item.GetX(),
                item.GetY()
                + ((total_h > hcheck) and [(total_h - hcheck) / 2] or [0])[0],
                wx.IMAGELIST_DRAW_TRANSPARENT,
            )
        ...
        if leftimage != _NO_IMAGE:
            ...
            imglist.Draw(
                ...
                4,
                item.GetY()
                + ((total_h > l_image_h) and [(total_h - l_image_h) / 2] or [0])[0],
                wx.IMAGELIST_DRAW_TRANSPARENT,
            )
        ...
        textrect = wx.Rect(
            wcheck + image_w + item.GetX(), item.GetY() + extraH, text_w, text_h
        )
        ...
        if separator:
            ...
            dc.DrawLine(
                item.GetX() + 2, item.GetY() + total_h / 2, w, item.GetY() + total_h / 2
            )
            dc.SetPen(oldPen)
    ...
    def PaintLevel(self, item, dc, level, y, align):
        ...
        if not self.HasAGWFlag(TR_HIDE_ROOT):
            ...
            if count > 0:
                ...
                if (
                    not self.HasAGWFlag(TR_NO_LINES)
                    and self.HasAGWFlag(TR_LINES_AT_ROOT)
                    and count > 0
                ):
        ...
        if self.IsExposed(exposed_x, exposed_y, 10000, h):  # 10000 = very much
            if wx.Platform == "__WXMAC__":
                ...
                pen = (
                    (item.IsSelected() and self._hasFocus)
                    and [self._borderPen]
                    or [wx.TRANSPARENT_PEN]
                )[0]
            else:
                pen = self._borderPen
            ...
            if self.HasAGWFlag(TR_ROW_LINES):
                ...
                dc.SetPen(
                    (
                        (self.GetBackgroundColour() == wx.WHITE)
                        and [medium_grey]
                        or [wx.WHITE_PEN]
                    )[0]
                )
            ...
            if item.HasPlus() and self.HasButtons():
                ...
                if self._imageListButtons:
                    ...
                    self._imageListButtons.Draw(
                        image, dc, xx, yy, wx.IMAGELIST_DRAW_TRANSPARENT
                    )
                    dc.DestroyClippingRegion()
                    ...
                    if self.HasAGWFlag(TR_TWIST_BUTTONS):
                        ...
                        if item.IsExpanded():
                            ...
                            button = [
                                wx.Point(x=x - 5, y=y_mid - 3),
                                wx.Point(x + 5, y_mid - 3),
                                wx.Point(x, y_mid + 3),
                            ]
                        else:
                            # button[0].x = x - 3
                        ...
                        self._drawingfunction(
                            self,
                            dc,
                            wx.Rect(x - wImage / 2, y_mid - hImage / 2, wImage, hImage),
                            flag,
                        )
    ...
    def OnKeyDown(self, event):
        ...
        if keyCode in [ord("+"), wx.WXK_ADD]:  # "+"
            if (
                self._current.HasPlus()
                and not self.IsExpanded(self._current)
                and self.IsItemEnabled(self._current)
            ):
            ...
            event._pointDrag = wx.Point(
                itemRect.GetX(), itemRect.GetY() + itemRect.GetHeight() / 2
            )
            event.SetEventObject(self)
            self.GetEventHandler().ProcessEvent(event)
        ...
        # of its children if it's expanded
            ...
            if not prev:
                ...
                if prev:
                    ...
                    if current == self.GetFirstChild(prev)[0] and self.IsItemEnabled(
                        prev
                    ):
        ...
        # left arrow goes to the parent
            ...
            if self.IsExpanded(self._key_current) and self.HasChildren(
                self._key_current
            ):
        ...
        # <Home> selects the root item
            ...
            if not event.HasModifiers() and (
                (ord("0") <= keyCode <= ord("9"))
                or (ord("a") <= keyCode <= ord("z"))
                or (ord("A") <= keyCode <= ord("Z"))
            ):
    ...
    def HitTest(self, point, flags=0):
        ...
        hit, flags = self._anchor.HitTest(
            self.CalcUnscrolledPosition(point), self, flags, 0
        )
    ...
    def GetBoundingRect(self, item, textOnly=False):
        ...
        rect.SetX(i.GetX() - startX * _PIXELS_PER_UNIT)
        # rect.y = i.GetY() - startY * _PIXELS_PER_UNIT
        rect.SetY(i.GetY() - startY * _PIXELS_PER_UNIT)
        # rect.width = i.GetWidth()
        rect.SetWidth(i.GetWidth())
        # rect.height = self.GetLineHeight(i)
        rect.SetHeight(self.GetLineHeight(i))
    ...
    def OnMouse(self, event):
        ...
        if (
            underMouse
            and (flags & TREE_HITTEST_ONITEM)
            and not event.LeftIsDown()
            and not self._isDragging
            and (not self._editTimer or not self._editTimer.IsRunning())
        ):
        ...
        if (
            ...
            if hoverItem is not None:
                ...
                if (
                    ...
                    and (flags & TREE_HITTEST_ONITEMLABEL)
        ...
        if not (
            ...
            or event.LeftDClick()
            or event.Dragging()
            or ((event.Moving() or event.RightUp()) and self._isDragging)
        ):
        ...
        if (
            ...
            and not self._isDragging
            and (
                (flags & TREE_HITTEST_ONITEMICON) or (flags & TREE_HITTEST_ONITEMLABEL)
            )
        ):
            ...
            if event.RightDown():
                ...
                if self._lastOnSame:
                    ...
                    if (
                        item == self._current
                        and (flags & TREE_HITTEST_ONITEMLABEL)
                        and self.HasAGWFlag(TR_EDIT_LABELS)
                    ):
                ...
                if item.GetType() > 0 and (flags & TREE_HITTEST_ONITEMCHECKICON):
                    ...
                    if event.LeftDown():
                        if flags & TREE_HITTEST_ONITEM and self.HasAGWFlag(
    ...
    def CalculateSize(self, item, dc, level=-1, align=0):
        ...
        if level >= 0 and wnd:
            if align == 0:
                if level in self.absoluteWindows:
                    self.absoluteWindows[level] = max(
    ...
    def RefreshSubtree(self, item):
        ...
        rect = (
            wx.Rect()
        )  # TODO: trouver un moyen d'utiliser les méthodes SetX, SetY, SetWidth, SetHeight
        rect.SetX(self.CalcScrolledPosition(0, item.GetY()).GetScaleX())
        rect.SetY(self.CalcScrolledPosition(0, item.GetY()).GetScaleY())
        rect.SetWidth(client.x)
        rect.SetHeight(client.y)
    ...
    def RefreshLine(self, item):
        ...
        rect = (
            wx.Rect()
        )  # DID : trouver un moyen d'utiliser les méthodes SetX, SetY, SetWidth, SetHeight
        # x, rect.y = self.CalcScrolledPosition(0, item.GetY())
        # rect.width = self.GetClientSize().x
        # rect.height = self.GetLineHeight(item)
        rect.SetX(self.CalcScrolledPosition(0, item.GetY()).GetScaleX())
        rect.SetY(self.CalcScrolledPosition(0, item.GetY()).GetScaleY())
        rect.SetWidth(self.GetClientSize().x)
        rect.SetHeight(self.GetLineHeight(item))
    ...
    def GetMaxWidth(self, respect_expansion_state=True):
        ...
        maxwidth = rect.GetX() + rect.GetWidth() + 4
        # lastheight = rect.y + rect.height
        lastheight = rect.GetY() + rect.GetHeight()
        ...
        maxwidth, lastheight = self.RecurseOnChildren(
            root, maxwidth, respect_expansion_state
        )
    ...
    def RecurseOnChildren(self, item, maxwidth, respect_expansion_state):
        ...
        while child:
            ...
            maxwidth = max(maxwidth, rect.GetX() + rect.GetWidth() + 4)
            # lastheight = rect.y + rect.height
            lastheight = rect.GetY() + rect.GetHeight()

            if self.IsExpanded(child) or not respect_expansion_state:
                maxwidth, lastheight = self.RecurseOnChildren(
                    child, maxwidth, respect_expansion_state
                )
    ...
    def GetClassDefaultAttributes(self):
        """
        Renvoie la police et les couleurs par défaut utilisées par le contrôle. Ceci est
        utile si vous souhaitez utiliser la même police ou couleur dans votre propre contrôle que dans
        un contrôle standard - ce qui est une bien meilleure idée que de coder en dur des couleurs ou des polices spécifiques
        qui pourraient paraître complètement n'est pas à sa place sur le système de l'utilisateur,
        surtout s'il utilise des thèmes.

        Cette méthode statique est "remplacée" dans de nombreuses classes dérivées et ainsi appelante,
        par exemple, `wx.Button.GetClassDefaultAttributes()` renverra généralement les valeurs
        appropriées pour un bouton qui seront normalement différentes de celles
        renvoyées par, disons, `wx.ListCtrl.GetClassDefaultAttributes()`.

        :return : Une instance de `wx.VisualAttributes`.

        :note : La structure `wx.VisualAttributes` contient au moins les champs `font`,
         `colFg` et `colBg`. Tous peuvent être invalides s'il n'était pas possible de
         déterminer l'apparence du contrôle par défaut ou, en particulier pour la couleur d'arrière-plan,
         si le champ n'a pas de sens comme c'est le cas pour `colBg` pour les contrôles
         avec fond thématique.

        :note : Remplacé par `wx.PyControl`.
        """
        ...
        # attr = wx.VisualAttributes()
        # attr.__setattr__("font", wx.SystemSettings.GetFont(wx.SYS_DEFAULT_GUI_FONT))
        # attr.__setattr__("colFg", wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT))
        # attr.__setattr__("colBg", wx.SystemSettings.GetColour(wx.SYS_COLOUR_LISTBOX))
        return attr
