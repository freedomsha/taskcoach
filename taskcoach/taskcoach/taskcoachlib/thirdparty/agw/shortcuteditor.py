...
class HTMLHelpWindow(wx.Frame):
    ...
    def __init__(self, parent, htmlFile):
        ...
        self.SetSize((old_div(xvideo,2), old_div(yvideo,2)))
    ...
    def BuildToolBar(self, toolbar):
        ...
        w, h = _html_reload.get_bitmap().GetWidth(), _html_reload.get_bitmap().GetHeight()
        toolbar.SetToolBitmapSize((w, h))

        toolbar.add_label_tool(wx.ID_BACKWARD, _('Back'), _html_back.get_bitmap(), shortHelp=_('Back'),
                               longHelp=_('Go to the previous page'))

        toolbar.add_label_tool(wx.ID_FORWARD, _('Forward'), _html_forward.get_bitmap(), shortHelp=_('Forward'),
                               longHelp=_('Go to the next page'))

        toolbar.AddSeparator()

        toolbar.add_label_tool(wx.ID_HOME, _('Home'), _html_home.get_bitmap(), shortHelp=_('Home Page'),
                               longHelp=_('Go to the home page'))

        toolbar.add_label_tool(wx.ID_REFRESH, _('Refresh'), _html_reload.get_bitmap(), shortHelp=_('Refresh'),
                               longHelp=_('Refresh the current page'))

        toolbar.AddSeparator()
        toolbar.AddStretchableSpace()

        toolbar.add_label_tool(wx.ID_PRINT, _('Print'), _html_print.get_bitmap(), shortHelp=_('Print'),
...
class Shortcut(object):
    ...
    def GetById(self, id, item=None):
        ...
        while child:
            if child.menuItem and child.menuItem.get_id() == id:
                return child
    ...
    def ToMenuItem(self, menuBar):
        ...
        if isinstance(menuItem, wx.Menu):
            ...
            menuBar.SetLabel(menuItem.get_id(), label)
...
class ConflictDialog(GMD.GenericMessageDialog):
    ...
    def __init__(self, parent, conflict):
        ...
        self.SetOKBitmap(_reassign.get_bitmap())
...
class ShortcutEditor(wx.Dialog):
    ...
    def CreateWidgets(self):
        ...
        w, h, d, e = self.hiddenText.GetFullTextExtent('Ctrl+Shift+Alt+q+g+M', self.hiddenText.get_font())
        self.hiddenText.SetMinSize((w, h+d-e+1))

        defaultBmp = _default.get_bitmap()
        self.defaultButton = buttons.ThemedGenBitmapTextButton(self, wx.ID_RESET, defaultBmp,
                                                               _('Restore Defaults '), size=(-1, 29))
        
        self.infoBitmap = wx.StaticBitmap(self, -1, _info.get_bitmap())

        message = _('To edit a shortcut key, click on the corresponding row\n' \
                    'and type a new accelerator, or press backspace to clear.')

        italicFont = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        italicFont.SetStyle(wx.ITALIC)
        
        self.infoStatic = wx.StaticText(self, -1, message)
        self.infoStatic.set_font(italicFont)

        okBmp = _ok.get_bitmap()
        cancelBmp = _cancel.get_bitmap()
        helpBmp = _help.get_bitmap()
    ...
    def SetColumnWidths(self):
        ...
        dialogHeight = old_div(wx.SystemSettings.GetMetric(wx.SYS_SCREEN_Y),2)
    ...
    def OnSetFilter(self, event=None):
        ...
        filter = self.searchText.get_value()
