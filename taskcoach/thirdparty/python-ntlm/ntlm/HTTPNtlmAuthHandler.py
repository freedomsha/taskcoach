...
class AbstractNtlmAuthHandler:
    ...
    def retry_using_http_NTLM_auth(self, req, auth_header_field, realm, headers):
        ...
        auth = '%s %s' % (
            self.header, ntlm.create_NTLM_NEGOTIATE_MESSAGE(
                user, type1_flags
            ).decode('ascii')
        )
        ...
        auth = '%s %s' % (
            self.header, ntlm.create_NTLM_AUTHENTICATE_MESSAGE(
...
class ProxyNtlmAuthHandler(AbstractNtlmAuthHandler, urllib.request.BaseHandler):
    ...
    def http_error_407(self, req, fp, code, msg, headers): # pylint: disable=unused-argument
        """
