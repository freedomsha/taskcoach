...
class FilterableViewerForTasksMixin(FilterableViewerForCategorizablesMixin):
    ...
    def createFilterUICommands(self):
        return super(FilterableViewerForTasksMixin,
            ...
            [uicommand.ViewerHideTasks(taskStatus, viewer=self, settings=self.settings) for taskStatus in
             task.Task.possibleStatuses()] + \
    ...
    def __setBooleanSetting(self, setting, boolean_value):
        self.settings.setboolean(self.settingsSection(), setting, boolean_value)
...
class ManualOrderingMixin(object):
    ...
    def createSortByUICommands(self):
        return [uicommand.ViewerSortByCommand(viewer=self, value='ordering',
                                              ...
                                              helpText=self.sortByOrderingHelpText)] + super().createSortByUICommands()
...
class SortableViewerForTasksMixin(ManualOrderingMixin, SortableViewerForCategorizablesMixin):
    ...
    def isSortByTaskStatusFirst(self):
        return self.settings.getboolean(self.settingsSection(), 'sortbystatusfirst')
