...
class RibbonGallery(RibbonControl):
    ...
    def EnsureVisible(self, item):
        ...
        self.ScrollLines(old_div(delta,self._bitmap_padded_size.GetHeight()))
    ...
    def OnPaint(self, event):
        ...
        for item in self._items:
            ...
            dc.DrawBitmap(item.get_bitmap(), offset_pos.GetLeft() + padding_left, offset_pos.GetTop() + padding_top)
    ...
    def DoGetNextSmallerSize(self, direction, relative_to):
        ...
        client.x = (old_div(client.x, self._bitmap_padded_size.x)) * self._bitmap_padded_size.x
        client.y = (old_div(client.y, self._bitmap_padded_size.y)) * self._bitmap_padded_size.y
    ...
    def DoGetNextLargerSize(self, direction, relative_to):
        ...
        nitems = (old_div(client.GetWidth(),self._bitmap_padded_size.x))*(old_div(client.GetHeight(),self._bitmap_padded_size.y))
