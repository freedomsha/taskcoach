...
# try:
import lockfile  # No module named lockfile( a installer)
...
class TaskFile(patterns.Observer):
    ...
    def onDomainObjectAddedOrRemoved(self, event):  # pylint: disable=W0613 'event' is not used
    ...
    def onCategoryChanged_Deprecated(self, event):
        if self.__loading or self.__saving:
            return
        changed_categories = [changedCategory for changedCategory in event.sources() if
        ...
        if changed_categories:
            ...
            for changedCategory in changed_categories:
    ...
    def close(self):
        if os.path.exists(self.filename()):
            ...
            # xml.ChangesXMLWriter(open(self.filename() + '.delta', 'wb')).write(changes)
    ...
    def _openForRead(self):
        ...
        # return open(self.__filename, 'rU')
    ...
    def load(self, filename=None):
        ...
        try:
            ...
            if os.path.exists(self.filename()):
                ...
                # xml.ChangesXMLWriter(open(self.filename() + '.delta', 'wb')).write(self.__changes)
        except:  # else ?
            self.setFilename('')
            raise
        finally:
