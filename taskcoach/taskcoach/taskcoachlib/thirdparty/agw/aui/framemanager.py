...
class AuiDockingGuideWindow(wx.Window):
    ...
    def DrawIcon(self, dc):
        ...
        if self._direction == wx.TOP:
            rect.height -= old_div(rect.height, 2)
            point = rect.GetBottomLeft()
            length = rect.width

        elif self._direction == wx.LEFT:
            rect.width -= old_div(rect.width, 2)
    ...
    def DrawArrow(self, dc):
        ...
        point.x = old_div((rect.GetLeft() + rect.GetRight()), 2)
        point.y = old_div((rect.GetTop() + rect.GetBottom()), 2)
...
class AuiCenterDockingGuide(AuiDockingGuide):
    ...
    def CreateShapesWithStyle(self):
        ...
        if not self._useAero:
            ...
            self._aeroBmp = aero_dock_pane.get_bitmap()
            region = wx.RegionFromBitmap(self._aeroBmp)

            self._allAeroBmps = [aero_dock_pane_left.get_bitmap(), aero_dock_pane_top.get_bitmap(),
                                 aero_dock_pane_right.get_bitmap(), aero_dock_pane_bottom.get_bitmap(),
                                 aero_dock_pane_center.get_bitmap(), aero_dock_pane.get_bitmap()]
            self._deniedBitmap = aero_denied.get_bitmap()
            self._aeroRects = [rectLeft, rectTop, rectRight, rectBottom, rectCenter]
            self._valid = True

        elif useWhidbey:

            self._aeroBmp = whidbey_dock_pane.get_bitmap()
            region = wx.RegionFromBitmap(self._aeroBmp)

            self._allAeroBmps = [whidbey_dock_pane_left.get_bitmap(), whidbey_dock_pane_top.get_bitmap(),
                                 whidbey_dock_pane_right.get_bitmap(), whidbey_dock_pane_bottom.get_bitmap(),
                                 whidbey_dock_pane_center.get_bitmap(), whidbey_dock_pane.get_bitmap()]
            self._deniedBitmap = whidbey_denied.get_bitmap()
            self._aeroRects = [rectLeft, rectTop, rectRight, rectBottom, rectCenter]
            self._valid = True
    ...
    def AeroMove(self, pos):
        ...
        thePos = pos + wx.Point(old_div((size.x-sizeY),2), old_div((size.y-sizeX),2))
    ...
    def OnPaint(self, event):
        ...
        if self._useAero:
            ...
            if not self._valid:
                ...
                xPos, yPos = (rect.x + old_div((rect.width),2) - old_div(bmpX,2)), (rect.y + old_div((rect.height),2) - old_div(bmpY,2))
...
class AuiFloatingFrame(wx.MiniFrame):
    ...
    def SetPaneWindow(self, pane):
        ...
        self._fly_step = old_div(abs(pane.floating_size.y - \
    ...
    **Layer** - A layer is akin to an onion. Layer 0 is the very center of the managed pane.
    ...
    def AddPane1(self, window, pane_info):
        ...
        if pinfo.name == "" or already_exists:
            pinfo.name = ("%s%08x%08x%08x")%(pinfo.window.get_name(), time.time(),
...
#                    self.SwitchToolBarOrientation(p)
    ...
    def UpdateDockingGuides(self, paneInfo):
        ...
        for indx, guide in enumerate(self._guides):
            ...
            if direction == AUI_DOCK_LEFT:
                pt.x = frameRect.x + old_div(guide_size.x, 2) + 16
                pt.y = frameRect.y + old_div(frameRect.height, 2)

            elif direction == AUI_DOCK_TOP:
                pt.x = frameRect.x + old_div(frameRect.width, 2)
                pt.y = frameRect.y + old_div(guide_size.y, 2) + 16

            elif direction == AUI_DOCK_RIGHT:
                pt.x = frameRect.x + frameRect.width - old_div(guide_size.x, 2) - 16
                pt.y = frameRect.y + old_div(frameRect.height, 2)

            elif direction == AUI_DOCK_BOTTOM:
                pt.x = frameRect.x + old_div(frameRect.width, 2)
                pt.y = frameRect.y + frameRect.height - old_div(guide_size.y, 2) - 16

            elif direction == AUI_DOCK_CENTER:
                rc = paneInfo.window.GetScreenRect()
                pt.x = rc.x + old_div(rc.width, 2)
                pt.y = rc.y + old_div(rc.height, 2)
                if paneInfo.HasCaption():
                    pt.y -= old_div(captionSize, 2)
                elif paneInfo.HasCaptionLeft():
                    pt.x -= old_div(captionSize, 2)

            # guide will be centered around point 'pt'
            targetPosition = wx.Point(pt.x - old_div(guide_size.x, 2), pt.y - old_div(guide_size.y, 2))
    ...
    def DoFrameLayout(self):
        ...
        for part in self._uiparts:            
            ...
            part.rect = part.sizer_item.get_rect()
...
##                    totalPixsize -= tmpPane.min_size.y
    ...
    def ShowHint(self, rect):
        ...
        if self._agwFlags & AUI_MGR_RECTANGLE_HINT and wx.Platform != "__WXMAC__":
            ...
            for pane in self._panes:            
                if pane.IsFloating() and pane.frame.IsShown():
                    ...
                    rect2 = wx.Rect(*pane.frame.get_rect())
                    if wx.Platform == "__WXGTK__":
            ...
            clip.IntersectRect(self._frame.get_rect())
        ...
        if self._hint_window:
            self._hint_window.set_rect(rect)
    ...
    def OnGripperClicked(self, pane_window, start, offset):
        ...
        if paneInfo.IsDocked():
            ...
            if paneInfo.frame:
                ...
                windowPt = paneInfo.frame.get_rect().GetTopLeft()
    ...
    def OnCaptionDoubleClicked(self, pane_window):
        ...
        if paneInfo.IsFloating():
            if paneInfo.name.startswith("__floating__"):
                ...
                win_rect = paneInfo.frame.get_rect()
                paneInfo.Dock()
                if paneInfo.IsToolbar():
    ...
    def RestrictResize(self, clientPt, screenPt, createDC):
        ...
        if self._action_part.type == AuiDockUIPart.typeDockSizer:
            ...
            while (oldPixsize > 0 and totalPixsize > 10 and \
                  old_div(oldPixsize*totalProportion,totalPixsize) < pane.dock_proportion):
            
                totalPixsize -= 1

            # calculate the new proportion of the pane
            
            newProportion = old_div(newPixsize*totalProportion,totalPixsize)
...
##            self._frame.Thaw()
    ...
    def OnMotion_ClickCaption(self, event):
        ...
        if self._action_pane.IsToolbar():
            ...
            windowPt = self._action_pane.frame.get_rect().GetTopLeft()
            ...
            if self._action_offset.x > old_div(frame_size.x * 2, 3):
                self._action_offset.x = old_div(frame_size.x, 2)
            if self._action_offset.y > old_div(frame_size.y * 2, 3):
                self._action_offset.y = old_div(frame_size.y, 2)
    ...
    def AnimateDocking(self, win_rect, pane_rect):
        ...
        wstep = int(old_div(abs(win_rect.width - pane_rect.width),step))
        hstep = int(old_div(abs(win_rect.height - pane_rect.height),step))
        xstep = old_div(int(win_rect.x - pane_rect.x),step)
        ystep = old_div(int(win_rect.y - pane_rect.y),step)
    ...
    def SmoothDock(self, paneInfo):
        ...
        step = old_div(self.GetAnimationStep(),3)

        wstep = int(old_div((win_rect.width - hint_rect.width),step))
        hstep = int(old_div((win_rect.height - hint_rect.height),step))
        xstep = old_div(int((win_rect.x - hint_rect.x)),step)
        ystep = old_div(int((win_rect.y - hint_rect.y)),step)

        for i in range(int(step)):
            width, height = win_rect.width - i*wstep, win_rect.height - i*hstep
            x, y = xstart - i*xstep, ystart - i*ystep
            new_rect = wx.Rect(x, y, width, height)
            paneInfo.frame.set_rect(new_rect)
            wx.MilliSleep(10)            
    ...
    def GetSnapPosition(self):
        ...
        if hAlign == wx.LEFT:
            ...
            pos.x = area.x + old_div((area.width - size.x),2)
        else:
        ...
        if vAlign == wx.TOP:
            ...
            pos.y = area.y + old_div((area.height - size.y),2)
        else:
