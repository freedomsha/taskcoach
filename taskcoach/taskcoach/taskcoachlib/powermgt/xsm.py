...
class ICELoop(threading.Thread):
    ...
    def __init__(self):
        ...
        self.watchProc = IceWatchProc(self._on_watch)
    ...
    def _on_watch(self, conn, client_data, opening, watchdata):
...
class SessionMonitor(ICELoop):
    ...
    this and overload the save_yourself, die, save_complete and
    shutdown_cancelled methods to do actual work.
    """

    def __init__(self):
        super().__init__()

        self.callbacks = SmcCallbacks(SmcSaveYourselfCallback(SmcSaveYourselfProc(self._save_yourself), None),
                                      SmcDieCallback(SmcDieProc(self._die), None),
                                      SmcSaveCompleteCallback(SmcSaveCompleteProc(self._save_complete), None),
                                      SmcShutdownCancelledCallback(SmcShutdownCancelledProc(self._shutdown_cancelled),
                                                                   None))
        ...
        try:
            ...
            IceSetIOErrorHandler(IceIOErrorHandler(self._on_ice_error))

    def is_valid(self):
        return self.conn is not None and self.connection is not None

    def _on_ice_error(self, conn):
        if self.is_valid():
    ...
    def _save_yourself(self, conn, client_data, save_type, shutdown, interact_style, fast):
        if self.is_valid():
    ...
    def _save_complete(self, conn, client_data):
        self.save_complete()
    ...
    def _shutdown_cancelled(self, conn, client_data):
    ...
    @property
    def version(self):
        return SmcProtocolVersion(self.conn) if self.is_valid() else None

    @property
    def revision(self):
        return SmcProtocolRevision(self.conn) if self.is_valid() else None

    @property
    def vendor(self):
        return SmcVendor(self.conn) if self.is_valid() else None  # Leak

    @property
    def release(self):
        return SmcRelease(self.conn) if self.is_valid() else None  # Leak
