...
_TOC = sequence(
    ...
    ul(
        ...
        li(
            ...
            ul(
                li(a_href(_('{name} on the iPhone'.format(**meta.metaDict)), 'taskcoachiphone')),
                li(a_href(_('Configuration'), 'iphoneconf')),
                li(a_href(_('Troubleshooting'), 'iphonetrouble')))),
        li(
            a_href(_('Android'), 'android'),
            ul(
                li(a_href(_('{name} on Android?'.format(**meta.metaDict)), 'taskcoachandroid')),
...
<a href="#categoryproperties">category properties</a> below.''')),
    ...
    p(
        _('''You can set a reminder for a specific Date and time. {name} will
...
completed. It is also possible to snooze the reminder.'''.format(**meta.metaDict))),
    p(
        _('''If you have Growl or Snarl installed, you can instruct
{name} to use these for reminders in the preferences dialog.'''.format(**meta.metaDict))))
...
attributes are 'cc' and 'to'. Examples:'''.format(**meta.metaDict))),
...
_multiuserSection = sequence(
    ...
    p(
        _('''A task file may be opened by several instances of {name}, either
...
resolved, usually by you winning the conflict. This serves two use cases:'''.format(**meta.metaDict))),
    ul(
...
for a more secure alternative). Changes to the task file are correctly detected by {name}
when it's updated.'''.format(**meta.metaDict))))
...
_emailSection = sequence(
    ...
    p(
        _('''{name} integrates with several mail user
agents, through drag and drop. This has some limitations; e-mails are
copied in a directory next to the {name} file, as .eml files and are
...
created it first.'''.format(**meta.metaDict))),
    p(
...
etc.''')),
    p(
        _('''{name} has built-in SyncML client support on Windows and Mac OS X
(provided that you use the supplied binaries). This means you can
setup {name} to synchronize with the same SyncML server you
synchronize Outlook with and have all Outlook tasks and notes in
your {name} file, as well as %(name)s tasks and notes in Outlook. Or
your Pocket PC.'''.format(**meta.metaDict))),
    p(
...
type being vcalendar, some {name} features cannot be presented to the
server.'''.format(**meta.metaDict)),
        ul(
...
<a target="_blank" href="{}"> Microsoft download site</a>.'''.format(_MSURL))),
    h4(
...
_iPhoneSection = sequence(
    ...
    h4(
        a_name(_('{name} on the iPhone/iPod Touch/iPad'.format(**meta.metaDict)), 'taskcoachiphone')),
    p(
        _('''There is an iPhone/iPod Touch/iPad companion app for {name},
available on <a
...
It supports the following features:'''.format(**meta.metaDict)),
        ul(
...
recurrence)...''')),
            ...
            li(_('Two-way synchronization with {name} on the desktop'.format(**meta.metaDict)))),
        p(
...
considered "soon".''')))),
        ...
        p(
            _('''Before synchronizing, you must also configure {name} on the
desktop; in the preferences, in the "Features" tab, check "Enable iPhone
synchronization". Restart {name}. Now, in the preferences, choose the
"iPhone" tab and fill in at least the password.'''.format(**meta.metaDict))),
        p(
            _('''When you tap the "Sync" button in the category view, {name}
will automatically detect running instances of {name} on your
...
change the "Bonjour service name" in the configuration.'''.format(**meta.metaDict))),
        p(
            _('''{name} will remember the chosen instance and try it next time
you synchronize; if it's not running, it will prompt you again.'''.format(**meta.metaDict))),
        p(
...
_androidSection = sequence(
    ...
    h4(
        a_name(_('{name} on Android?'.format(**meta.metaDict)), 'taskcoachandroid')),
    p(
        _('''No, {name} is not available for the Android platform. But,
...
format</a> and then edit your tasks on your Android device.'''.format(**meta.metaDict))),
    h4(
...
Dropbox folder. {name} can import from and export to that todo.txt
file.'''.format(**meta.metaDict))),
    p(
        _('''When exporting to Todo.txt, {name} creates another file alongside
the target one, suffixed with "-meta". Don't delete this file; {name} uses it
...
the Todo.txt file wins the conflict.'''.format(**meta.metaDict))),
    p(
...
exporting of Todo.txt, {name} will keep the task file and the
...
or delete it.'''.format(**meta.metaDict))),
    h4(
        a_name(_('Importing todo.txt'), 'todotxtimporting')),
    p(
        _('''{name} imports task subjects, planned start Date, due Date, completion
Date, priority, contexts and projects. Contexts and projects are both
transformed into categories in {name}. Projects cannot be transformed into
parent tasks because Todo.txt allows tasks to belong to multiple projects,
while %(name)s only allows one parent task per task.'''.format(**meta.metaDict))),
    p(
        _('''When importing, {name} tries to find matching tasks and
...
item if any.'''.format(**meta.metaDict))),
    h4(
        a_name(_('Exporting todo.txt'), 'todotxtexporting')),
    p(
        _('''{name} exports task subjects, planned start Date, due Date, completion
Date, priority and categories to the Todo.txt format. Other attributes are not
exported.'''.format(**meta.metaDict))),
    p(
        _('''{name} supports dates and times, but Todo.txt only supports
...
completion dates, but no due dates. Therefore, {name} uses an extension to
export the due Date of tasks. Due dates are shown as "due:YYYY-MM-DD" in the
todo.txt file.'''.format(**meta.metaDict))),
    p(
        _('''Todo.txt has priorities in the form of a letter ('A'-'Z').
{name} has numerical priorities. {name} priority 1 is transformed into 'A',
2 becomes 'B', etc., and 26 is 'Z'. %(name)s priorities below 1 and above 26
are not exported.'''.format(**meta.metaDict))),
    p(
...
Subcategories are exported if their parent category is.'''.format(**meta.metaDict))))
...
will become red if {name} cannot figure out what it means. Example
dates:'''.format(**meta.metaDict))),
    ul(
        li('3 pm tomorrow'),
        li('next saturday'),
        li('noon')
...
_shortcutSection = sequence(
    ...
    p(
        _('''{name} has several keyboard shortcuts, listed below. Keyboard
shortcuts are not configurable at the moment.'''.format(**meta.metaDict))),
    p(
        table(
...
helpHTML = ' '.join([_TOC, _taskSection, _effortSection, _categorySection,
                    _noteSection, _printingAndExportingSection, _multiuserSection, _emailTasksSection, _emailSection,
                    _syncmlSection, _iPhoneSection, _androidSection, _templatesSection,
                    _guiSection, _shortcutSection])

aboutHTML = _('''<h4>{name} - {description}</h4>
<h5>Version {version}, {date}</h5>
<p>By {author} &lt;{author_email}&gt;<p>
<p><a href="{urlpo}" target="_blank">{urlpo}</a></p>
<p>{copyright}</p>
<p>{license_notice_html}</p>
'''.format(**meta.metaDict))
