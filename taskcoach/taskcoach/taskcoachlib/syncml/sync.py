...
class TaskCoachDMTree(DMTree):
    def __init__(self, sync_ml_config, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.__syncMLConfig = sync_ml_config

    def isLeaf(self, node):
        return TaskCoachManagementNode(self.__syncMLConfig, node).getMaxChildrenCount() == 0

    def readManagementNode(self, node_name):
        node = TaskCoachManagementNode(self.__syncMLConfig, node_name)

        for name in node.getChildrenNames():
            node.addChild(TaskCoachManagementNode(self.__syncMLConfig, node_name, name))
...
class Synchronizer(wx.ProgressDialog):
    def __init__(self, reportCallback, taskFile, password):
        super().__init__(_('Synchronization'),
        ...
        self.taskdbname = cfg[self.clientName]['spds']['sources']['%s.Tasks' %
                                                                  self.clientName].get('uri', ).encode('UTF-8')
        self.notedbname = cfg[self.clientName]['spds']['sources']['%s.Notes' %
                                                                  self.clientName].get('uri', ).encode('UTF-8')
