...
class XMLReader(object):  # nouvelle classe
    ...
    def read(self):
        ...
        if os.path.exists(changesName):
            # file -> open ?
            changes = ChangesXMLReader(open(self.__fd.name + '.delta', 'rU')).read()
        else:
            changes = dict()
        ...
        for categoryId, categorizableIds in list(self.__categorizables.items()):
            ...
            for categorizableId in categorizableIds:
                if categorizableId in categorizableMap:
                    the_categorizable = categorizableMap[categorizableId]
                    theCategory.addCategorizable(the_categorizable)
                    the_categorizable.addCategory(theCategory, event=event)
        event.send()
    ...
    def __parse_category_node(self, category_node):
        ...
        filtered = self.__parse_boolean(category_node.attrib.get('filtered', 'False'))
        exclusive = self.__parse_boolean(category_node.attrib.get('exclusiveSubcategories', 'False'))
    ...
    def _parse_task_node(self, task_node):
        ...
        self.__prerequisites[kwargs['id']] = [id_ for id_ in task_node.attrib.get('prerequisites', '').split(' ') if
                                              id_]
        if self.__tskversion > 20:
    ...
    def __parse_recurrence_node(self, task_node):
        ...
        if node is not None:
            kwargs = dict(unit=node.attrib.get('unit', ''),
                          ...
                          sameWeekday=self.__parse_boolean(node.attrib.get('sameWeekday', 'False')),
    ...
    def __parse_base_attributes(self, node):
        ...
        attributes = dict(id=node.attrib.get('id', ''),
                          creationDateTime=self.__parse_datetime(node.attrib.get('creationDateTime', '1-1-1 0:0')),
