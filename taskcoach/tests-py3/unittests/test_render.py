...
class RenderTimeSpentTest(tctest.TestCase):
    ...
    def testTenHours(self):
        self.assertEqual("10:00:00", render.timeSpent(date.TimeDelta(hours=10)))
    ...
    def testDecimalNegative(self):
        self.assertEqual(
            "-1.25",
            render.timeSpent(date.TimeDelta(hours=-1, minutes=-15), decimal=True),
        )
...
class RenderRecurrenceTest(tctest.TestCase):
    ...
    def testEveryOtherDay(self):
        self.assertEqual(
            _("Every other day"), render.recurrence(date.Recurrence("daily", amount=2))
        )
    ...
    def testThreeDaily(self):
        self.assertEqual(
            "Every 3 days", render.recurrence(date.Recurrence("daily", amount=3))
        )

    def testThreeWeekly(self):
        self.assertEqual(
            "Every 3 weeks", render.recurrence(date.Recurrence("weekly", amount=3))
        )
