...
class ProgressGauge(wx.PyWindow):
    ...
    def OnPaint(self, event):
        ...
        status = old_div(self._current,(self._steps - old_div(int(self._gaugeproportion*xsize),int(interval))))
...
class PyProgress(wx.Dialog):
    ...
    def __init__(self, parent=None, id=-1, title="", message="",
        ...
        sizeDlg.x = max(widthText, old_div(4*sizeDlg.y,3))
        sizeDlg.x *= 3
        sizeDlg.x /= 2
    ...
    def SetTimeLabel(self, val, label=None):
        ...
        if label:
            ...
            hours = old_div(val,3600)
            minutes = old_div((val%3600),60)
            seconds = val%60
