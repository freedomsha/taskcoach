...
class EffortViewer(base.ListViewer,
    ...
    def createFilter(self, taskList):
        ...
        searchFilter = filter.SearchFilter(self.create_aggregator(categoryFilter,
                                                                   aggregation))
        return searchFilter

    # @staticmethod
    def create_aggregator(self, taskList, aggregation):
    ...
    def newItemDialog(self, *args, **kwargs):
        selectedTasks = kwargs.get('selected_tasks', [])
        ...
        return super().newItemDialog(selectedTasks,
                                     bitmap=bitmap)
    ...
    def getItemTooltipData(self, item):
        ...
        if isinstance(item, effort.CompositeEffort) and len(item):
            ...
            for theEffort in item:
                details.append(u'%s (%s)' % (
                    render.dateTimePeriod(theEffort.getStart(), theEffort.getStop(), humanReadable=True),
...
class EffortViewerForSelectedTasks(EffortViewer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__currentTaskViewer = self.__viewerContainer.activeViewer() \
            if self.__viewerContainer.activeViewer().isShowingTasks() else None
