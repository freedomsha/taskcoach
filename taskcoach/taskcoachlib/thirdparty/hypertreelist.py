...
class TreeListHeaderWindow(wx.Window):
    ...
    def __init__(
        ...
        size=wx.DefaultSize,
        style=0,
        name="wxtreelistctrlcolumntitles",
    ):
        """
        Default class constructor.
    ...
    def OnPaint(self, event):
        ...
        for i in xrange(numColumns):
            ...
            if image != -1 and imageList:
                params.m_labelBitmap = imageList.GetBitmap(image)  # Property 'm_labelColour' cannot be set

            if self._headerCustomRenderer is not None:
                self._headerCustomRenderer.DrawHeaderButton(dc, rect, flags, params)
            else:
                wx.RendererNative.Get().DrawHeaderButton(
                    self, dc, rect, flags, wx.HDR_SORT_ICON_NONE, params
                )
    ...
    def OnMouse(self, event):
        ...
        if self._isDragging:
            ...
            if event.ButtonUp():
                ...
                self.Refresh()
                self.SendListEvent(
                    wx.wxEVT_COMMAND_LIST_COL_END_DRAG, event.GetPosition()
                )
            else:
            ...
            if event.LeftDown() or event.RightUp():
                if hit_border and event.LeftDown():
                    ...
                    self.DrawCurrent()
                    self.SendListEvent(
                        wx.wxEVT_COMMAND_LIST_COL_BEGIN_DRAG, event.GetPosition()
                    )
                else:  # click on a column
                    evt = (
                        event.LeftDown()
                        and [wx.wxEVT_COMMAND_LIST_COL_CLICK]
                ...
                self.SetColumnWidth(
                    self._column, self._owner.GetBestColumnWidth(self._column)
                )
                self.Refresh()
...
class TreeListItem(GenericTreeItem):
    ...
    def __init__(
        ...
        GenericTreeItem.__init__(
            self, parent, text, ct_type, wnd, image, selImage, data
        )
        ...
        if not theCtrl.HasAGWFlag(wx.TR_HIDE_ROOT) or level > 0:
            ...
            if point.y >= self._y and point.y <= self._y + h:
                ...
                if self.HasPlus() and theCtrl.HasButtons():
                    ...
                    if (
                        (point.x >= bntX)
                        and (point.x <= (bntX + theCtrl._btnWidth))
                        and (point.y >= bntY)
                        and (point.y <= (bntY + theCtrl._btnHeight))
                    ):
                ...
                if self.GetType() != 0:
                    ...
                    chkX = (
                        self._text_x
                        - imageWidth
                        - numberOfMargins * _MARGIN
                        - theCtrl._checkWidth
                    )
                    chkY = y_mid - theCtrl._checkHeight2
                    if (
                        (point.x >= chkX)
                        and (point.x <= (chkX + theCtrl._checkWidth))
                        and (point.y >= chkY)
                        and (point.y <= (chkY + theCtrl._checkHeight))
                    ):
                ...
                if self.GetCurrentImage() != _NO_IMAGE:
                    ...
                    if (
                        (point.x >= imgX)
                        and (point.x <= (imgX + theCtrl._imgWidth))
                        and (point.y >= imgY)
                        and (point.y <= (imgY + theCtrl._imgHeight))
                    ):
                ...
                # check for label hit
                if (point.x >= self._text_x) and (
                    point.x <= (self._text_x + self._width)
                ):
                ...
                for j in xrange(theCtrl.GetColumnCount()):
                    ...
                    if (j != maincol) and (point.x >= x and point.x < x + w):
                        ...
                        flags |= (
                            wx.TreeCtrl.HitTest().TREE_HITTEST_ONITEMCOLUMN
...
class EditTextCtrl(EditCtrl, wx.TextCtrl):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(EditTextCtrl, self).__init__(
            parent,
            id,
            item,
            column,
...
class TreeListMainWindow(CustomTreeCtrl):
    ...
    def __init__(
        ...
        CustomTreeCtrl.__init__(
            self, parent, id, pos, size, style, agwStyle, validator, name
        )
    ...
    def AddRoot(self, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None):
        ...
        if text.find("\n") >= 0 and not (
            self._agwStyle & wx.TR_HAS_VARIABLE_ROW_HEIGHT
        ):
            raise Exception(
        ...
        arr = [""] * self.GetColumnCount()
        arr[self._main_column] = text
        self._anchor = TreeListItem(
            self, None, arr, ct_type, wnd, image, selImage, data
        )
    ...
    def ScrollTo(self, item):
        ...
        if item._y < start_y + 3:
            ...
            self.SetScrollbars(
                xUnit,
                yUnit,
                (xUnit and [x / xUnit] or [0])[0],
                (yUnit and [y / yUnit] or [0])[0],
                x_pos,
                (yUnit and [item._y / yUnit] or [0])[0],
            )
            ...
            self.SetScrollbars(
                ...
                (
                    yUnit
                    and [(item._y + self.GetLineHeight(item) - client_h) / yUnit]
                    or [0]
                )[0],
            )
    ...
    def PaintItem(self, item, dc):
        ...
        if item.IsSelected():
            if wx.Platform == "__WXMAC__" and self._hasFocus:
                colTextHilight = wx.SystemSettings.GetColour(
        ...
        text_w, text_h, dummy = dc.GetMultiLineTextExtent(
            item.GetText(self.GetMainColumn())
        )
        ...
        if self.HasAGWFlag(wx.TR_FULL_ROW_HIGHLIGHT):
            ...
            if item == self._dragItem:
                ...
                if item.PartialHilight():
                    itemrect = wx.Rect(
                        0, item.GetY() + off_h, total_w - 1, total_h - off_h
                    )
                    dc.SetBrush(wx.Brush(colBg, wx.SOLID))
        ...
        img_extraH = (
            total_h > self._imgHeight and [(total_h - self._imgHeight) / 2] or [0]
        )[0]
        x_colstart = 0
        ...
        for i in xrange(self.GetColumnCount()):
            ...
            dc.SetClippingRegion(
                x_colstart, item.GetY(), col_w, total_h
            )  # only within column
            ...
            if not self.HasAGWFlag(wx.TR_FULL_ROW_HIGHLIGHT):
                ...
                dc.SetPen(
                    (self._hasFocus and [self._borderPen] or [wx.TRANSPARENT_PEN])[0]
                )
                if i == self.GetMainColumn():
                    if item == self._dragItem:
                        if (
                        ...
                        if self._usegradients:
                            if self._gradientstyle == 0:  # Horizontal
                                self.DrawHorizontalGradient(
                                    dc, itemrect, self._hasFocus
                                )
                            else:  # Vertical
                                self.DrawVerticalGradient(dc, itemrect, self._hasFocus)
                        elif self._vistaselection:
                        ...
                        dc.SetPen(
                            (self._hasFocus and [wx.BLACK_PEN] or [wx.TRANSPARENT_PEN])[
                                0
                            ]
                        )
            ...
            if self.HasAGWFlag(wx.TR_COLUMN_LINES):  # vertical lines between columns
                pen = wx.Pen(
                    wx.SystemSettings.GetColour(wx.SYS_COLOUR_3DLIGHT), 1, wx.SOLID
                )
                dc.SetPen(
                    (
    ...
    def PaintLevel(self, item, dc, level, y, x_maincol):
        ...
        if self.IsExposed(exposed_x, exposed_y, _MAX_WIDTH, h + draw_row_lines):
            ...
            clip_width = (
                self._owner.GetHeaderWindow().GetColumn(self._main_column).GetWidth()
            )
            ...
            if item.HasPlus() and self.HasButtons():  # should the item show a button?
                ...
                if self._imageListButtons:
                    ...
                    rect = wx.Rect(
                        x - self._btnWidth2,
                        y_mid - self._btnHeight2,
                        self._btnWidth,
                        self._btnHeight,
                    )
                    flag = (item.IsExpanded() and [wx.CONTROL_EXPANDED] or [0])[0]
                    wx.RendererNative.GetDefault().DrawTreeItemButton(
                        self, dc, rect, flag
                    )
    ...
    def HitTest(self, point, flags=0):
        ...
        hit, flags, column = self._anchor.HitTest(
            self.CalcUnscrolledPosition(point), self, flags, column, 0
        )
        if not hit:
            flags = wx.TREE_HITTEST_NOWHERE
            column = -1
    ...
    def EditLabel(self, item, column=None):
        ...
        if self._editCtrl is not None and (
            item != self._editCtrl.item() or column != self._editCtrl.column()
        ):
            self._editCtrl.StopEditing()
    ...
    def OnMouse(self, event):
        ...
        item, flags, column = self._anchor.HitTest(
            self.CalcUnscrolledPosition(p), self, flags, self._curColumn, 0
        )
        ...
        if (
            underMouse
            and (flags & wx.TREE_HITTEST_ONITEM)
        ...
        if event.Dragging():
            ...
            command = (
                event.LeftIsDown()
                and [wx.wxEVT_COMMAND_TREE_BEGIN_DRAG]
                or [wx.wxEVT_COMMAND_TREE_BEGIN_RDRAG]
            )[0]
        ...
        if (
            item is None or not self.IsItemEnabled(item)
        ) and not event.GetWheelRotation():
            self._owner.GetEventHandler().ProcessEvent(event)
            return
        ...
        if event.RightUp():
            ...
            self.SetFocus()
            nevent = TreeEvent(
                wx.wxEVT_COMMAND_TREE_ITEM_RIGHT_CLICK, self._owner.GetId()
            )
            nevent.SetEventObject(self._owner)
            ...
            if self._lastOnSame:
                if (
                    item == self._current
                    and self._curColumn != -1
                    and self._owner.GetHeaderWindow().IsColumnEditable(self._curColumn)
                    and flags
                    & (wx.TREE_HITTEST_ONITEMLABEL | wx.TREE_HITTEST_ONITEMCOLUMN)
                ):
            ...
            if (
                (
                    (flags & wx.TREE_HITTEST_ONITEMBUTTON)
                    or (flags & wx.TREE_HITTEST_ONITEMICON)
                )
                and self.HasButtons()
                and item.HasPlus()
            ):
            ...
            if not self._left_down_selection:
                unselect_others = not (
                    (event.ShiftDown() or event.CmdDown())
                    and self.HasAGWFlag(wx.TR_MULTIPLE)
                )
                self.DoSelectItem(item, unselect_others, event.ShiftDown())
                self.EnsureVisible(item)
                self._current = self._key_current = (
                    item  # make the new item the current item
                )
            else:
                self._left_down_selection = False
            ...
            if event.LeftDClick():
                ...
                nevent = TreeEvent(
                    wx.wxEVT_COMMAND_TREE_ITEM_ACTIVATED, self._owner.GetId()
                )
                nevent.SetEventObject(self._owner)
                nevent.SetItem(item)  # the item clicked
                nevent.SetInt(self._curColumn)  # the column clicked
    ...
    def CalculateSize(self, item, dc):
        ...
        text_w, dummy, dummy = dc.GetMultiLineTextExtent(
            item.GetText(self._main_column)
        )
        text_h += 2
    ...
    def CalculateLevel(self, item, dc, level, y, x_colstart):
        ...
        if self.HasAGWFlag(wx.TR_HIDE_ROOT) and (level == 0):
            ...
            for n in xrange(count):
                y = self.CalculateLevel(
                    children[n], dc, level, y, x_colstart
                )  # recurse
...
class HyperTreeList(wx.PyControl):
    ...
    def __init__(
        ...
        self._main_win = TreeListMainWindow(
            self, -1, wx.Point(0, 0), size, main_style, agwStyle, validator
        )
        self._main_win._buffered = False

        self._header_win = TreeListHeaderWindow(
            self, -1, self._main_win, wx.Point(0, 0), wx.DefaultSize, wx.TAB_TRAVERSAL
        )
    ...
    def CalculateAndSetHeaderHeight(self):
        ...
        if self._header_win:
            h = max(
                wx.RendererNative.Get().GetHeaderButtonHeight(self._header_win),
                self._minHeaderHeight,
            )
    ...
    def SetColumnWidth(self, column, width):
        ...
        if width == wx.LIST_AUTOSIZE_USEHEADER:
            ...
            width, dummy, dummy = dc.GetMultiLineTextExtent(
                self._header_win.GetColumnText(column)
            )
    ...
    def InsertColumn(
        ...
        self._header_win.InsertColumn(
            before, text, width, flag, image, shown, colour, edit
        )
        self._header_win.Refresh()
    ...
    def GetTextCtrlAlignmentStyle(self, column):
        ...
        return {
            wx.ALIGN_LEFT: wx.TE_LEFT,
            wx.ALIGN_RIGHT: wx.TE_RIGHT,
            wx.ALIGN_CENTER: wx.TE_CENTER,
        }[alignment]
