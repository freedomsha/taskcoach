...
class NewSubTaskCommand(base.NewSubItemCommand, SaveTaskStateMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        planned_start_date_time = kwargs.pop('plannedStartDateTime', date.DateTime())
...
class MarkActiveCommand(base.SaveStateMixin, base.BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        items_to_save = set([relative for item in self.items for relative in item.family()])
        self.saveStates(items_to_save)
...
class StopEffortCommand(EffortCommand):
    ...
    def tasksToStopTracking(self):
        try:
            stoppable = lambda effort: effort.is_being_tracked() and not effort.is_total()  # method lambda deprecied
        finally:
            def stoppable(effort):
                return effort.is_being_tracked() and not effort.is_total()
