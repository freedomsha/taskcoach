...
def GetdragcursorImage():
    ...
    stream = io.StringIO(GetdragcursorData())
        ...
        if self._style & IL_FIXED_SIZE:
            ...
            if self._width > 0 and bitmap.GetWidth() > self._width and \
               bitmap.GetHeight() >= self._height:
                ...
                numImages = old_div(bitmap.GetWidth(),self._width)
...
class SelectionStore(object):
    ...
    def SelectRange(self, itemFrom, itemTo, select=True):
        ...
        if itemTo - itemFrom > old_div(self._count,2):
...
class UltimateListItem(wx.Object):
    ...
    def SetMask(self, mask):
        ...
        :param `mask`: any combination of the following bits:
         ...
         ``ULC_MASK_FONTCOLOUR``           0x80 L{get_text_colour} is valid
         ``ULC_MASK_FONT``                0x100 L{get_font} is valid
         ...
         ``ULC_MASK_WINDOW``             0x4000 L{get_window} is valid
    ...
    def GetTextColour(self):
        ...
        return (self.HasAttributes() and [self._attr.get_text_colour()] or [wx.NullColour])[0]
    ...
    def GetFont(self):
        ...
        return (self.HasAttributes() and [self._attr.get_font()] or [wx.NullFont])[0]
    ...
    def Init(self):
        ...
        self._font = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
...
class CommandListEvent(wx.PyCommandEvent):
    ...
    def __init__(self, commandTypeOrEvent=None, winid=0):
        ...
        if type(commandTypeOrEvent) == int:
            ...
            wx.PyCommandEvent.__init__(self, commandTypeOrEvent.GetEventType(), commandTypeOrEvent.get_id())
...
class UltimateListEvent(CommandListEvent):
    ...
    def __init__(self, commandTypeOrEvent=None, winid=0):
        ...
        if type(commandTypeOrEvent) == int:
            ...
            self.notify = wx.NotifyEvent(commandTypeOrEvent.GetEventType(), commandTypeOrEvent.get_id())
...
class UltimateListItemData(object):
    ...
    def SetItem(self, info):
        ...
        if info._mask & ULC_MASK_FONTCOLOUR:
            self.SetColour(info.get_text_colour())

        if info._mask & ULC_MASK_FONT:
            self.SetFont(info.get_font())
        ...
        if info.HasAttributes():
            ...
            if self._attr:
                ...
                self._attr = UltimateListItemAttr(info.get_text_colour(), info.GetBackgroundColour(),
                                                  info.get_font(), info.IsEnabled(), info.GetFooterTextColour(),
...
class UltimateListHeaderData(object):
    ...
    def GetItem(self, item):
        ...
        if self._font:
            ...
            item.Attributes().set_font(self._font)
...
class GeometryInfo(object):
    ...
    def ExtendWidth(self, w):
        ...
        self._rectLabel.x = self._rectAll.x + old_div((w - self._rectLabel.width),2)
        self._rectIcon.x = self._rectAll.x + old_div((w - self._rectIcon.width),2)
        self._rectHighlight.x = self._rectAll.x + old_div((w - self._rectHighlight.width),2)
...
class UltimateListLineData(object):
    ...
    def SetPosition(self, x, y, spacing):
        ...
        if mode in [ULC_ICON, ULC_SMALL_ICON]:
            ...
            if item.HasImage():
                ...
                self._gi._rectIcon.x = self._gi._rectAll.x + 4 + old_div((self._gi._rectAll.width - self._gi._rectIcon.width),2)
            ...
            if item.HasText():
                ...
                if self._gi._rectLabel.width > spacing:
                    ...
                    self._gi._rectLabel.x = self._gi._rectAll.x + 2 + (old_div(spacing,2)) - (old_div(self._gi._rectLabel.width,2))
    ...
    def DrawInReportMode(self, dc, line, rect, rectHL, highlighted, current, enabled, oldPN, oldBR):
        ...
        y = rect.y
        height = rect.height
        boldFont = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        ...
        for col, item in enumerate(self._items):
            ...
            if item.GetKind() in [1, 2]:
                ...
                self._owner.DrawCheckbox(dc, xOld, y + old_div((height-iy+1),2), item.GetKind(), checked, enabled)
                xOld += ix
                width -= ix
            ...
            if item.HasImage():
                ...
                for img in images:
                    ...
                    self._owner.DrawImage(img, dc, xOld, y + old_div((height-iy),2), enabled)
...
##                    width -= IMAGE_MARGIN_IN_REPORT_MODE - MARGIN_BETWEEN_TEXT_AND_ICON
            ...
            if wnd:
                ...
                if rect.height > ySize and not item._expandWin:
                    ya += old_div((rect.height - ySize),2)
            ...
            if item.HasText():
                ...
                if item.IsHyperText():
                    dc.set_font(self._owner.GetHyperTextFont())
                    if item.GetVisited():
...
class UltimateListHeaderWindow(wx.PyControl):
    ...
    def __init__(self, win, id, owner, pos=wx.DefaultPosition,
        ...
        if _USE_VISATTR:
            ...
            if not self._hasFont:
                self.SetOwnFont(wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT))
    ...
    def GetTextHeight(self):
        ...
        for i in range(numColumns):
            ...
            if item.get_font().IsOk():
                dc.set_font(item.get_font())
            else:
                dc.set_font(self.GetFont())
        ...
        for i in range(numColumns):
            ...
            if isFooter:
                if item.GetFooterFont().IsOk():
                    dc.set_font(item.GetFooterFont())
                else:
                    dc.set_font(self.GetFont())
            else:
                if item.get_font().IsOk():
            ...
            if kind in [1, 2]:
                ...
                self._owner.DrawCheckbox(dc, x + HEADER_OFFSET_X, HEADER_OFFSET_Y + old_div((h - 4 - iy),2), kind, checked, enabled)
            ...
            if align == ULC_FORMAT_LEFT:
                ...
                xAligned = x + wcheck + old_div((cw - wLabel),2)
                ...
                for indx, img in enumerate(image):
                    if img >= 0:
                        imageList.Draw(img, dc,
                                       ...
                                       HEADER_OFFSET_Y + old_div((h - 4 - iy),2),
    ...
    def HandleColumnCheck(self, column, pos):
        ...
        w, h = self.GetClientSize()
        ix, iy = self._owner.GetCheckboxImageSize()
        rect = wx.Rect(theX + HEADER_OFFSET_X, HEADER_OFFSET_Y + old_div((h - 4 - iy),2), ix, iy)
    ...
    def SendListEvent(self, eventType, pos):
        ...
        parent = self.GetParent()
        le = UltimateListEvent(eventType, parent.get_id())
...
class UltimateListMainWindow(wx.PyScrolledWindow):
    ...
    def SetFont(self, font):
        ...
        if not wx.PyScrolledWindow.set_font(self, font):
            return False
...
# ----------------------------------------------------------------------------
    ...
    def GetLineCheckboxRect(self, line):
        ...
        rect = wx.Rect(HEADER_OFFSET_X, self.GetLineY(line) + old_div(LH,2) - old_div(hcheck,2), wcheck, hcheck)
        return rect
        ...
        if self.InReportView():# and not self.IsVirtual():
            ...
            for col, item in enumerate(ld._items):
                ...
                if item.GetKind() in [1, 2]:
                    ...
                    LH = self.GetLineHeight(line)
                    rect = wx.Rect(xOld, lineY + old_div(LH,2) - old_div(iy,2), ix, iy)
                    if rect.Contains((x, y)):
                        newItem = self.GetParent().GetItem(line, col)
...
# ----------------------------------------------------------------------------
    ...
    def OnPaint(self, event):
        ...
        if self.InReportView():
            ...
            if self.IsVirtual():
                ...
                evCache = UltimateListEvent(wxEVT_COMMAND_LIST_CACHE_HINT, self.GetParent().get_id())
    ...
    def SendNotify(self, line, command, point=wx.DefaultPosition):
        ...
        bRet = True
        le = UltimateListEvent(command, self.GetParent().get_id())
    ...
    def EditLabel(self, item):
        ...
        le = UltimateListEvent(wxEVT_COMMAND_LIST_BEGIN_LABEL_EDIT, self.GetParent().get_id())
        le.SetEventObject(self.GetParent())
        le.m_itemIndex = item
    ...
    def OnRenameAccept(self, itemEdit, value):
        ...
        le = UltimateListEvent(wxEVT_COMMAND_LIST_END_LABEL_EDIT, self.GetParent().get_id())
        le.SetEventObject(self.GetParent())
    ...
    def OnMouse(self, event):
        ...
        if self.IsEmpty():
            if event.RightDown():
                ...
                evtCtx = wx.ContextMenuEvent(wx.wxEVT_CONTEXT_MENU, self.GetParent().get_id(),
        ...
        if self.InReportView():
            if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
                current = old_div(y,self.GetLineHeight())
                if current < count:
    ...
    def DragFinish(self, event):
        ...
        le = UltimateListEvent(wxEVT_COMMAND_LIST_END_DRAG, self.GetParent().get_id())
        le.SetEventObject(self.GetParent())
    ...
    def MoveToItem(self, item):
        ...
        if self.InReportView():
            ...
            if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):            
                ...
                if rect.y < view_y:
                    self.Scroll(-1, old_div(rect.y,hLine))
                if rect.y+rect.height+5 > view_y+client_h:
                    self.Scroll(-1, old_div((rect.y+rect.height-client_h+hLine),hLine))
                ...
                if start_y < view_y:
                    ...
                    self.Scroll(-1, old_div(start_y,SCROLL_UNIT_Y))
                    
                if start_y + height > view_y + client_h:
                    while start_y + height < view_y + client_h:
                        start_y += SCROLL_UNIT_Y

                    self.Scroll(-1, old_div((start_y+height-client_h+SCROLL_UNIT_Y),SCROLL_UNIT_Y))
            ...
            if rect.x-view_x < 5:
                sx = old_div((rect.x - 5),SCROLL_UNIT_X)
            if rect.x+rect.width-5 > view_x+client_w:
                sx = old_div((rect.x + rect.width - client_w + SCROLL_UNIT_X),SCROLL_UNIT_X)

            if rect.y-view_y < 5:
                sy = old_div((rect.y - 5),hLine)
            if rect.y + rect.height - 5 > view_y + client_h:
                sy = old_div((rect.y + rect.height - client_h + hLine),hLine)
...
# ----------------------------------------------------------------------------
    ...
    def OnChar(self, event):
        ...
        if self.HasCurrent():
            le = UltimateListEvent(wxEVT_COMMAND_LIST_KEY_DOWN, self.GetParent().get_id())
...
# ----------------------------------------------------------------------------
    ...
    def OnSetFocus(self, event):
        ...
        if self.GetParent():
            event = wx.FocusEvent(wx.wxEVT_SET_FOCUS, self.GetParent().get_id())
            event.SetEventObject(self.GetParent())
            if self.GetParent().GetEventHandler().ProcessEvent(event):
                return
    ...
    def OnKillFocus(self, event):
        ...
        if self.GetParent():
            event = wx.FocusEvent(wx.wxEVT_KILL_FOCUS, self.GetParent().get_id())
            event.SetEventObject(self.GetParent())
            if self.GetParent().GetEventHandler().ProcessEvent(event):
                return
    ...
    def GetCheckboxImageSize(self):
        ...
        bmp = self._image_list_check.get_bitmap(0)
        return bmp.GetWidth(), bmp.GetHeight()
    ...
    def SetImageListCheck(self, sizex, sizey, imglist=None):
        ...
        if imglist is None:
            ...
            for ii in range(self._image_list_check.GetImageCount()):
                ...
                bmp = self._image_list_check.get_bitmap(ii)
                newbmp = MakeDisabledBitmap(bmp)
...
# ----------------------------------------------------------------------------
    ...
    def CheckItem(self, item, checked=True, sendEvent=True):
        ...
        if sendEvent:
            ...
            parent = self.GetParent()
            le = UltimateListEvent(wxEVT_COMMAND_LIST_ITEM_CHECKING, parent.get_id())
        ...
        le = UltimateListEvent(wxEVT_COMMAND_LIST_ITEM_CHECKED, parent.get_id())
        le.m_itemIndex = item._itemId
        le.m_item = item
        le.SetEventObject(parent)
        parent.GetEventHandler().ProcessEvent(le)
    ...
    def RecalculatePositions(self, noRefresh=False):
        ...
        if self.InReportView():
            ...
            if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
                ...
                self._linesPerPage = old_div(clientHeight,lineHeight)

                self.SetScrollbars(SCROLL_UNIT_X, lineHeight,
                                   old_div((self.GetHeaderWidth()-decrement),SCROLL_UNIT_X),
                                   old_div((entireHeight + lineHeight - 1),lineHeight),
                ...
                self.SetScrollbars(SCROLL_UNIT_X, SCROLL_UNIT_Y,
                                   old_div((self.GetHeaderWidth()-decrement),SCROLL_UNIT_X),
                                   old_div((entireHeight + SCROLL_UNIT_Y - 1),SCROLL_UNIT_Y),
                                   self.GetScrollPos(wx.HORIZONTAL),
                                   self.GetScrollPos(wx.VERTICAL),
                                   True)
            ...
            if self.HasAGWFlag(ULC_ALIGN_LEFT | ULC_ALIGN_TOP):
                ...
                self.SetScrollbars(SCROLL_UNIT_X, lineHeight,
                                   old_div((x + SCROLL_UNIT_X),SCROLL_UNIT_X),
                                   old_div((y + lineHeight),lineHeight),
                                   self.GetScrollPos(wx.HORIZONTAL),
                                   self.GetScrollPos(wx.VERTICAL),
                                   True)
                ...
                self.SetScrollbars(SCROLL_UNIT_X, lineHeight,
                                   old_div((entireWidth + SCROLL_UNIT_X),SCROLL_UNIT_X),
                                   0,
                                   self.GetScrollPos(wx.HORIZONTAL),
                                   0,
                                   True)
...
# ----------------------------------------------------------------------------
    ...
    def DoDeleteAllItems(self):
        ...
        event = UltimateListEvent(wxEVT_COMMAND_LIST_DELETE_ALL_ITEMS, self.GetParent().get_id())
        event.SetEventObject(self.GetParent())
...
# ----------------------------------------------------------------------------
    ...
    def GetCountPerPage(self):
        ...
        if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
            if not self._linesPerPage:
                self._linesPerPage = old_div(self.GetClientSize().y,self.GetLineHeight())
    ...
    def ScrollList(self, dx, dy):
        ...
        if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
            ...
            self.Scroll(-1, top + old_div(dy,hLine))
        else:
            self.Scroll(-1, top + old_div(dy,SCROLL_UNIT_Y))
...
# ----------------------------------------------------------------------------
    ...
    def SetFont(self, font):
        ...
        if not wx.PyControl.set_font(self, font):
            return False
    ...
    def GetClassDefaultAttributes(self, variant):
        ...
        attr.font  = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        return attr
    ...
    def Refresh(self, eraseBackground=True, rect=None):
        ...
        if not rect:
            ...
            if self._headerWin:
                ...
                rectHeader = self._headerWin.get_rect()
