...
def DrawTreeItemButton(win, dc, rect, flags):
    ...
    # black lines
    xMiddle = rect.x + old_div(rect.width,2)
    yMiddle = rect.y + old_div(rect.height,2)

    # half of the length of the horz lines in "-" and "+"
    halfWidth = old_div(rect.width,2) - 2
    ...
    if not flags & _CONTROL_EXPANDED:
        ...
        # turn "-" into "+"
        halfHeight = old_div(rect.height,2) - 2
...
class DragImage(wx.DragImage):
    ...
    def __init__(self, treeCtrl, item):
        ...
        text = item.GetText()
        font = item.Attr().get_font()
        colour = item.Attr().get_text_colour()
        if not colour:
            colour = wx.BLACK
        if not font:
        ...
        if image != _NO_IMAGE:    
            if treeCtrl._imageListNormal:
                ...
                image_w += 4
                itemimage = treeCtrl._imageListNormal.get_bitmap(image)
        ...
        if checkimage is not None:
            if treeCtrl._imageListCheck:
                ...
                wcheck += 4
                itemcheck = treeCtrl._imageListCheck.get_bitmap(checkimage)
        ...
        if image_w:
            ximagepos = wcheck
            yimagepos = ((total_h > image_h) and [old_div((total_h-image_h),2)] or [0])[0]

        if checkimage is not None:
            xcheckpos = 2
            ycheckpos = ((total_h > image_h) and [old_div((total_h-image_h),2)] or [0])[0] + 2

        extraH = ((total_h > height) and [old_div((total_h - height),2)] or [0])[0]
        ...
        if total_h < 30:
            ...
            total_h += old_div(total_h,10)   # otherwise 10% extra spacing
...
class TreeTextCtrl(ExpandoTextCtrl):
    ...
    def __init__(self, owner, item=None):
        ...
        wnd = self._itemEdited.get_window()
        if wnd:
...
class GenericTreeItem(object):
    ...
    def IsChecked(self):
        """
        This is just a maybe more readable synonym for L{get_value}.
        Returns whether the item is checked or not.
...
# -----------------------------------------------------------------------------
    ...
    def GetItemTextColour(self, item):
        ...
        return item.Attr().get_text_colour()
    ...
    def SetItemTextColour(self, item, colour):
        ...
        item.Attr().set_text_colour(colour)
        self.RefreshLine(item)
    ...
    def SetFont(self, font):
        ...
        wx.PyScrolledWindow.set_font(self, font)
...
# -----------------------------------------------------------------------------
    ...
    def ScrollTo(self, item):
        ...
        if item_y < start_y+3:
            ...
            self.SetScrollbars(_PIXELS_PER_UNIT, _PIXELS_PER_UNIT, old_div(x,_PIXELS_PER_UNIT), old_div(y,_PIXELS_PER_UNIT), x_pos, old_div(item_y,_PIXELS_PER_UNIT))
            ...
            self.SetScrollbars(_PIXELS_PER_UNIT, _PIXELS_PER_UNIT, old_div(x,_PIXELS_PER_UNIT), old_div(y,_PIXELS_PER_UNIT), x_pos, old_div((item_y+self.GetLineHeight(item)-client_h),_PIXELS_PER_UNIT) )
    ...
    def CalculateLineHeight(self):
        ...
        if self._lineHeight < 30:
            ...
            self._lineHeight += old_div(self._lineHeight,10)   # otherwise 10% extra spacing
    ...
    def SetImageListCheck(self, sizex, sizey, imglist=None):
        ...
        if imglist is None:
            ...
            for ii in range(self._imageListCheck.GetImageCount()):
                ...
                bmp = self._imageListCheck.get_bitmap(ii)
                newbmp = MakeDisabledBitmap(bmp)
...
# -----------------------------------------------------------------------------
    ...
    def AdjustMyScrollbars(self):
        ...
        if self._anchor:
            ...
            self.SetScrollbars(_PIXELS_PER_UNIT, _PIXELS_PER_UNIT, old_div(x,_PIXELS_PER_UNIT), old_div(y,_PIXELS_PER_UNIT), x_pos, y_pos)
    ...
    def PaintItem(self, item, dc, level, align):
        ...
        if attr and attr.HasFont():
            dc.set_font(attr.get_font())
        else:
            if item.IsBold():
                dc.set_font(self._boldFont)
            elif item.IsItalic():
                dc.set_font(self._italicFont)
        if item.IsHyperText():
            dc.set_font(self.GetHyperTextFont())
            if item.GetVisited():
        ...
        if image != _NO_IMAGE:
            ...
            imglist.Draw(image, dc,
                         item.GetX() + wcheck,
                         item.GetY() + ((total_h > image_h) and [old_div((total_h-image_h),2)] or [0])[0],
        ...
        if wcheck:
            ...
            imglist.Draw(checkimage, dc,
                         item.GetX(),
                         item.GetY() + ((total_h > hcheck) and [old_div((total_h-hcheck),2)] or [0])[0],
                         wx.IMAGELIST_DRAW_TRANSPARENT)
        ...
        if leftimage != _NO_IMAGE:
            ...
            imglist.Draw(leftimage, dc,
                         4,
                         item.GetY() + ((total_h > l_image_h) and [old_div((total_h-l_image_h),2)] or [0])[0],
                         wx.IMAGELIST_DRAW_TRANSPARENT)

        dc.SetBackgroundMode(wx.TRANSPARENT)
        extraH = ((total_h > text_h) and [old_div((total_h - text_h),2)] or [0])[0]
        ...
        if wnd:
            ...
            if item.GetHeight() > item.GetWindowSize()[1]:
                ya += old_div((item.GetHeight() - item.GetWindowSize()[1]),2)
        ...
        if separator:
            ...
            if item.IsEnabled():
                if attr and attr.HasTextColour():
                    separatorPen = wx.Pen(attr.get_text_colour(), 1)
                else:
            ...
            dc.DrawLine(item.GetX()+2, item.GetY()+old_div(total_h,2), w, item.GetY()+old_div(total_h,2))
            dc.SetPen(oldPen)
            
        # restore normal font
        dc.set_font(self._normalFont)
    ...
    def PaintLevel(self, item, dc, level, y, align):
        ...
        left_image_list = 0
        if self._imageListLeft:
            left_image_list += self._imageListLeft.get_bitmap(0).GetWidth()
        ...
        if self.IsExposed(exposed_x, exposed_y, 10000, h):  # 10000 = very much
            ...
            if item.HasPlus() and self.HasButtons():
                ...
                if self._imageListButtons:
                    ...
                    xx = x - old_div(image_w,2)
                    yy = y_mid - old_div(image_h,2)
                    ...
                    if self.HasAGWFlag(TR_TWIST_BUTTONS):
                        ...
                        self._drawingfunction(self, dc, wx.Rect(x - old_div(wImage,2), y_mid - old_div(hImage,2),wImage, hImage), flag)
...
# -----------------------------------------------------------------------------
    ...
    def OnKeyDown(self, event):
        ...
        if keyCode in [ord("+"), wx.WXK_ADD]:       # "+"
            ...
            event._pointDrag = wx.Point(itemRect.GetX(), itemRect.GetY() + old_div(itemRect.GetHeight(),2))
            event.SetEventObject(self)
            self.GetEventHandler().ProcessEvent(event)
