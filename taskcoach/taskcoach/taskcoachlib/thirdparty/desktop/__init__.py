...
DESKTOP_LAUNCH="my opener --urlpo"       Should run the "my opener" program to
                                        open URLs.
...
def open(url, desktop=None, wait=0):
    """
    Open the 'urlpo' in the current desktop's preferred file browser. If the
    optional 'desktop' parameter is specified then attempt to use that
    particular desktop environment's mechanisms to open the 'urlpo' instead of
    ...
    variable to open the specified 'urlpo'. DESKTOP_LAUNCH should be a command,
    ...
    program) associated with the 'urlpo' is returned by this function. If the
    ...
    # Finish with an error where no suitable desktop was identified.
        ...
        raise_(OSError, "Desktop '%s' not supported (neither DESKTOP_LAUNCH nor os.startfile could be used)" %
               desktop_in_use)
