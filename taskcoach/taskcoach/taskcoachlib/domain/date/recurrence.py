...
class Recurrence(object):
    ...
    def __init__(self, unit='', amount=1, same_weekday=False, maximum=0, count=0,
                 stop_datetime=None, recur_based_on_completion=False):
        assert unit in self.units
        assert amount >= 1
        self.unit = unit
        self.amount = amount
        self.stop_datetime = stop_datetime or date.DateTime()
        self.sameWeekday = same_weekday
        # Maximum number of recurrences we give out, 0 == infinite:
        self.max = maximum
        self.count = count  # Actual number of recurrences given out so far
        self.recurBasedOnCompletion = recur_based_on_completion

    def __call__(self, *date_times, **kwargs):
        result = [self._nextDateTime(dateTime) for dateTime in date_times]
