...
class FMTitlePanel(wx.Panel):
    ...
    def __init__(self, parent, title):
        ...
        dc.set_font(wx.SystemSettings_GetFont(wx.SYS_DEFAULT_GUI_FONT))
...
class FMCustomizeDlg(wx.Dialog):
    ...
    def CreateMenusPage(self):
        ...
        for i in range(len(mb._items)):
            ...
            dummy, lableOnly = ArtManager.Get().GetAccelIndex(mb._items[i].get_title())
    ...
    def CreateOptionsPage(self):
        ...
        self._shadowUnderTB.set_value(ArtManager.Get().GetRaiseToolbar())
        ...
        self._colour = wx.ComboBox(options, self._colourID, ArtManager.Get().GetMenuBarColourScheme(),
    ...
    def OnChangeStyle(self, event):
        ...
        if event.get_id() == self._menuStyleID:
        ...
        if event.get_id() == self._drawBorderID:
            ArtManager.Get().DrawMenuBarBorder(event.IsChecked())
            mb.Refresh()
            return

        if event.get_id() == self._drawVertGradID:
            ArtManager.Get().SetMBVerticalGradient(event.IsChecked())
            mb.Refresh()
            return

        if event.get_id() == self._colourID:
        ...
        if event.get_id() == self._shadowUnderTBID:
