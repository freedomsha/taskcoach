...
def Intersection(line1, line2):
    ...
    if line1.slope == line2.slope:
        ...
        y = old_div(((line1.c * line2.slope) - (line2.c * line1.slope)), (line2.slope - line1.slope))
        x = old_div((y - line1.c), line1.slope)
...
def PointOnLine(pt1, pt2, length, maxLen):
    ...
    if pt2.x != pt1.x:
        ...
        x = old_div((B + sqrt(B * B - (4.0 * A * C))), (2.0 * A))
        y = m * x + c
...
class RGBCube(BasePyControl):
    ...
    def __init__(self, parent):
        ...
        BasePyControl.__init__(self, parent, bitmap=RGBCubeImage.get_bitmap())
        self._index = -1
    ...
    def DrawMarkers(self, dc=None):
        ...
        pt[0] = PointOnLine(Vertex, Top, old_div((colour.r * redLen), 255), redLen)
        pt[1] = PointOnLine(Vertex, Left, old_div((colour.g * greenLen), 255), greenLen)
        pt[2] = PointOnLine(Vertex, Right, old_div((colour.b * blueLen), 255), blueLen)
    ...
    def OnMotion(self, event):
        ...
        if self._index == RED:
            ...
            pt = PointOnLine(Vertex, Top, old_div((colour.r * redLen), 255), redLen)
            ...
            pt = PointOnLine(Vertex, Left, old_div((colour.g * greenLen), 255), greenLen)
            ...
            pt = PointOnLine(Vertex, Right, old_div((colour.b * blueLen), 255), blueLen)
...
class HSVWheel(BasePyControl):
    ...
    def __init__(self, parent):
        ...
        BasePyControl.__init__(self, parent, bitmap=HSVWheelImage.get_bitmap())
        self._mouseIn = False
...
class AlphaCtrl(BaseLineCtrl):
    ...
    def DrawAlphaShading(self, dc, rect):
        ...
        alpha = 255.0
        vstep = old_div(255.0 * 2, (rect.height - 1))
...
class CustomPanel(wx.PyControl):
    ...
    def OnLeftDown(self, event):
        ...
        selX = old_div((x - self._customColourRect.x), (self._smallRectangleSize.x + self._gridSpacing))
        selY = old_div((y - self._customColourRect.y), (self._smallRectangleSize.y + self._gridSpacing))
        ptr = selX + selY * 8
    ...
    def PaintHighlight(self, dc, draw=True):
        ...
        y = old_div(self._colourSelection, 8)
...
class CubeColourDialog(wx.Dialog):
    ...
    def SetSpinVals(self):
        ...
        self.redSpin.set_value(self._colour.r)
        self.greenSpin.set_value(self._colour.g)
        self.blueSpin.set_value(self._colour.b)

        self.hueSpin.set_value(self._colour.h)
        self.saturationSpin.set_value(self._colour.s)
        self.brightnessSpin.set_value(self._colour.v)

        self.alphaSpin.set_value(self._colour._alpha)
    ...
    def SetCodes(self):
        ...
        self.htmlCode.set_value(colour)
        self.htmlCode.Refresh()

        if colour in HTMLCodes:
            colourName, access, webSafe = HTMLCodes[colour]
            self.webSafe.set_value(webSafe)
            self.accessCode.set_value(access)
            self.htmlName.set_value(colourName)
        else:
            self.webSafe.set_value("")
            self.accessCode.set_value("")
            self.htmlName.set_value("")
