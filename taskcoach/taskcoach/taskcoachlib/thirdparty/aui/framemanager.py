...
try:
    ...
    from . import auibar
# except SyntaxError:
#    import taskcoachlib.thirdparty.aui.auibar
try:
    import auibook
except ModuleNotFoundError:
    from . import auibook
...
class AuiDockingGuideWindow(wx.Window):
    ...
    def OnPaint(self, event):
        ...
        if self._currentImage.IsOk() and self._valid:
            ...
            # Type 'AutoBufferedPaintDC' doesn't have expected attribute 'DrawRectangleRect'
            self.Draw(dc)
...
class AuiCenterDockingGuide(AuiDockingGuide):
    ...
    def CreateShapesWithStyle(self):
        ...
        # Local variable 'region' might be referenced before assignment
        self.region = region
    ...
    def SetPaneWindow(self, pane):
        ...
        if min_size.y < caption_size or min_size.x < button_size:
            ...
            if min_size.y < caption_size:
                new_y = \
                (pane.IsResizeable() and [2 * wx.SystemSettings.GetMetric(wx.SYS_EDGE_Y) + caption_size] or [1])[0]
            if min_size.x < button_size:
                new_x = (pane.IsResizeable() and [2 * wx.SystemSettings.GetMetric(wx.SYS_EDGE_X) + button_size] or [1])[
                    0]
...
def FindOppositeDocks(docks, dock_direction):
    ...
    # Local variable 'arr' might be referenced before assignment
    return arr
        ...
        if pane.frame:
            ...
            if diff.x != 0 or diff.y != 0:
                if wx.Platform == "__WXMSW__" and (
                        self._agwFlags & AUI_MGR_TRANSPARENT_DRAG) == 0:  # and not self.CheckPaneMove(pane):
                    # return
