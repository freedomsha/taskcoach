...
class AbstractHandler(object):
    ...
    def __init__(self, pObject):
        ...
        self._window = pObject.get_window()
...
class CheckBoxHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        if check.Is3State():
            ...
            obj.SaveCtrlValue(PERSIST_CHECKBOX, check.get_value())
...
class RadioButtonHandler(AbstractHandler):
    ...
    def Save(self):
        radio, obj = self._window, self._pObject
        obj.SaveCtrlValue(PERSIST_RADIOBUTTON_VALUE, radio.get_value())
        return True
...
class SliderHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        obj.SaveCtrlValue(PERSIST_SLIDER_VALUE, slider.get_value())
        return True
...
class SpinHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        obj.SaveCtrlValue(PERSIST_SPIN_VALUE, spin.get_value())
        return True
...
class TextCtrlHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        obj.SaveCtrlValue(PERSIST_TEXTCTRL_VALUE, text.get_value())
        return True
...
class ToggleButtonHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        obj.SaveValue(PERSIST_TOGGLEBUTTON_TOGGLED, toggle.get_value())
        return True
...
class TreeCtrlHandler(AbstractHandler):
    ...
    def __init__(self, pObject):
        ...
        AbstractHandler.__init__(self, pObject)
        self._isTreeList = isinstance(pObject.get_window(), wx.gizmos.TreeListCtrl)
...
class DatePickerHandler(AbstractHandler):
    ...
    def Save(self):
        ...
        obj.SaveCtrlValue(PERSIST_DATEPICKER_DATE, wxDate2PyDate(datePicker.get_value()))
        return True
    ...
    def Restore(self):
        ...
        if value is not None:
            datePicker.set_value(PyDate2wxDate(value))
            return True
...
class MediaCtrlHandler(AbstractHandler):
    ...
    def Restore(self):
        ...
        return position is not None and volume is not None and rate is not None
        ...
        for indx in range(menuCount):
            ...
            for item in menu.GetMenuItems():
                if item.GetKind() in [wx.ITEM_CHECK, wx.ITEM_RADIO]:
                    checkRadioItems[item.get_id()] = item.IsChecked()
        ...
        for indx in range(toolCount):
            ...
            if tool is not None:
                if tool.GetKind() in [AUI.ITEM_CHECK, AUI.ITEM_RADIO]:
                    checkRadioItems[tool.get_id()] = tool.GetState() & AUI.AUI_BUTTON_STATE_CHECKED
...
def FindHandler(pObject):
    ...
    for handler, subclasses in list(STANDALONE_HANDLERS.items()):
        for subclass in subclasses:
            if issubclass(klass, subclass):
                return eval(handler)(pObject)

    for handler, subclasses in list(HANDLERS.items()):
        for subclass in subclasses:
            if issubclass(klass, subclass):
                return eval(handler)(pObject)

    raise Exception("Unsupported persistent handler (class=%s, name=%s)" % (klass, window.get_name()))
