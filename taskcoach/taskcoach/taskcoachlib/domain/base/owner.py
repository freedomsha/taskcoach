...
def DomainObjectOwnerMetaclass(name, bases, ns):
    ...
    @patterns.eventSource
    def addObject(instance, owned_object, event=None):
        getattr(instance, '_%s__%ss' % (name, klass.__ownedType__.lower())).append(owned_object)
        changedEvent(instance, event, owned_object)
        addedEvent(instance, event, owned_object)
    ...
    @patterns.eventSource
    def removeObjects(instance, *owned_objects, **kwargs):
        if not owned_objects:
            return
        for ownedObject in owned_objects:
            try:
                getattr(instance, '_%s__%ss' % (name, klass.__ownedType__.lower())).remove(ownedObject)
            except ValueError:
                pass
        event = kwargs.pop('event', None)
        changedEvent(instance, event, *owned_objects)
        removedEvent(instance, event, *owned_objects)
