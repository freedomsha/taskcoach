...
class RibbonPanel(RibbonControl):
    ...
    def DoGetNextSmallerSize(self, direction, relative_to):
        ...
        if direction & wx.HORIZONTAL:        
            current.x = old_div((current.x * 4), 5)
            if current.x < minimum.x:            
                current.x = minimum.x
            
        if direction & wx.VERTICAL:        
            current.y = old_div((current.y * 4), 5)
    ...
    def DoGetNextLargerSize(self, direction, relative_to):
        ...
        if direction & wx.HORIZONTAL:        
            current.x = old_div((current.x * 5 + 3), 4)
        
        if direction & wx.VERTICAL:
            current.y = old_div((current.y * 5 + 3), 4)
    ...
    def GetExpandedPosition(self, panel, expanded_size, direction):
        ...
        if direction == wx.NORTH:
            pos.x = panel.GetX() + old_div((panel.GetWidth() - expanded_size.GetWidth()), 2)
            ...
            pos.y = panel.GetY() + old_div((panel.GetHeight() - expanded_size.GetHeight()), 2)
            secondary_x = -1
