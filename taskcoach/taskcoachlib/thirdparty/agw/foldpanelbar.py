...
class CaptionBar(wx.Window):
    ...
    def __init__(self, parent, id, pos, size, caption="",
        ...
        if foldIcons is None:
            ...
            bmp = ExpandedIcon.get_bitmap()
            foldIcons.Add(bmp)
            bmp = CollapsedIcon.get_bitmap()
            foldIcons.Add(bmp)
    ...
    def ApplyCaptionStyle(self, cbstyle=None, applyDefault=True):
        ...
        if applyDefault:
            ...
            if not newstyle.CaptionFontUsed():
                newstyle.SetCaptionFont(self.GetParent().get_font())
    ...
    def OnPaint(self, event):
        ...
        dc.set_font(self._style.GetCaptionFont())
        dc.SetTextForeground(self._style.GetCaptionColour())

        if vertical:
            dc.DrawText(self._caption, 4, old_div(FPB_EXTRA_Y,2))
        else:
            dc.DrawRotatedText(self._caption, old_div(FPB_EXTRA_Y,2),
        ...
        if self._foldIcons:
            ...
            if vertical:
                ...
                self._foldIcons.Draw(index, dc, drw,
                                     old_div((wndRect.GetHeight() - self._iconHeight),2),
                                     wx.IMAGELIST_DRAW_TRANSPARENT)
            else:
                self._foldIcons.Draw(index, dc,
                                     old_div((wndRect.GetWidth() - self._iconWidth),2),
