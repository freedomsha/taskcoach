...
# wx.TR_EXTENDED n'existe pas dans wx retiré de customtreectrl
...
class TreeListHeaderWindow(wx.Window):
    ...
    def OnPaint(self, event):
        ...
        for i in range(numColumns):
            ...
            if image != -1 and imageList:
                params.m_labelBitmap = imageList.get_bitmap(image)
    ...
    def SendListEvent(self, evtType, pos):
        ...
        le = wx.ListEvent(evtType, parent.get_id())
...
class EditCtrl(object):
    ...
    def __init__(self, parent, id=wx.ID_ANY, item=None, column=None, owner=None,
        ...
        wnd = self._itemEdited.get_window(column)
        if wnd:
...
# ----------------------------------------------------------------------------
    ...
    def ScrollTo(self, item):
        ...
        if item._y < start_y+3:
            ...
            self.SetScrollbars(xUnit, yUnit, (xUnit and [old_div(x,xUnit)] or [0])[0], (yUnit and [old_div(y,yUnit)] or [0])[0],
                               x_pos, (yUnit and [old_div(item._y,yUnit)] or [0])[0])
            
        elif item._y+self.GetLineHeight(item) > start_y+client_h:
            # going up, item should appear at bottom
            item._y += yUnit + 2
            self.SetScrollbars(xUnit, yUnit, (xUnit and [old_div(x,xUnit)] or [0])[0], (yUnit and [old_div(y,yUnit)] or [0])[0],
                               x_pos, (yUnit and [old_div((item._y+self.GetLineHeight(item)-client_h),yUnit)] or [0])[0])
...
# ----------------------------------------------------------------------------
    ...
    def AdjustMyScrollbars(self):
        ...
        if self._anchor:
            ...
            self.SetScrollbars(xUnit, yUnit, old_div(x,xUnit), old_div(y,yUnit), x_pos, y_pos)
        else:
...
##        clipper = wx.DCClipper(dc, 0, item.GetY(), total_w, total_h) # only within line
        ...
        if self.HasAGWFlag(wx.TR_FULL_ROW_HIGHLIGHT):
            ...
            if item == self._dragItem:
                ...
                wnd = item.get_window(self._main_column)
                wndx = 0
                if wnd:
        ...
        text_extraH = (total_h > text_h and [old_div((total_h - text_h),2)] or [0])[0]
        img_extraH = (total_h > self._imgHeight and [old_div((total_h-self._imgHeight),2)] or [0])[0]
        x_colstart = 0
        ...
        for i in range(self.GetColumnCount()):
            ...
            if alignment == wx.ALIGN_RIGHT:
                ...
                w = old_div((col_w - (image_w + wcheck + text_w + off_w + _MARGIN)),2)
                x += (w > 0 and [w] or [0])[0]
            else:
            ...
            if wcheck:
                ...
                imglist.Draw(checkimage, dc,
                             ...
                             item.GetY() + ((total_h > hcheck) and [old_div((total_h-hcheck),2)] or [0])[0]+1,
            ...
            if wnd:
                ...
                if item.GetHeight() > item.GetWindowSize(i)[1]:
                    ya += old_div((item.GetHeight() - item.GetWindowSize(i)[1]),2)
    ...
    def PaintLevel(self, item, dc, level, y, x_maincol):
        ...
        if self.HasButtons():
            ...
            x += (self._indent-old_div(self._indent,2))
...
##            clipper = wx.DCClipper(dc, x_maincol, y_top, clip_width, 10000)
            ...
            if not self.HasAGWFlag(wx.TR_NO_LINES):  # connection lines
                ...
                if self.HasButtons():
                    ...
                    dc.DrawLine(x2, y_mid, x - old_div(self._indent,2), y_mid)
        ...
        if item.IsExpanded():
            ...
            for child in item.GetChildren():
                ...
                if not self.HasAGWFlag(wx.TR_NO_LINES):
                    Y1 = child.GetY() + old_div(child.GetHeight(),2)
...
# ----------------------------------------------------------------------------
    ...
    def OnPaint(self, event):
        ...
        self._btnWidth2 = old_div(self._btnWidth,2)
        self._btnHeight2 = old_div(self._btnHeight,2)

        # calculate image size
        if self._imageListNormal:
            self._imgWidth, self._imgHeight = self._imageListNormal.GetSize(0)
        
        self._imgWidth2 = old_div(self._imgWidth,2)
        self._imgHeight2 = old_div(self._imgHeight,2)

        if self._imageListCheck:
            self._checkWidth, self._checkHeight = self._imageListCheck.GetSize(0)

        self._checkWidth2 = old_div(self._checkWidth,2)
        self._checkHeight2 = old_div(self._checkHeight,2)
    ...
    def EditLabel(self, item, column=None):
        ...
        te = TreeEvent(wx.wxEVT_COMMAND_TREE_BEGIN_LABEL_EDIT, self._owner.get_id())
    ...
    def OnAcceptEdit(self, value):
        ...
        le = TreeEvent(wx.wxEVT_COMMAND_TREE_END_LABEL_EDIT, self._owner.get_id())
    ...
    def OnMouse(self, event):
        ...
        if event.Dragging():
            ...
            nevent = TreeEvent(command, self._owner.get_id())
            ...
            nevent = TreeEvent(wx.wxEVT_COMMAND_TREE_END_DRAG, self._owner.get_id())
            nevent.SetEventObject(self._owner)
        ...
        if event.RightUp():
            ...
            self.SetFocus()
            nevent = TreeEvent(wx.wxEVT_COMMAND_TREE_ITEM_RIGHT_CLICK, self._owner.get_id())
            nevent.SetEventObject(self._owner)
            ...
            if event.LeftDClick():
                ...
                nevent = TreeEvent(wx.wxEVT_COMMAND_TREE_ITEM_ACTIVATED, self._owner.get_id())
                nevent.SetEventObject(self._owner)
                nevent.SetItem(item) # the item clicked
                nevent.SetInt(self._curColumn) # the column clicked
                nevent.SetPoint(p)
