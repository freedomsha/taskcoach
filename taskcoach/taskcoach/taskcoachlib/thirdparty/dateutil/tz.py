...
def gettz(name=None):
    ...
    if name is None or name == ":":
        for filepath in TZFILES:
            ...
            if os.path.isfile(filepath):
                try:
                    tz = tzfile(filepath)  # Type 'str' doesn't have expected attribute 'read'
                    break
        ...
        if os.path.isabs(name):
            if os.path.isfile(name):
                tz = tzfile(name)  # Type 'str' doesn't have expected attribute 'read'
            else:
                tz = None
        else:
            for path in TZPATHS:
                ...
                if not tz:
                    ...
                    tz = gettz(name)  # zoneinfo.gettz() will be removed in future versions,
                    # to use the dateutil-provided zoneinfo files,
                    # instantiate a ZoneInfoFile object and use ZoneInfoFile.zones.get() instead.
