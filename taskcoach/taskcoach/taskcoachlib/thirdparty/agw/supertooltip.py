...
class ToolTipWindowBase(object):
    ...
    def OnPaint(self, event):
        ...
        topRect = wx.Rect(frameRect.x, frameRect.y, frameRect.width, old_div(frameRect.height,2))
        bottomRect = wx.Rect(frameRect.x, frameRect.y+old_div(frameRect.height,2), frameRect.width, old_div(frameRect.height,2)+1)
        ...
        if header:
            ...
            dc.set_font(headerFont)
            textWidth, textHeight = dc.GetTextExtent(header)

        # Calculate the header height
        height = max(textHeight, bmpHeight)
        if header:
            dc.DrawText(header, bmpXPos+bmpWidth+self._spacing, old_div((height-textHeight+self._spacing),2))
        if headerBmp and headerBmp.IsOk():
            dc.DrawBitmap(headerBmp, bmpXPos, old_div((height-bmpHeight+self._spacing),2), True)
        ...
        for indx, line in enumerate(lines):
            ...
            yText += old_div(textHeight,2)+self._spacing
        ...
        if textHeight or bmpHeight:
            if drawFooter:
                ...
                dc.SetPen(wx.GREY_PEN)
                dc.DrawLine(self._spacing, yPos-old_div(self._spacing,2)+toAdd, width-self._spacing, yPos-old_div(self._spacing,2)+toAdd)
        ...
        yPos += toAdd
        if footer:
            dc.DrawText(footer, bmpXPos+bmpWidth+self._spacing, yPos + old_div((height-textHeight+self._spacing),2))
        if footerBmp and footerBmp.IsOk():
            dc.DrawBitmap(footerBmp, bmpXPos, yPos + old_div((height-bmpHeight+self._spacing),2), True)
    ...
    def SetFont(self, font):
        ...
        wx.PopupWindow.set_font(self, font)
    ...
    def CalculateBestSize(self):
        ...
        if header:
            ...
            maxHeight += old_div(self._spacing,2)
        if headerBmp and headerBmp.IsOk():
        ...
        if textHeight or bmpHeight:
            maxHeight += old_div(self._spacing,2) + max(textHeight, bmpHeight)
