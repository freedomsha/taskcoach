...
def GetIndicatorImage():
    ...
    stream = io.StringIO(GetIndicatorData())
...
def MakePalette(tr, tg, tb):
    ...
    l = []
    for i in range(255):
        l.extend([old_div(tr*i, 255), old_div(tg*i, 255), old_div(tb*i, 255)])
...
class Indicator(object):
    ...
    def GetPosition(self):
        ...
        if orient == wx.HORIZONTAL:
            xpos = int(pos*right) - old_div(self._img.GetWidth(),2)
            if flip:
                ypos = top
            else:
                ypos = bottom - self._img.GetHeight()
        else:
            ypos = int(pos*bottom) - old_div(self._img.GetHeight(),2)
            if flip:
                xpos = left
            else:
    ...
    def RotateImage(self, img=None):
        ...
        if orient == wx.HORIZONTAL:
            ...
            if flip:
                img = img.Rotate(old_div(-math.pi,2), (5, 5), True)
            else:
                img = img.Rotate(old_div(math.pi,2), (5, 5), True)
...
class RulerCtrl(wx.PyPanel):
    ...
    def SetIndicatorValue(self, sendEvent=True):
        ...
        if sendEvent:
            event = RulerCtrlEvent(wxEVT_INDICATOR_CHANGING, self._currentIndicator.get_id())
            event.SetOldValue(self._currentIndicator.get_value())
            ...
            self._currentIndicator.set_value(value)
    ...
    def GetIndicator(self, mousePos):
        ...
        for indicator in self._indicators:
            if indicator.get_rect().Contains(mousePos):
        ...
        if self._format == IntFormat:
            ...
            if major:
                ...
                if self.GetTimeFormat() == HHMMSS_Format:
                    ...
                    secs = int(d + 0.5)
                    if self._minor >= 1.0:
                        s = ("%d:%02d:%02d")%(old_div(secs,3600), (old_div(secs,60))%60, secs%60)
                    
                    else:
                        t1 = ("%d:%02d:")%(old_div(secs,3600), (old_div(secs,60))%60)
                    ...
                    if self._minor >= 3600.0:
                        ...
                        if minutes >= 60:
                            m = "%d:%02d:00"%(old_div(minutes,60), minutes%60)
                        else:
                            m = "%d:00"%minutes
                        ...
                        secs = int(d + 0.5)
                        if secs >= 3600:
                            t = "%d:%02d:%02d"%(old_div(secs,3600), (old_div(secs,60))%60, secs%60)
                        elif secs >= 60:
                            t = "%d:%02d"%(old_div(secs,60), secs%60)
                        else:
                            t = "%d"%secs
    ...
    def Tick(self, dc, pos, d, major):
        ...
        dc.set_font((major and [self._majorfont] or [self._minorfont])[0])
    ...
    def Update(self, dc):
        ...
        if not self._log:
            ...
            if self._min*self._max < 0.0: 
                mid = int(self._length*(old_div(self._min,(self._min-self._max))) + 0.5)
                self.Tick(dc, mid, 0.0, True)
            
            sg = ((UPP > 0.0) and [1.0] or [-1.0])[0]
            
            # Major ticks
            d = self._min - old_div(UPP,2)
            lastd = d
            majorint = int(math.floor(old_div(sg*d,self._major)))
            ii = -1
            ...
            while ii <= self._length:
                ...
                if int(math.floor(old_div(sg*d,self._major))) > majorint:
                    majorint = int(math.floor(old_div(sg*d,self._major)))
            ...
            d = self._min - old_div(UPP,2)
            lastd = d
            minorint = int(math.floor(old_div(sg*d,self._minor)))
            ii = -1
            ...
            while ii <= self._length:
                ...
                if int(math.floor(old_div(sg*d,self._minor))) > minorint:
                    minorint = int(math.floor(old_div(sg*d,self._minor)))
            ...
            scale = old_div(self._length,(hilog - lolog))
