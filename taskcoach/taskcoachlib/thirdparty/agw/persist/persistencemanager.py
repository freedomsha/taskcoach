...
class PersistentObject(object):
    ...
    def __init__(self, window, persistenceHandler=None):
        ...
        if self._name in BAD_DEFAULT_NAMES:
            warnings.warn(
                "Window names should not be defaulted! (class=%s, name=%s)" % (window.__class__, window.get_name()))
...
class PersistenceManager(object):
    ...
    def Find(self, window):
        ...
        if window.get_name() in self._persistentObjects:
            return window
    ...
    def Register(self, window, persistenceHandler=None):
        ...
        if self.Find(window):
            raise Exception("Object (class=%s, name=%s) is already registered" % (window.__class__, window.get_name()))
    ...
    def SaveAndUnregister(self, window=None):
        ...
        if window is None:
            for name, obj in self._persistentObjects.items():
                self.SaveAndUnregister(obj.get_window())
    ...
    def GetKey(self, obj, keyName):
        ...
        key += CONFIG_PATH_SEPARATOR + obj.get_name()
