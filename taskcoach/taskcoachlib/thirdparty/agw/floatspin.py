...
""" Emitted when the user changes the value of L{FloatSpin}, either with the mouse or""" """ with the keyboard. """
...
class FloatSpin(wx.PyControl):
    ...
    def __init__(
        ...
        # Ensure the default value respects the min_val
        self._value = self.ClampValue(FixedPoint(int(value), 20))
        if min_val is not None and self._value < min_val:
            self._value = min_val
        self._defaultvalue = self._value
        self._increment = FixedPoint(int(increment), 20)
        self._spinmodifier = FixedPoint(int(1.0), 20)
        self._digits = digits
        ...
        self._validkeycode.extend(
            [wx.WXK_RETURN, wx.WXK_TAB, wx.WXK_BACK, wx.WXK_LEFT, wx.WXK_RIGHT]
        )
    ...
    def ClampValue(self, var):
        ...
        La méthode ClampValue de FloatSpin assure que la valeur fournie (var)
        reste dans un intervalle défini par _min et _max.
        En d'autres termes, elle ramène var à ces limites si elle en sort.
        ...
        # Vérification de la borne inférieure (_min):
        # Si _min est défini (c'est-à-dire qu'il n'est pas None)
        # et que var est inférieur à cette valeur, var est mis à _min.
        if self._min is not None:
            if var < self._min:
                var = self._min
                return var

        # Vérification de la borne supérieure (_max):
        # Si _max est défini et que var est supérieur à cette valeur,
        # var est mis à _max.
        if self._max is not None:
            if var > self._max:
                var = self._max

        # La méthode retourne la valeur de var après vérification et potentielle modification.
        return var
