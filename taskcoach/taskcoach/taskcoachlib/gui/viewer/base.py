...
# class PreViewer(type(patterns.Observer), type(wx.Panel)):
#     pass


class Viewer(patterns.Observer, wx.Panel, metaclass=patterns.makecls(patterns.NumberedInstances)):
    ...
    def __DisplayBalloon(self):
        ...
        if self.toolbar.IsShownOnScreen() and hasattr(wx.GetTopLevelParent(self), 'AddBalloonTip'):
            wx.GetTopLevelParent(self).AddBalloonTip(self.settings, 'customizabletoolbars', self.toolbar,
                                                     message=_('''Click on the gear icon on the right 
                                                     to add buttons and rearrange them.'''),
    ...
    def previousSettingsSection(self):
        ...
        while previousSectionNumber > 0:
            previous_section = self.__settingsSection + str(previousSectionNumber)
            if self.settings.has_section(previous_section):
...
class CategorizableViewerMixin(object):
    def getItemTooltipData(self, item):
        return [('folder_blue_arrow_icon',
                 [u', '.join(sorted([cat.subject() for cat in item.categories()]))] if item.categories() else [])] + \
...
class TreeViewer(Viewer):  # pylint: disable=W0223
    ...
    def visibleItems(self):
        ...
        for item in yield_items_and_children(self.getRootItems()):
            yield item
...
class ViewerWithColumns(Viewer):  # pylint: disable=W0223 better TreeViewer than Viewer ?
    ...
    def showColumnByName(self, column_name, show=True):
    ...
    def hideColumn(self, visible_column_index):
    ...
    def isVisibleColumnByName(self, column_name):
        return column_name in [column.name() for column in self.__visibleColumns]
    ...
    def validateDrag(self, dropItem, dragItems, columnIndex):
        ...
        if len(set([item.parent() for item in dragItems])) >= 2:
            wx.GetTopLevelParent(self).AddBalloonTip(self.settings, 'treemanualordering',
                                                     self, message=_('''When in tree mode, 
                                                     manual ordering is only possible 
                                                     when all selected items are siblings.'''),
        ...
        if dragItems[0].parent() != (None if dropItem is None else dropItem.parent()):
            wx.GetTopLevelParent(self).AddBalloonTip(self.settings, 'treechildrenmanualordering',
                                                     self, message=_('''When in tree mode, you can only put objects
    ...
    def renderSubjectsOfRelatedItems(self, item, getItems):
        ...
        isListViewer = not self.isTreeViewer()  # pylint: disable=E1101  # where come from ?
        if isListViewer or self.isItemCollapsed(item):
            childItems = [theItem for theItem in getItems(recursive=True, upwards=isListViewer) if
                           theItem not in ownItems]
            if childItems:
    ...
    def isItemCollapsed(self, item):
        ...
        # pylint: disable=E1101
        return not self.getItemExpanded(item) if self.isTreeViewer() and item.children() else False
