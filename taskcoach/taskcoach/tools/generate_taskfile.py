...
def generateCategory(index, children=3, chanceNextLevel=0.2):
    newCategory = category.Category(subject='Category %s: %s' % ('.'.join(index), randomSubject()),
                                    exclusive_subcategories=random.random() < 0.3,
                                    icon=randomIcon(),
                                    fgColor=randomColor(),
                                    bgColor=randomColor(),
                                    font=randomFont(),
    ...
    if random.random() < chanceNextLevel:
        for childNr in range(children):
            child = generateCategory(index + [str(childNr)], chanceNextLevel=old_div(chanceNextLevel, 2))  # //2
...
def generateTask(index, categories, children=3, chanceNextLevel=0.2):
    ...
    if random.random() < chanceNextLevel:
        for childNr in range(children):
            child = generateTask(index + [str(childNr)], categories, chanceNextLevel=old_div(chanceNextLevel, 2))  # //2
