...
class ObservableComposite(Composite):
    ...
    def modificationEventTypes(class_):
        try:
            event_types = super().modificationEventTypes()
        except AttributeError:
            event_types = []
        return event_types + [class_.addChildEventType(),
...
class CompositeCollection(object):
    ...
    def extend(self, composites, event=None):
        ...
        super().extend(compositesAndAllChildren,
                       event=event)
        self._addCompositesToParent(composites, event)

    @staticmethod
    def _compositesAndAllChildren(composites):
    ...
    def remove(self, composite, event=None):
        return self.removeItems([composite], event=event) if composite in self else event
    ...
    @staticmethod
    def _removeCompositesFromParent(composites, event):
        for composite in composites:
            parent = composite.parent()
            if parent:
    ...
    def rootItems(self):
        return [composite for composite in self if composite.parent() is None or
                composite.parent() not in self]  # Expected type collections.iterable, got CompositeCollection instead
