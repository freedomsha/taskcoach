...
class ThunderbirdMailboxReader(object):
    ...
    def __init__(self, url):
        """urlpo is the internal reference to the mail, as collected
        through drag-n-drop."""
    ...
    def read(self):
        ...
        for line in self.fp:
            if not starting:
                if line.startswith('From '):  # expected type bytes tuple, got str instead for 'From '
                    break
            lines.append(line)
            starting = False
