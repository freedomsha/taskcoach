    ...
    def OnSwitchPane(self, event):
        ...
        for k in xrange(2):
            if k == 0:
                ...
                items.AddGroup(_("Toolbars"), "toolbars").break_column()
            ...
            for pane in self._mgr.GetAllPanes():
                ...
                if caption and (toolBar  and k == 1) or (not toolBar and k == 0):
                    items.AddItem(caption, name, -1).set_window(pane.window)

        # Now add the wxAuiNotebook pages

        items.AddGroup(_("Notebook Pages"), "pages").break_column()
        ...
        if ans == wx.ID_OK and dlg.GetSelection() != -1:
            ...
            if item.get_id() == -1:
                info = self._mgr.GetPane(item.get_name())
                info.Show()
                self._mgr.Update()
                info.window.set_focus()

            else:
                nb = item.get_window().GetParent()
                win = item.get_window();
                if isinstance(nb, aui.AuiNotebook):
                    nb.SetSelection(item.get_id())
                    win.set_focus()
...
class SwitcherItem(object):
    ...
    def break_column(self, breakCol=True):
        self._breakColumn = breakCol
        return self

    def get_break_column(self):
...
class SwitcherItems(object):
    ...
    def FindItemByName(self, name):
        ...
        for i in range(len(self._items)):
            if self._items[i].get_name() == name:
                return i
    ...
    def PaintItems(self, dc, win):
        ...
        standardFont = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        groupFont = wx.SystemSettings.get_font(wx.SYS_DEFAULT_GUI_FONT)
        ...
        for i in range(len(self._items)):
            ...
            if i == self._selection:
                ...
                dc.DrawRectangleRect(item.get_rect())

            clippingRect = wx.Rect(*item.get_rect())
            clippingRect.Deflate(1, 1)

            dc.SetClippingRect(clippingRect)

            if item.get_text_colour().IsOk():
                dc.SetTextForeground(item.get_text_colour())
            else:
            ...
            if item.get_font().IsOk():
                ...
                if item.get_is_group():
                    dc.set_font(groupFont)
                else:
                    dc.set_font(standardFont)

            w, h = dc.GetTextExtent(item.get_title())
            x = item.get_rect().x

            x += textMarginX

            if not item.get_is_group():
                if item.get_bitmap().IsOk() and item.get_bitmap().GetWidth() <= 16 \
                        and item.get_bitmap().GetHeight() <= 16:
                    x -= textMarginX
                    dc.DrawBitmap(item.get_bitmap(), x, item.get_rect().y + \
                                  old_div((item.get_rect().height - item.get_bitmap().GetHeight()), 2),
                                  True)
                    x += 16 + textMarginX
                # x += textMarginX

            y = item.get_rect().y + old_div((item.get_rect().height - h), 2)
            dc.DrawText(item.get_title(), x, y)
    ...
    def GetIndexForFocus(self):
        ...
        for i, item in enumerate(self._items):
            if item.get_window():
                ...
                if FindFocusDescendant(item.get_window()):
                    return i
...
class MultiColumnListCtrl(wx.PyControl):
    ...
    def OnKey(self, event):
        ...
        if event.GetEventType() == wx.wxEVT_KEY_UP:
            if event.GetKeyCode() == self.GetModifierKey():
                ...
                closeEvent = wx.CloseEvent(wx.wxEVT_CLOSE_WINDOW, topLevel.get_id())
        ...
        if keyCode in [wx.WXK_ESCAPE, wx.WXK_RETURN]:
            ...
            row = item.get_row_pos()
            newCol = item.get_col_pos() - 1
            if newCol < 0:
            ...
            for i in range(self._items.GetItemCount() - 1, -1, -1):
                ...
                if item2.get_col_pos() == newCol and item2.get_row_pos() <= row:
    ...
    def AdvanceToNextSelectableItem(self, direction):
        ...
        while 1:
            ...
            if self._items.GetItem(self._items.GetSelection()).get_is_group():
        ...
        selection = self._items.GetItem(self._items.GetSelection()).get_window()
    ...
    def CalculateLayout(self, dc=None):
        ...
        while 1:
            ...
            item.set_rect(wx.Rect(x, y, itemSize.x, itemSize.y))
            item.set_col_pos(columnCount - 1)
            item.set_row_pos(currentRow)

            if item.get_rect().GetBottom() > self._overallSize.y:
                self._overallSize.y = item.get_rect().GetBottom() + yMargin

            if item.get_rect().GetRight() > self._overallSize.x:
                self._overallSize.x = item.get_rect().GetRight() + xMargin
            ...
            if currentRow > self._items.GetRowCount() or (item.get_break_column() and not breaking and currentRow != 1):
                currentRow = 0
                columnCount += 1
                ...
                if item.get_is_group() or (item.get_break_column() and not breaking):
                    self._overallSize = oldOverallSize

                    if item.get_break_column():
                        breaking = True
...
class SwitcherDialog(wx.Dialog):
    ...
    def OnPaint(self, event):
        ...
        if self._switcherBorderStyle == wx.BORDER_SIMPLE:
            ...
            rect = wx.Rect(*self._descriptionCtrl.get_rect())
            rect.Inflate(1, 1)
            dc.DrawRectangleRect(rect)
    ...
    def ShowDescription(self, i):
        ...
        html = _("<body bgcolor=\"#") + backgroundColourHex + _("\"><b>") + item.get_title() + _("</b>")
