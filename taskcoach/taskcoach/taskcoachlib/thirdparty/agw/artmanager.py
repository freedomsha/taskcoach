...
class ArtManager(wx.EvtHandler):
        ...
        for i in range(size):
            ...
            if startAtUpperLeft:
                ...
                if rect.width > rect.height:
                    ...
                    xTo = (int(rect.x + sizeX - proportion * i) > rect.x and [int(rect.x + sizeX - proportion * i)] or [
                        rect.x])[0]
    ...
    def MixColours(self, firstColour, secondColour, percent):
        ...
        # calculate gradient coefficients
        redOffset = float(
            (old_div(secondColour.Red() * (100 - percent), 100)) - (old_div(firstColour.Red() * percent, 100)))
        greenOffset = float(
            (old_div(secondColour.Green() * (100 - percent), 100)) - (old_div(firstColour.Green() * percent, 100)))
        blueOffset = float(
            (old_div(secondColour.Blue() * (100 - percent), 100)) - (old_div(firstColour.Blue() * percent, 100)))
        ...
        if style & BU_EXT_RIGHT_TO_LEFT_STYLE:
            ...
            if style & BU_EXT_RIGHT_ALIGN_STYLE:
                ...
                maxWidth = rect.x + rect.width - (
                            2 * alignmentBuffer) - bitmap.GetWidth()  # the alignment is for both sides
