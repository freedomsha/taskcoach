...
class TaskEditorTestCase(tctest.wxTestCase):
    ...
    def setUp(self):
        ...
        self.editor = self.editorClass(
            self.frame, self.getItems(), self.settings, self.taskList, self.taskFile
        )
...
class EditorDisplayTest(TaskEditorTestCase):
    ...
    def createTasks(self):
        ...
        self.task.setRecurrence(
            date.Recurrence("daily", amount=1, stop_datetime=self.stop_datetime)
        )
        return [self.task]
...
class EditTaskTestMixin(object):
    ...
    def testAddAttachment(self):
        ...
        self.assertTrue(
            "filename" in [att.getLocation() for att in self.task.attachments()]
        )
        self.assertTrue(
