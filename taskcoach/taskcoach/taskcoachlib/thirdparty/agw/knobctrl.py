...
class BufferedWindow(wx.Window):
    ...
    def OnSize(self,event):
        ...
        minradius = min(old_div(0.9*self.Width,2), old_div(0.9*self.Height,2))
        memory.DrawCircle(old_div(self.Width,2), old_div(self.Height,2), minradius)
...
class KnobCtrl(BufferedWindow):
    ...
    def DrawTags(self, dc, size):
        ...
        for tags in self._tags:
            ...
            dc.DrawLine(old_div(width,2) - sxi, old_div(height,2) - syi,
                        old_div(width,2) - dxi, old_div(height,2) - dyi) 
    ...
    def DrawInsetCircle(self, dc, pencolour):
        ...
        self._knobcenter = self.CircleCoords(self._minradius*0.8, self.GetTrackPosition(),
                                             old_div(self.Width,2), old_div(self.Height,2))
        ...
        pt1 = wx.Point(cx-old_div(r*math.sqrt(2),2), cy+old_div(r*math.sqrt(2),2))
        pt2 = wx.Point(cx+old_div(r*math.sqrt(2),2), cy-old_div(r*math.sqrt(2),2))
    ...
    def DrawBoundingCircle(self, dc, size):
        ...
        radius = old_div(0.9*min(size.x, size.y),2)
        dc.SetBrush(wx.TRANSPARENT_BRUSH)
        dc.SetPen(wx.Pen(self._boundingcolour))
        dc.DrawCircle(old_div(self.Width,2), old_div(self.Height,2), radius)
    ...
    def SetTrackPosition(self):
        ...
        if self._anglestart < 0 and val >= 360.0 + self._anglestart:
            scaledval = old_div((val  - (360.0 + self._anglestart)),coeff)
        else:
            scaledval = old_div((val  - self._anglestart),coeff)
    ...
    def GetAngleFromCoord(self, cx, cy):
        ...
        ang = 0
        y = old_div((old_div(height,2) - float(cy)),(old_div(height,2))) 
        x = old_div((float(cx) - old_div(width,2)),(old_div(height,2)))
