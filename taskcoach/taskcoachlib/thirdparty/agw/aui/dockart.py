...
class AuiDefaultDockArt(object):
    ...
    **Button Identifiers**
    ...
    def DrawIcon(self, dc, rect, pane):
        ...
        if pane.icon.Ok():
            if pane.HasCaptionLeft():
                ...
                dc.DrawBitmap(bmp.ConvertToBitmap(), rect.x + old_div((rect.width - pane.icon.GetWidth()), 2),
                              rect.y + rect.height - 2 - pane.icon.GetHeight(), True)
            else:
                dc.DrawBitmap(pane.icon, rect.x + 2, rect.y + old_div((rect.height - pane.icon.GetHeight()), 2), True)
    ...
    def DrawCaption(self, dc, window, text, rect, pane):
        ...
        dc.SetPen(wx.TRANSPARENT_PEN)
        dc.set_font(self._caption_font)
        ...
        if captionLeft:
            dc.DrawRotatedText(draw_text, rect.x + (old_div(rect.width, 2)) - (old_div(h, 2)) - 1,
                               rect.y + rect.height - 3 - caption_offset, 90)
        else:
            dc.DrawText(draw_text, rect.x + 3 + caption_offset, rect.y + (old_div(rect.height, 2)) - (old_div(h, 2)) - 1)
    ...
    def DrawSashGripper(self, dc, orient, rect):
        ...
        if orient == wx.HORIZONTAL:  # horizontal sash
            ...
            y = rect.y + (old_div(rect.height, 2)) - 1
            ...
            x = rect.x + (old_div(rect.width, 2)) - 1
...
if _ctypes:
    class RECT(ctypes.Structure):
        ...
        _fields_ = [('left', ctypes.c_ulong), ('top', ctypes.c_ulong), ('right', ctypes.c_ulong),
...
class ModernDockArt(AuiDefaultDockArt):
    ...
    def DrawCaption(self, dc, window, text, rect, pane):
        ...
        if captionLeft:
            dc.DrawRotatedText(draw_text, rect.x + (old_div(rect.width, 2)) - (old_div(h, 2)) - diff,
                               rect.y + rect.height - 3 - caption_offset, 90)
        else:
            dc.DrawText(draw_text, rect.x + 3 + caption_offset, rect.y + (old_div(rect.height, 2)) - (old_div(h, 2)) - diff)
    ...
    def DrawCaptionBackground(self, dc, rect, pane):
        ...
        if self.usingTheme:
            ...
            if active:
                winxptheme.DrawThemeBackground(self.hTheme1, dc.GetHDC(), 5, 1, (rc.top, rc.left, rc.right, rc.bottom),
                                               None)
            else:
                winxptheme.DrawThemeBackground(self.hTheme1, dc.GetHDC(), 5, 2, (rc.top, rc.left, rc.right, rc.bottom),
                                               None)
    ...
    def DrawPaneButton(self, dc, window, button, button_state, rect, pane):
        ...
        if self.usingTheme:
            ...
            try:
                winxptheme.DrawThemeBackground(hTheme, dc.GetHDC(), btntype, state,
