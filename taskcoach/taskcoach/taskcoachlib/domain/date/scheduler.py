...
class TwistedScheduler(object):
    ...
    def __schedule(self, job, date_time, interval):
        if self.__nextCall is not None:
            self.__nextCall.cancel()
            self.__nextCall = None
        index = bisect.bisect_right([v[0] for v in self.__jobs], date_time)
        self.__jobs.insert(index, (date_time, job, interval))
...
class Scheduler(metaclass=patterns.Singleton):
    ...
    def schedule(self, function, date_time):
        job = ScheduledMethod(function)
        self.__scheduler.scheduleDate(job, date_time)
        return job
