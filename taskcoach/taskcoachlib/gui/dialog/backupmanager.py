...
class BackupManagerDialog(wx.Dialog):
    def __init__(self, parent, settings, selectedFile=None):
        ...
        for filename in self.__filenames:
            ...
            item = self.__files.InsertItem(self.__files.GetItemCount(), os.path.split(filename)[-1])
        ...
        if selection is not None:
            self.__files.SetItemState(selection, wx.LIST_STATE_FOCUSED | wx.LIST_STATE_SELECTED,
    ...
    def _OnSelectFile(self, event):
        ...
        for index, dateTime in enumerate(self.__manifest.listBackups(self.__filenames[event.GetIndex()])):
            ...
            self.__backups.InsertItem(index, render.dateTime(dateTime, humanReadable=True))
