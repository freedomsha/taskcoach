...
from . import aquabutton as AB
from . import gradientbutton as GB
    ...
    def Realize(self):
        ...
        if wx.Platform == "__WXMAC__":
            ...
            if self._buttonAffirmative:
                ...
                if self._buttonAffirmative.get_id() == wx.ID_SAVE:
...
class GenericMessageDialog(wx.Dialog):
    ...
    def CreateMessageDialog(self):
        ...
        # populate the sizers...
        icon = wx.StaticBitmap(self, -1, bitmap.get_bitmap())
        ...
        if self._extendedMessage.strip():
            ...
            for index in range(len(msgSizer.GetChildren())):
                msgSizer.GetItem(index).get_window().set_font(boldFont)
        ...
        if size.x < old_div(size.y*3,2):
            size.x = old_div(size.y*3,2)
            self.SetSize(size)
