           ...
           <div class="row">
            ...
            <div class="row">
                ...
                <div class="span4">
                    ...
                    <p><a href="https://twitter.com/share" class="twitter-share-button" data-urlpo="https://taskcoach.org" data-text="Check out Task Coach: a free and open source todo app for Windows, Mac, and Linux." data-count="horizontal" data-via="taskcoach">Tweet</a><script type="text/javascript" src="https://platform.twitter.com/widgets.js"></script></p>
...
pages['changes'] = '''
            ...
            <div class="row">
                <div class="span10">''' + file('changes.html').read().decode('UTF-8') + '''
                    <p><a class="btn" href="allChanges.html">View complete change history &raquo;</a></p>
                </div>
                <div class="span2">''' + ads + '''
                </div>
            </div>'''
...
pages['allChanges'] = '''
            ...
            <div class="row">
                <div class="span10">''' + file('allChanges.html').read().decode('UTF-8') + '''
                </div>
                <div class="span2">''' + ads + '''
                </div>
            </div>'''
...
def download_table(**kwargs):
    ...
    if 'platform' in kwargs:
        download_url_template = '%(action)s <a href="%(urlpo)s">%(package_type)s</a> for %(platform)s from %(source)s'
        platform = kwargs['platform']
        kwargs['platform_versions'] = 'Platforms' if platform == 'all platforms' else platform + ' versions'
    else:
        download_url_template = '%(action)s <a href="%(urlpo)s">%(package_type)s</a> from %(source)s'
        kwargs['platform_versions'] = 'Platforms'
    urls = []
    for source, url in list(kwargs['download_urls'].items()):
        parameters = kwargs.copy()
        parameters['urlpo'] = url
...
def languages(nr_columns=9):
    ...
    while len(languages)%nr_columns:
        languages.append('')
    nr_of_rows = old_div(len(languages),nr_columns)
    rows = []
...
def createThumbnail(srcFilename, targetFolder, bitmapType=wx.BITMAP_TYPE_PNG,
    ...
    scaleFactor = old_div(thumbnailWidth, image.Width)
