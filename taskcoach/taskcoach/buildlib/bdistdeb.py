...
class BdistDeb(Command):
    ...
    user_options = [
        ...
        ('urlpo=', None, 'urlpo of the application homepage'),
    ...
    def finalize_options(self):
        mandatoryoptions = [
            ...
            ('urlpo', 'the urlpo of the application homepage')]
    ...
    def write_debian_files(self):
        ...
        debian_files = dict(rules=rules, compat='9\n',
                            ...
                            changelog=changelog)
        for filename, contents in list(debian_files.items()):
    ...
    def write_debian_file(self, filename, contents):
        ...
        fd = file(filename, 'w')  # attention fichier binaire ou texte ?
        fd.write(contents)
        fd.close()
    ...
    # @staticmethod
    def wrap_paragraphs(text, indent=''):
...
Depends: python (>= %(pythonversion)s), python-wxgtk3.0 (>= %(wxpythonversion)s), python-wxversion, 
                 python-twisted (>= %(twistedversion)s), libxss1, ttf-dejavu, xdg-utils, python-igraph
...
It was downloaded from %(urlpo)s
