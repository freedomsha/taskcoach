...
class CalendarCanvas(wx.Panel):
    ...
    def HitTest(self, x, y):
        ...
        xIndex = int(old_div(x, self._eventWidth))
        yIndex = int(old_div((y - self._marginTop), (self._eventHeight + self._margin)))
        ...
        for event, (startIndex, endIndex, startIndexRecursive, endIndexRecursive, yMin, yMax) in list(self._coords.items()):
            ...
            if startIndexRecursive <= xIndex < endIndexRecursive and yMax > yIndex >= yMin:
                # May be a child
                children = []
                ...
                for candidate in reversed(children):
                    if candidate in self._coords:
                        ...
                        # if si is not None and abs(x - si * self._eventWidth)
                        # <= self._margin and yIndex >= ymin and yIndex < ymax:
                        if si is not None and abs(x - si * self._eventWidth) <= self._margin and ymin <= yIndex < ymax:
                            result = _HitResult(x, y, candidate, dateTime)
                            result.position = result.HIT_START
                            return result
                        # if ei is not None and abs(x - ei * self._eventWidth)
                        # <= self._margin and yIndex >= ymin and yIndex < ymax:
                        if ei is not None and abs(x - ei * self._eventWidth) <= self._margin and ymin <= yIndex < ymax:
                            result = _HitResult(x, y, candidate, dateTime)
                            result.position = result.HIT_END
                            return result
                        # if xIndex >= sir and xIndex < eir and yIndex >= ymin and yIndex < ymax:
                        if sir <= xIndex < eir and ymax > yIndex >= ymin:
                            result = _HitResult(x, y, candidate, dateTime)
    ...
    def _DrawHeader(self, gc, w, h):
        ...
        for startIndex, endIndex in self._headerSpans:
            ...
            text = shortenText(gc, self.FormatDateTime(
                self._start + datetime.timedelta(minutes=self._precision * startIndex)), x1 - x0)
            tw, th = gc.GetTextExtent(text)
            gc.DrawText(text, x0 + old_div((x1 - x0 - tw), 2), old_div((self._marginTop - 2.0 - th), 2))
    ...
    def _OnMotion(self, event):
        ...
        if self._mouseState in [self.MS_DRAG_LEFT, self.MS_DRAG_RIGHT]:
            ...
            if self._mouseState == self.MS_DRAG_LEFT:
                dateTime = self._start + datetime.timedelta(seconds=math.floor(old_div(old_div(total_seconds(dateTime - self._start), 60), precision)) * precision * 60)
            ...
            if self._mouseState == self.MS_DRAG_RIGHT:
                dateTime = self._start + datetime.timedelta(seconds=math.ceil(old_div(old_div(total_seconds(dateTime - self._start), 60), precision)) * precision * 60)
    ...
    def _DrawParent(self, gc, startIndex, endIndex, startIndexRecursive, endIndexRecursive, y, yMax, event, w):
        ...
        # Span
        path = gc.CreatePath()
        delta = old_div(self._eventHeight, 4)
