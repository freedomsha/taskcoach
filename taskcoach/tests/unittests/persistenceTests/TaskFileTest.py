...
class ChangingAttachmentsTestsMixin(object):
    ...
    def addAttachment(self, anAttachment):
        print(f"TaskFileTest.ChangingAttachmentsTestsMixin.addAttachment : 🛠️ DEBUG - Création d'une tâche self={self} avec attachements: {self.attachments}, anAttachment={anAttachment}")
    ...
    def addMailAttachment(self):
        self.mailAttachment = attachment.MailAttachment(
            self.filename, readMail=lambda location: ("", "")  # pylint: disable=W0201
        )
...
class TaskFileMonitorTestBase(TaskFileTestCase):
    ...
    def testGUIDPresentAfterLoad(self):
        self.failUnless(
            self.taskFile.monitor().guid() in self._loadChangesFromFile(self.filename)
        )
    ...
    def testDeleteObject(self):
        ...
        self.assertEqual(
            allChanges[self.otherFile.monitor().guid()].getChanges(self.task), None
        )
...
class TaskFileMultiUserTestBase(object):
    ...
    def _testChangeAttribute(self, name, value, listName):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            getattr(getattr(self.taskFile2, listName)().rootItems()[0], name)(), value
        )
    ...
    def testChangeExclusiveSubcategories(self):
        ...
        self.doSave(self.taskFile2)
        self.assert_(
            self.taskFile2.categories().rootItems()[0].hasExclusiveSubcategories()
        )
    ...
    def _testDeleteModifiedRemoteObject(self, listName):
        ...
        self.assertEqual(len(getattr(self.taskFile2, listName)()), 1)
        self.assertEqual(
            getattr(self.taskFile2, listName)().rootItems()[0].subject(), "New subject."
        )
    ...
    def _testAddNoteToObject(self, listName):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            len(getattr(self.taskFile2, listName)().rootItems()[0].notes()), noteCount
        )
    ...
    def testAddNoteToAttachment(self):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            len(self.taskFile2.tasks().rootItems()[0].attachments()[0].notes()), 1
        )
    ...
    def _testAddAttachmentToObject(self, listName):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            len(getattr(self.taskFile2, listName)().rootItems()[0].attachments()),
            attachmentCount,
        )
    ...
    def testRemoveNoteFromAttachment(self):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            len(self.taskFile2.tasks().rootItems()[0].attachments()[0].notes()), 0
        )
    ...
    def testChangeNoteBelongingToTask(self):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            self.taskFile2.tasks().rootItems()[0].notes()[0].subject(), "New subject"
        )
    ...
    def testChangeEffortStart(self):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            self.taskFile2.tasks().rootItems()[0].efforts()[0].getStart(), newDate
        )
    ...
    def testChangeEffortStop(self):
        ...
        self.doSave(self.taskFile2)
        self.assertEqual(
            self.taskFile2.tasks().rootItems()[0].efforts()[0].getStop(), newDate
        )
