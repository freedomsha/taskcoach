        ...
        if self.use_gc:
            if h is not None:
                ...
                brush = self.context.CreateLinearGradientBrush(
                    x, y, x + w, y + h, schedule.color, SCHEDULER_BACKGROUND_BRUSH()
                )
                ...
                if h is not None:
                    ...
                    if schedule.complete:
                        ...
                        self.context.DrawRectangle(
                            x + SCHEDULE_INSIDE_MARGIN,
                            y + offsetY,
                            int((w - 2 * SCHEDULE_INSIDE_MARGIN) * schedule.complete),
                            10,
                        )
...
class HeaderDrawerDCMixin(object):
    ...
    def _DrawHeader(
        ...
        if alignRight:
            self.context.DrawText(text, x + w - textW * 1.5, y + textH * 0.25)
        else:
            self.context.DrawText(text, x + (w - textW) / 2, y + textH * 0.25)
    ...
    def DrawSchedulesCompact(self, day, schedules, x, y, width, height, highlightColor):
        ...
        if day is not None:
            ...
            for schedule in schedules:
                ...
                results.append(
                    (schedule, wx.Point(x, y), wx.Point(x + width, y + textH * 1.2))
                )
...
class HeaderDrawerGCMixin(object):
    ...
    def _DrawHeader(
        ...
        try:
            ...
            if alignRight:
                self.context.DrawText(text, x + w - 1.5 * textW, y + textH * 0.25)
            else:
                self.context.DrawText(text, x + (w - textW) / 2, y + textH * 0.25)
        ...
        try:
            ...
            if day is not None:
                ...
                for schedule in schedules:
                    ...
                    self.context.SetBrush(brush)
                    self.context.DrawRoundedRectangle(
                        x, y, width, textH * 1.2, 1.0 * textH / 2
                    )
                    results.append(
                        (schedule, wx.Point(x, y), wx.Point(x + width, y + textH * 1.2))
                    )
            ...
            for i, hour in enumerate(self.displayedHours):
                if hour.GetMinute() == 0:
                    if direction == wxSCHEDULER_VERTICAL:
                        ...
                        self.context.DrawLine(
                            x + i * hourW, y + hourH * 1.25, x + i * hourW, y + h
                        )
                        if includeText:
                            self.context.DrawText(
                                wxTimeFormat.FormatTime(hour),
                    ...
                    if direction == wxSCHEDULER_VERTICAL:
                        self.context.DrawLine(
                            x + LEFT_COLUMN_SIZE, y + i * hourH, x + w, y + i * hourH
                        )
                    else:
                        self.context.DrawLine(
                            x + i * hourW, y + hourH * 1.4, x + i * hourW, y + h
                        )

            if direction == wxSCHEDULER_VERTICAL:
                self.context.DrawLine(
                    x + LEFT_COLUMN_SIZE - 1, y, x + LEFT_COLUMN_SIZE - 1, y + h
                )
...
class wxFancyDrawer(
    ...
    def DrawHours(self, x, y, w, h, direction, includeText=True):
        if direction == wxSCHEDULER_VERTICAL:
            brush = self.context.CreateLinearGradientBrush(
                x, y, x + w, y + h, SCHEDULER_BACKGROUND_BRUSH(), DAY_BACKGROUND_BRUSH()
            )
            self.context.SetBrush(brush)
            self.context.DrawRectangle(x, y, LEFT_COLUMN_SIZE, h)
