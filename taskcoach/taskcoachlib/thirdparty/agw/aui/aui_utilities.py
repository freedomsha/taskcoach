...
def GetDockingImage(direction, useAero, center):
    ...
    if direction == wx.TOP:
        bmp_unfocus = eval("%sup%s"%(prefix, suffix)).get_bitmap()
        bmp_focus = eval("%sup_focus%s"%(prefix, suffix)).get_bitmap()
    elif direction == wx.BOTTOM:
        bmp_unfocus = eval("%sdown%s"%(prefix, suffix)).get_bitmap()
        bmp_focus = eval("%sdown_focus%s"%(prefix, suffix)).get_bitmap()
    elif direction == wx.LEFT:
        bmp_unfocus = eval("%sleft%s"%(prefix, suffix)).get_bitmap()
        bmp_focus = eval("%sleft_focus%s"%(prefix, suffix)).get_bitmap()
    elif direction == wx.RIGHT:
        bmp_unfocus = eval("%sright%s"%(prefix, suffix)).get_bitmap()
        bmp_focus = eval("%sright_focus%s"%(prefix, suffix)).get_bitmap()
    else:
        bmp_unfocus = eval("%stab%s"%(prefix, suffix)).get_bitmap()
        bmp_focus = eval("%stab_focus%s"%(prefix, suffix)).get_bitmap()
...
def RescaleScreenShot(bmp, thumbnail_size=200):
    ...
    if bmpW > bmpH:
        if bmpW > thumbnail_size:
            ...
            newW, newH = int(old_div(bmpW,ratio)), int(old_div(bmpH,ratio))
