...
class SButton(wx.Window):
    ...
    def DrawMainButton(self, dc, width, height):
        ...
        if abs(main - secondary) < 1e-6:
            ...
            if main > secondary:
                ...
                rect2 = w
                rect3 = old_div(w*secondary,main)

            else:
                # This Is An Ellipse With Main Axis Aligned With Y Axis
                rect3 = w
                rect2 = old_div(w*main,secondary)
        ...
        if abs(main - secondary) < 1e-6:
            if height > width:
                xpos = 0
                ypos = old_div((height - width),2)
            else:
                xpos = old_div((width - height),2)
                ypos = 0
        else:
            if height > width:
                if main > secondary:
                    xpos = 0
                    ypos = old_div((height - rect3),2)
                else:
                    xpos = old_div((width - rect2),2)
                    ypos = old_div((height - rect3),2)
            else:
                if main > secondary:
                    xpos = old_div((width - rect2),2)
    ...
    def DrawLabel(self, dc, width, height, dw=0, dh=0):
        ...
        if angle == 0:
            dc.DrawText(label, old_div((width-tw),2)+dw, old_div((height-th),2)+dh)
        else:
            xc, yc = (old_div(width,2), old_div(height,2))

            xp = xc - (old_div(tw,2))* cos(angle) - (old_div(th,2))*sin(angle)
            yp = yc + (old_div(tw,2))*sin(angle) - (old_div(th,2))*cos(angle)

            dc.DrawRotatedText(label, xp + dw, yp + dh , old_div(angle*180,pi))
    ...
    def DrawFocusIndicator(self, dc, width, height):
        ...
        if abs(main - secondary) < 1e-6:
            ...
            if height > width:
                dc.DrawCircle(old_div(width,2), old_div(height,2), old_div(width,2)-4)
            else:
                dc.DrawCircle(old_div(width,2), old_div(height,2), old_div(height,2)-4)
        else:
            # This Is An Ellipse
    ...
    def IsOutside(self, x, y):
        ...
        if abs(main - secondary) < 1e-6:
            ...
            if ((old_div((x-xc),main))**2.0 + (old_div((y-yc),secondary))**2.0) > 1:
                return True
    ...
    def SetAngleOfRotation(self, angle=None):
        ...
        self._rotation = old_div(angle*pi,180)
...
class SBitmapButton(SButton):
    ...
    def DrawLabel(self, dc, width, height, dw=0, dh=0):
        ...
        dc.DrawBitmap(bmp, old_div((width - bw),2) + dw, old_div((height - bh),2) + dh, hasMask)
...
class SBitmapTextButton(SBitmapButton):
    ...
    def DrawLabel(self, dc, width, height, dw=0, dh=0):
        ...
        pos_x = old_div((width - bw - tw),2) + dw      # adjust for bitmap and text to centre

        rotangle = self.GetAngleOfRotation()*pi/180.0

        if bmp != None:
            if rotangle < 1.0/180.0:
                dc.DrawBitmap(bmp, pos_x, old_div((height - bh),2) + dh, hasMask) # draw bitmap if available
                pos_x = pos_x + 4   # extra spacing from bitmap
            else:
                pass

        if rotangle < 1.0/180.0:
            dc.DrawText(label, pos_x + dw + bw, old_div((height-th),2)+dh)      # draw the text
        else:
            xc, yc = (old_div(width,2), old_div(height,2))
            xp = xc - (old_div(tw,2))* cos(rotangle) - (old_div(th,2))*sin(rotangle)
            yp = yc + (old_div(tw,2))*sin(rotangle) - (old_div(th,2))*cos(rotangle)
